...
| **IMDb ratings** *(second v1.1 epic, owner-decided `A50`)* | **REQ-088 – REQ-093** | **v1 closes.** Specified in full — PRD Epic M / US-044…US-046, and **ADR-0011** for the load-bearing decisions. It depends on Epic F (the combined list) and on TMDB matching, because a rating is keyed on the `imdb_id` matching produces. ⚠️ **IMDb has no free API** — the official one is ~$150k/year, and the free bulk dataset was rejected because ingesting it needs a scheduled job. The source is **OMDb**, owner-chosen over TMDB's own `vote_average`. ⚠️ Ratings are looked up by **`imdb_id` only, never by title text** (ADR-0011 D-2); a title-text fallback attaches a plausible rating to the wrong film. ⚠️ Promotion **requires incrementing US-036 AC-2 and `T-CI-005` by one**, in the same change — read the current count rather than writing a literal, since Epic L claims an increment too and the right value depends on which is promoted first. |
