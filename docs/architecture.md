...
| NFR-010 | No automated requests of any kind to a streaming service | Only five outbound destinations exist: TMDB, Azure OpenAI, Azure AI Vision, OMDb (ADR-0011) and Watchmode (ADR-0010 Rev 3, #380 — a release-date aggregator sent a date window only, not a streaming service). None is a streaming service, and only the two extractors are ever sent screenshot bytes | Outbound host allow-list test (US-038 AC-2/AC-5), `T-SEC-031` |
...
| **Runtime filter and sort (REQ-035/037)** | ~~v1.1, decision D2 — TMDB runtime is ambiguous for TV and the record contains no decision. Runtime is still stored, so v1.1 is additive~~ **PROMOTED INTO SCOPE at `A48`.** The blocker was never the data — `Title.tmdbRuntimeMinutes` has been stored since v1 — it was the missing TV semantic, which is now decided: TMDB gives series a per-episode `episode_run_time`, `tmdbClient.readRuntime` already takes it, and REQ-119 requires the label to say `45m/ep` so the number cannot be misread as a series total. Additive as predicted: no migration, no new column | ~~A decision on TV runtime semantics~~ — **settled at `A48`** |
