...
|---|---|---|---|
| **ASM-059** *(new — v1.1, confirmed at `A49` 2026-08-20)* | The owner's streaming region for availability lookups is **`US`**. TMDB watch-provider data is region-specific, so this determines every availability answer in Epic L (REQ-086) | **High — user-stated (`A49`), never an open assumption in practice: it was raised as `OQ-030` and answered in the same exchange.** Recorded because a *silent* region default would be an unfindable, load-bearing constant | The owner relocates, or wants availability for a second region. ⚠ Neither is a code change **provided** `availabilityRegion` was stored on the row and passed explicitly (US-042 AC-10) rather than hard-coded — which is precisely why that AC exists |
