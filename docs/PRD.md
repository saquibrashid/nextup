...
| AC-3 | A batch in **append-only** mode | The review pass renders | All extracted titles remain visible, including already-present matches with an explicit "Nothing to add" next step. Only removals are absent. The owner-approved tile-first revision separates accounting for each source tile from confirming new additions (REQ-011) |
...
**Open questions:** ~~OQ-015~~ — **CLOSED in phase 8.** See AC-6′ and AC-7. The identity grammar is specified in full in `specs/data-model.md` (`workIdentity`, `normaliseTitleText`); the implementer MUST use it rather than inventing a key.
...
| AC-3 | The purge | It runs | It changes no user-visible list state, and is therefore one of exactly four permitted non-owner-initiated processes (REQ-041, US-036) |
...
| AC-8 | `WATCH_PROVIDER_MAX_AGE_DAYS` | The source is inspected | It is a **fourth, independent** constant, declared separately from `TMDB_METADATA_MAX_AGE_DAYS = 183` (NFR-014), `IMAGE_RETENTION_DAYS = 30` (NFR-019) and `IMDB_RATING_MAX_AGE_DAYS = 14` (Epic M), with no shared call site — the `T-INV-008` rule extended to four (ADR-0010 Trap 5). ~~"a **third, independent** constant … extended to three"~~ *(Superseded — corrected in place because this criterion is executed literally: Epic M added a fourth age constant after ADR-0010 was written. The rule is unchanged; only the count was stale.)* |
...
| Review — additions | Loading candidates | Distinguish no candidates, known-only, unidentified and discarded additions | Match failure banner; unmatched section populated (US-008) | Saved cards stay authoritative; separate unsaved intent requires explicit online reconciliation, never reconnect replay | Additions confirmed; decisions remain editable until Apply |
...
⚠ **ASM-058's RATIONALE was itself falsified at TASK-151 (2026-08-20); its CONCLUSION is unchanged and strengthened.** The rationale argued from a per-path format map — *screenshots are PNG, camera photos are HEIC, so all three must be accepted*. The owner's own screenshot fixture is **JPEG**, with an EXIF block, no GPS and no device model (`T-SEC-032l`). **The correct reasoning is unpredictability, not a map: no capture path's output format can be predicted, so accept all three and determine the format by magic bytes only** — never from the declared `Content-Type` (iOS commonly sends `application/octet-stream`) and never inferred from the ingest source. ~~Screenshots are normally PNG while camera photos are HEIC.~~ This is the **fifth** agent-derived inference corrected on contact with real data, and the second time the correction *reinforced* the conclusion it was offered to support — which is exactly why a right answer held for a wrong reason is still a defect: the next person to reason from the same premise will reach a different, wrong conclusion.
