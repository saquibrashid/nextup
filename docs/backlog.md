...
> 0.25/0.5), **TASK-007** (deploy workflow — ghcr.io push + fine-grained PAT,
...
|---|---|---|---|---|---|
| TASK-001 | US-039 | npm-workspaces monorepo: `package.json` (workspaces `packages/domain`, `apps/api`, `apps/web`), root `tsconfig.base.json` + per-workspace `tsconfig.json` (strict, `noUncheckedIndexedAccess`), `.eslintrc.cjs`, `.prettierrc`, `.editorconfig`, `.gitignore`. | S | — | `npm ci && npm run lint && npm run build` succeeds on a clean clone |
...
| TASK-146 **(new, R4)** | US-039 | **ghcr.io PAT lifecycle runbook (`RSK-031`/registry give-up).** Dropping ACR's managed-identity pull reintroduces a **quiet-expiry credential**: a fine-grained PAT scoped `read:packages` (pull) and a separate `write:packages` push identity. Create `docs/ghcr-pat.md` documenting: generation with the **longest permitted expiry**, storage as a **Key Vault-referenced ACA secret**, rotation steps, and a **calendar reminder** ahead of expiry. The failure mode is a deployment that breaks weeks later with an auth error for no code reason — the runbook names that symptom explicitly so it is diagnosable. | XS | 006, 007 | `docs/ghcr-pat.md` exists with generation, storage, rotation and a dated reminder; the PAT is never committed; a comment in `deploy.yml` links to it |
...
| TASK-017 **(REVISED, R3; R4; R7)** | `prisma/schema.prisma` generated from the DDL in `specs/data-model.md` §15.3 (nine tables, the partial unique indexes, the `CHECK` constraint) + the initial migration + `apps/api/src/repository/ownerData.ts` — **`ownerId` is the first positional parameter of every method**, applied as an `owner_id` filter on every read and write; `docker-compose.test.yml` with **`postgres:16-alpine`** + Azurite, migrations applied before the suite. ~~`parseOrThrow` on every read~~ — Prisma's generated types are the read contract; Zod stays at the API boundary only. **↳ R4 (A40):** the authoritative DDL is now **§16.3** (Azure SQL T-SQL). Prisma **stays** (provider `sqlserver`, `datasource db { provider = "sqlserver" }`), but **the SQL-Server-specific DDL — filtered unique indexes (§16.4), `CHECK` constraints, `ISJSON`, `Latin1_General_100_BIN2` collation on identity/key columns — lives in RAW migration SQL**, not in Prisma model attributes (Prisma's `sqlserver` modeling is thinner). The fixture container becomes **`mcr.microsoft.com/mssql/server:2022-latest`** (`ACCEPT_EULA=Y`, `MSSQL_SA_PASSWORD`, ~2 GB, health-wait — see `testing.md` §3.3a); `sqlcmd` is at `/opt/mssql-tools18/bin/sqlcmd` and needs `-C`. Upsert uses **explicit UPDATE-then-INSERT-if-zero-rows, NOT `MERGE`** (SQL Server `MERGE` has correctness sharp edges). `owner_id` filter is unchanged. The `23505` assertions become **`2627`/`2601`**. <br> **↳ R7 (`A45`) — the two new `uploaded_image` columns are owned HERE; NO new migration task was created.** Per `specs/data-model.md` §16.3, `uploaded_image` gains **`file_name NVARCHAR(255) NOT NULL`** with `CONSTRAINT ck_image_file_name CHECK (LEN(LTRIM(RTRIM(file_name))) > 0)` — display only, **never used to compose `blob_path`** — and **`ingest_source NVARCHAR(16) NOT NULL`** with `CONSTRAINT df_image_ingest_source DEFAULT 'upload'` and `CONSTRAINT ck_image_ingest_source CHECK (ingest_source IN ('paste','upload','drop'))`. Both live in the **raw migration SQL** alongside the other SQL-Server-specific DDL, not in Prisma model attributes. **The migration is ADDITIVE — `ALTER TABLE ... ADD` plus the CHECK constraints; no column is dropped and no row is rewritten, so `T-MIG-001` (TASK-144) is unaffected.** The `'upload'` default is deliberate and is a **true statement about history**: every row predating A45 did arrive by upload. `retain_until` is untouched — 30 days applies identically to a pasted image. | **L** *(was M)* | 012 | Integration suite green offline against **mssql/server:2022**; a method without `ownerId` fails typecheck; `T-INV-001/002/015` assert the **database** raises `2627`/`2601`; `T-SEC-021` fails any Prisma `where` without `ownerId` |
...
| TASK-029 | `tests/security/ownership.spec.ts` — route-enumerated allow-list coverage; cross-owner access returns **404, not 403**. | S | 023 | `T-SEC-017`, `T-SEC-002`, `T-SEC-028`, `T-SEC-030` |
...
| TASK-056 | `apps/api/src/extraction/azureVisionExtractor.ts` — Read F0, managed identity, timeouts, bounded retries, no PII in logs. **Still shipped: it is the cross-check leg and the one-config-value revert to ADR-0001 Rev 1.** | S | 055 | `T-AI-001`, `T-AI-002`, `T-AI-009` |
...
| Task | Description | Size | Depends on | Done when |
|---|---|---|---|---|
| TASK-045 | `apps/api/src/clients/tmdbClient.ts` + `GET /api/tmdb/search` + msw fixtures + `TMDB_UNAVAILABLE` handling. | S | 023 | `T-TMDB-001`, `T-TMDB-002` |
| TASK-060 | `packages/domain/src/matching/tmdbMatcher.ts` — **deterministic** scoring (jaro-winkler), `MATCH_AUTO_THRESHOLD = 0.92`, `MATCH_REVIEW_FLOOR = 0.70`, alternatives list, year hint. **No ML, no AI call.** | M | 045 | `T-AI-009`, `T-AI-010`, `T-AI-011` |
| TASK-061 | TMDB metadata allow-list storage — a Zod schema that **rejects** unlisted fields rather than stripping them. | S | 060 | `T-TMDB-004`, `T-DM-011` |
...
| Task | Description | Size | Depends on | Done when |
|---|---|---|---|---|
| TASK-064 | `packages/domain/src/classify.ts` — new vs already-present per service. | S | 060, 063 | `T-CLS-001`, `T-CLS-002`, `T-CLS-003` |
...
| Task | Description | Size | Depends on | Done when |
|---|---|---|---|---|
| TASK-068 | Unmatched handling UI + `unresolvedKept` recorded on close — an unmatched candidate is **kept**, never discarded. | S | 065, 071 | `T-REV-009`, `T-AI-024` |
...
| Task | Description | Size | Depends on | Done when |
|---|---|---|---|---|
| TASK-065 | `GET /api/batches/:batchId/review` — sections `additions`, `unmatched`, `probablyNotTitles`; shape exactly per `api.md` §6.17. | M | 064 | `T-REV-001`, `T-REV-002`, `T-REV-003` |
| TASK-066 | `PATCH /api/batches/:batchId/candidates/:candidateId` — disposition changes, `reclassifyAsTitle`, confirm-all. | S | 065 | `T-REV-004`, `T-REV-010` |
| TASK-069 | `apps/web/src/pages/ReviewPage.tsx` + `components/CandidateCard.tsx` + sticky action bar + section empty states. | M | 065 | `T-UI-005`, `T-UX-010`, `T-UX-011` |
...
| TASK-071 | `POST /api/batches/:batchId/close` — `PENDING_ADDITIONS` guard, apply confirmed and corrected candidates, return a summary. | M | 066, 054 | `T-BATCH-008`, `T-REV-011` |
...
| Task | Description | Size | Depends on | Done when |
|---|---|---|---|---|
| TASK-083 | `packages/domain/src/removals.ts` — full-update removals, **service-scoped**, excluding already-removed and suppressed works. | M | 081 | `T-REM-001`, `T-REM-002`, `T-REM-003` |
...
| Task | Description | Size | Depends on | Done when |
|---|---|---|---|---|
| TASK-085 | `PATCH /api/batches/:batchId/removals` — tick/untick; **ticked by default**. | S | 083 | `T-UI-007`, `T-REM-004` |
...
| TASK-091 | Undo affordance shown immediately after confirmation + an undo entry written to the batch record. | S | 090 | `T-UI-015`, `T-UX-013` |
...
| Task | Description | Size | Depends on | Done when |
|---|---|---|---|---|
| TASK-033 | `GET /api/titles` — active and visible only, suppression anti-join, service badges, cursor pagination. | M | 017, 023 | `T-LIST-001`, `T-LIST-002`, `T-API-004` |
| TASK-034 | `GET /api/titles/:titleId`. | XS | 033 | `T-LIST-003` |
| TASK-038 | `apps/web/src/pages/ListPage.tsx` + `components/TitleList.tsx` + `TitleRow.tsx` — badges, poster, **deep links out to the service**. | M | 033, 025 | `T-UI-001`, `T-UI-002`, `T-LINK-001` |
| TASK-040 | Two **distinct** empty states (nothing imported yet vs everything filtered out) + a load-failure error state. | S | 038 | `T-UX-001`, `T-UX-002`, `T-UX-003` |
...
| Task | Description | Size | Depends on | Done when |
|---|---|---|---|---|
| TASK-037 | Filters by service, type, genre — **AND across dimensions, OR within**; a work with `genres: []` matches no genre filter. | S | 033 | `T-LIST-006`, `T-LIST-007`, `T-LIST-008` |
| TASK-039 | `components/FilterBar.tsx` + query-string sync + zero-match state. | S | 037, 038 | `T-UI-016`, `T-UX-004` |
...
| TASK-036 | Ordering with a `title.id` ascending tie-breaker, nulls last, `dir` parameter. | S | 016, 033 | `T-LIST-009`, `T-LIST-010` |
...
| TASK-042 | `components/FreshnessStrip.tsx` + a degraded state when service state is unavailable. | XS | 041 | `T-UI-017` |
...
| Task | Description | Size | Depends on | Done when |
|---|---|---|---|---|
| TASK-043 | **Lazy** TMDB refresh inline on read paths, `TMDB_METADATA_MAX_AGE_DAYS = 183`, `metadataStale` flag, 5-second budget, never blocking the response beyond it. | M | 045, 033 | `T-TMDB-005`, `T-TMDB-006`, `T-CI-005` |
...
| Task | Description | Size | Depends on | Done when |
|---|---|---|---|---|
| TASK-024 | `GET /api/me` returning the attribution payload. | XS | 023 | `T-API-005` |
...
| Task | Description | Size | Depends on | Done when |
|---|---|---|---|---|
| TASK-047 **(REVISED, R3; R4)** | The indexes of `specs/data-model.md` §15.6 in a Prisma migration (`title_list_default`, `listing_removed_view`, `listing_by_title`, `batch_change_by_batch`, `candidate_by_batch`, plus `pg_trgm` GIN on `tmdb_name` and `normalised_text`) + **keyset pagination** in the repository + an `EXPLAIN`-based query-plan harness. ~~Composite indexes in `infra/cosmos.bicep` + a query-cost harness.~~ **`OFFSET` MUST NOT be used** — it degrades with history size and would break the exact `NFR-018` claim these indexes exist to defend. **↳ R4 (A40):** the authoritative index set is now **§16.6** (Azure SQL). The B-tree/keyset indexes port directly. **`pg_trgm` GIN is GONE** — Azure SQL Basic has no trigram index; fuzzy search becomes **`LIKE N'%term%' COLLATE Latin1_General_100_CI_AI`** per-query (NOT index-backed, loses typo tolerance; escape `%_[` metacharacters and parameterize — SQLi). Keyset listing pagination is **unaffected** (still index-backed). Full-Text Search is a named escalation, not built now. The query-plan harness uses **`SET STATISTICS IO`/actual execution plan**, not `EXPLAIN`. | **S** *(was XS)* | 006, 033 | `T-PERF-001` (index seek, rows-read bounded by page size, **no table scan** on `service_listing`), `T-PERF-003`; the search `LIKE` is parameterized and its metacharacters escaped |
| TASK-095 | Removed-view query with ordinals, search, and service filter. | M | 088, 047 | `T-REM-005`, `T-REM-006`, `T-REM-007` |
...
| Task | Description | Size | Depends on | Done when |
|---|---|---|---|---|
| TASK-098 | `POST /api/listings/:listingId/restore` with 409s: `WORK_SUPPRESSED`, `DUPLICATE_WORK_IDENTITY`, `LISTING_NOT_REMOVED`. | M | 095, 101 | `T-REM-008`, `T-REM-009`, `T-REM-010` |
| TASK-099 | Restore UI + the un-suppress-first flow. | S | 098 | `T-UX-014`, `T-UI-018` |
...
| Task | Description | Size | Depends on | Done when |
|---|---|---|---|---|
| TASK-106 | `GET /api/suppressions` + unsuppress — the suppression record is **never deleted**; `restoredAnything: false` when nothing came back. | S | 101 | `T-SUP-007`, `T-SUP-008` |
...
| Task | Description | Size | Depends on | Done when |
|---|---|---|---|---|
| TASK-074 | Provenance model + `createdByBatchId` + `provenance.created`. **Close fails atomically if provenance cannot be written.** | M | 071 | `T-PROV-001`, `T-PROV-002`, `T-PROV-003` |
| TASK-075 | `provenance.modified` for in-review corrections; out-of-batch edits record `batchId: null`. | S | 074 | `T-PROV-004`, `T-PROV-005` |
| TASK-076 | `GET /api/batches` + `apps/web/src/pages/BatchHistoryPage.tsx` + batch-detail provenance view. | S | 074 | `T-UI-019`, `T-BATCH-009` |
...
| Task | Description | Size | Depends on | Done when |
|---|---|---|---|---|
| TASK-109 | `POST /api/titles/:titleId/fix-match` — preserves listings, dates, and sort position; 409s enumerated. | M | 074, 060 | `T-FIX-001`, `T-FIX-002`, `T-FIX-003` |
| TASK-110 | **SD-06** suppression migration on identity change + `FIXMATCH_SUPPRESSION_MIGRATED` (this is US-028 **AC-7**). | S | 109, 101 | `T-FIX-005`, `T-SUP-009` |
| TASK-111 | `components/FixMatchDialog.tsx` + a TMDB-unavailable state. | S | 109 | `T-UI-020`, `T-UX-015` |
...
| Task | Description | Size | Depends on | Done when |
|---|---|---|---|---|
| TASK-114 | Refusal payload: full enumeration, `truncated: false`, per-item remedies, `currentState`. **Nothing is written on a refusal.** | M | 113 | `T-UNDO-005`, `T-UNDO-007` |
| TASK-115 | A 400-title mixed fixture proving the enumeration is complete and untruncated. | S | 114 | `T-UNDO-006` |
| TASK-116 | Full-screen refusal repair panel with `UNDO_REFUSAL_*` copy and per-title actions. | M | 114 | `T-UI-021`, `T-UX-016` |
...
| TASK-124 | axe-core suite across all nine routes — **zero serious or critical**. | S | 123 | `T-A11Y-003` … `T-A11Y-012` |
...
| TASK-089 | `tests/invariants/no-hard-delete.spec.ts` — static **and** behavioural, with the two documented exemptions. | XS | 088 | `T-INV-012` |
...
| TASK-007 | Deploy workflow with a traffic-gated revision (**R4:** ghcr.io push + fine-grained PAT). | **M** | 006 | smoke gates traffic |
...
| TASK-032 | `tests/fixtures/seed.ts` deterministic seed + `asOwner` helper + injected clock. | S | 017 | `T-META-003` |
