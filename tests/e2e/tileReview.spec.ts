...
for (const width of [280, 390, 1440]) {
  describe(`Tile-first review at ${width}px`, () => {
    test('T-AI-067f: every original tile, known match and next step survives saved review', async ({
      page,
    }, testInfo) => {
      const svg =
      ...
      await expect(first.getByRole('button', { name: 'Confirm match', exact: true })).toHaveCount(
        0,
      );
      await expect(
