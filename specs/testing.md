...
| `T-BRAND-001` | U | **ADR-0014, issue #288.** `apps/web/test/brands.spec.tsx` cases `a`–`f` — the `components/brands/` register carries the icon register's contract, which is the entire price of excluding it from `T-A11Y-016d`. `a`: every mark renders `fill="currentColor"` on a `0 0 24 24` grid. `b`: the exported set is exactly the five vendored marks, so the register cannot grow a file at a time. `c`: no colour literal anywhere in the directory — ⚠ the scan strips comments first, because `#288` is three hex digits and matches a `#rgb` pattern. `d`: no icon/brand **package** is imported and no `fetch`/URL appears — product invariant 10 forbids requesting anything from a streaming service. `e`: `ATTRIBUTION.md` names the CC0 source and a **pinned commit**, so a mark can be re-vendored and audited. `f`: decorative by default, `role="img"` + `aria-label` when named. | ADR-0014 §3 |
| `T-BRAND-002` | U | **ADR-0014, issue #288.** `apps/web/test/brands.spec.tsx` cases `a`–`d` — `ServiceMark` on both surfaces (`specs/ui.md` §2.2a). `a`: all **eight** services expose `SERVICE_LABELS[service]` as their accessible name. `b`: the accessible name is identical whether the name is visible or visually hidden — `nameHidden` controls visibility, never presence, and a logo-only badge is invisible to a screen reader *and* to find-in-page, neither of which a screenshot shows. `c`: ⚠ **asserts Prime Video, Disney+ and Peacock still render a word mark with NO mark** — an "every service renders something" test would stay green while a contributor helpfully drew the three the CC0 source deliberately does not carry (ADR-0014 §5). `d`: a service with a mark renders exactly one `<svg>`, beside the hidden name. | ADR-0014 §5/§6 |
