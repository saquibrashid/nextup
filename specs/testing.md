...
### Owner-approved edition labels (TASK-243)

| Id | Level | What it asserts | Source |
|---|---|---|---|
| `T-EDITION-001` | U | Catalogue-typed edition recognition requires distinguishing source/search text; canonical film identity and original year remain unchanged. Validated labels merge without loss; malformed persisted data is not silently discarded. `packages/domain/test/editions.spec.ts`. | TASK-243 |
| `T-EDITION-002` | U | Live-shaped TMDB alternative-title lookup recognizes Vrach Frankenshteyn under movie 8836, carries full source evidence despite a shortened inference, caches bounded lookups and distinguishes absence from outages. `apps/api/test/unit/clients/tmdbEditions.spec.ts`. | TASK-243 |
| `T-EDITION-003` | I | Real SQL close preserves one film and unchanged listings/dates, stores only confirmed labels with provenance, rolls back on failure, and does not create undo-blocking modifications for new titles. `apps/api/test/integration/batchClose.spec.ts`. | TASK-243 |
| `T-EDITION-004` | U | Review, search labels and saved film show the edition and original-film year without duplicate rows/badges; known films expose explicit edition confirmation; older responses remain supported. `apps/web/test/editions.spec.tsx`. | TASK-243 |
| `T-EDITION-005` | U | Manual selections are checked against catalogue metadata; owner-scoped edition writes merge under the existing title lock and record same-transaction provenance. `apps/api/test/unit/titleEditions.spec.ts`. | TASK-243 |
