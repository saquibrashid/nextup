...
| **`T-WAIT-020`** (`a`–`g`) | U | `TASK-257` | #382, the waiting view in the Library look (`specs/ui.md` §7b). `a` one `h1` with the helper as its subtitle, no heading on the search, row names the only `h2`s; `b` a `year · type` meta line whose separators stay out of the text; `c` the forecast headline carries an *Estimate*/*Announced* tag, the service mark and the when, is `aria-hidden`, and the sentence under it still leads with "Estimate:"; `d` *Not interested* is a ghost control in the card footer whose accessible name includes the title; `e` the search is Library's field — visually hidden label, icon submit named *Search*, inside `.list-search__control`; `f` no raw ISO date anywhere on a row; `g` the stylesheet pins the catalogue surface, 2:3 posters at 4.5rem/6rem and two columns from 1024 px. `apps/web/test/waitingRestyle.spec.tsx` |
| **`T-WAIT-021`** (`a`–`c`) | E | `TASK-257` | In a real browser on both projects at 320, 640 and 1280 px: one page heading and a visible subtitle, poster width 72 px below 640 px and 96 px from it, the forecast headline visible with its tag, every *Not interested* at least 44 px tall and under 60% of its card's width, cards side by side only from 1024 px, and no horizontal overflow. `tests/e2e/waiting.spec.ts` |
