...
| AC-6 | I/C | **`T-LIST-026`/`T-UI-024`**, `T-UX-138` | Recently added reverses to Oldest additions in one press on the real list; ordering stays deterministic and the direction persists for the session. `T-UI-024o`–`q` retain URL reconciliation: a remembered non-default direction reaches the request, unrelated params survive, and the field default need not be written. The label must never describe a different order from the API request. |
...
| AC-7 | U | `T-UX-115`, `T-UI-024o`–`q`, **`T-UX-146i`** | The remembered direction reaches the **query string** and the mark agrees (`115a`), survives a field whose default differs (`115b`), and yields to an explicit `dir` in the URL (`115c`). ⚠ **Asserting either half alone passes on the broken version**, which renders the remembered direction without writing it and shows a newest-first list under an oldest-first mark. ⚠ **`T-UX-146i` pins the direction of the dependency: the URL leads and session storage follows.** Writing the remembered direction *before* navigating lets the reconcile effect see a dir-less URL with a remembered non-default direction and `replace` the entry the pending navigation was about to push — the chosen field is silently dropped and only the direction survives. |
...
| **`T-UX-120`** | U | **REQ-037 · runtime complete orders** use Longest runtime / Shortest runtime (`apps/web/test/sortControl.spec.tsx`). Inactive runtime selects `desc`; active reverses. Returning to dateAdded removes the default sort key. The key is URL-only; direction is remembered for the session. The approved 2026-09-16 labels replace the old direction-toggle wording. |
...
| `T-UX-115` | U | A `sessionStorage` direction with no URL `dir` is reconciled into the URL when it differs from the field default, and the selected label matches the request. Name defaults `asc`, others `desc`; explicit URL direction wins. Both URL and rendered state must be asserted together | §5 |
