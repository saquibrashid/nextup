...
| `T-PHONE-005` | U | The filters sheet's chips are native checkboxes and radios that write the canonical URL (`service`, `category`, `priority`); Reset clears every filter but keeps `q`; *Show N titles* counts and closes; genres past `PHONE_GENRE_LIMIT` sit behind a named More chip. `apps/web/test/phoneLibrary.spec.tsx`. | TASK-255 |
