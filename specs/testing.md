...
| AC-6 | I/C | **`T-LIST-026`/`T-UI-024`** | Reversing direction re-orders deterministically, tie-breaker unchanged (`T-LIST-026`); the sort control itself renders on the combined list, defaults to "Newest first", and toggling it drives the reversal (`T-UI-024` — the affordance-existence check `T-LIST-026` alone cannot make) | ⚠ **`T-UI-024o`–`q` are the cases with teeth.** `ListRoute` forwards `params.toString()` verbatim to `GET /api/titles`, so the URL — not the component's state — is what the server sorts by. A control that renders "Oldest first" from session storage without writing `dir=asc` into the query string shows a newest-first list under an oldest-first label, silently. `T-UI-024j` asserts only that `readSortDir` returns `asc`, which the broken version also satisfies; `o` proves the reconciliation reaches the URL, `p` that it preserves the other filters, `q` that the `desc` default is NOT written (an absent `dir` already means `desc` per `api.md` §6.2).
...
| **`T-UX-010`** | C | State 2.1 **Loading (initial)** on `/`: a **freshness-strip skeleton, a filter-bar skeleton and six row skeletons**, all `aria-hidden`, under one `role="status"` wrapper. ⚠ The list rendered **one sentence**. ⚠ **Six is load-bearing** (`T-UX-010a`): a skeleton's whole job is to claim the **shape** of what is coming, and one grey bar promises a list of one — the same damaging lie about a full library that the empty state would tell, which is why `T-UX-010c` also asserts the never-uploaded empty state is **absent** while loading. ⚠ **The skeletons are `aria-hidden`** (`T-UX-010b`): announcing six empty rows describes a list that does not exist, so the single wrapper carries the whole message. That is why **`T-DATA-002c` moved from `getByText` to `getByLabelText` in the same commit** — `LIST_LOADING_BODY` became the wrapper's accessible **name** rather than visible text, and re-greening it by restoring a paragraph the spec does not ask for would have been the wrong repair. ⚠ **`T-UX-010d`/`e` are the wiring cases and are not redundant with `T-UX-001a`-`h`**: those drive a probe component, so every one of them still passes with `ListPage` never calling `useSlowRequest` at all — precisely the state this screen was in. Stubbing the phase to `'normal'` in the page kills `T-UX-010d`+`e` and nothing else. Retired from `T-META-007`'s `KNOWN_UNCOVERED` with `apps/web/test/slowResponse.spec.tsx`. | `T-META-007`, `specs/ux-states.md` §2.1, `specs/ui.md` §2.1 |
