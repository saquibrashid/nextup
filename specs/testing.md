...
| `T-UX-166a`–`g` | Web | #328: cross-restart destination restoration before list fetch, all return routes and receipt state, explicit URL authority, Back/Forward/clear, absent genres and cursor exclusion, obsolete values, blocked-storage disclosure and in-app fallback. `apps/web/test/libraryNavigation.spec.tsx`. |
| `T-UX-166h` | E2E | #328: real shell desktop/phone navigation, first restored request, chips/sort/rows, browser-context restart, explicit link and Clear persistence in Chromium/WebKit. `tests/e2e/refinedLibrary.spec.ts`. |
...
| AC-6 | I/C | **`T-LIST-026`/`T-UI-024`**, `T-UX-138`, `T-UX-166` | Recently added reverses to Oldest additions in one press; ordering stays deterministic. #328 extends persistence across browser restarts for the complete destination. `T-UI-024o`–`q` assert remembered return URLs, explicit-filter link authority and field defaults. The label must never describe a different order from the API request. |
...
| AC-7 | U | `T-UX-115`, `T-UI-024o`–`q`, **`T-UX-146i`**, `T-UX-166` | Remembered complete order reaches the query string before list reads and the mark agrees (`115a`, `166a`). Explicit field links use their own default (`115b`) and explicit direction wins (`115c`). URL leads and persistence follows; choosing a field must not lose it during a competing preference reconciliation (`146i`). #328 intentionally retires the direction-only session fallback that rewrote older history entries. |
