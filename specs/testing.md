...
| `T-MOCK-005` | C+E2E | Wide library heading, persistent search and controls share a compact header; service/filter choices remain canonical URL state. Real Add title and Service updates clicks must work through the shared grid, not merely have non-overlapping boxes. Phone disclosure, desktop Escape focus, explicit submit/clear, sorting, layout memory and no-overflow behavior remain. `apps/web/test/listSearch.spec.tsx`, `tests/e2e/refinedLibrary.spec.ts`. |
