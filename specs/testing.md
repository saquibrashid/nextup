...
| --- | --- | --- |
| `T-AI-062` | U+G | `packages/domain/test/tileEvidence.spec.ts` (`a`–`g`) and `tests/extraction/tileCrop.spec.ts` (`h`). Legacy OCR-backed association refuses guessed model locations. The corpus case replays historical **whole-image** recordings through cross-check and measured evidence, pairs crops to ground truth **by identity**, requires every paired crop to cover at least half its tile, at least 15 crops overall and two measured crops. It does not measure the new crop-read pipeline's recognition quality. |
| `T-AI-063` | U | `apps/api/test/unit/extraction/tileEvidence.spec.ts` (`a`–`d`), `runExtraction.spec.ts` (`e`), `startExtraction.spec.ts` (`f`), `batchReviewRoutes.spec.ts` (`g`). Real pixels traverse detection, actual per-tile lossless reader input, persisted region parsing and review projection. Assert both readers receive each exact original-resolution crop, never the whole image; distinct source regions produce distinct inputs. Refused layouts do not invent counts; opaque-alpha PNGs retain one-channel raster semantics. Counts and geometry are bounded and batch-image scoped. Runner and writer persist measurements and source image IDs; missing tile coverage withholds removals. |
...
| `T-AI-065` | U | `packages/domain/test/reviewEvidence.spec.ts` (`a`–`f`). Same-input-tile readings and possible catalogue alternatives remain evidence, never transferred geometry or invented identity. Ignore collapsed/discarded/chrome peers. Shared bulk policy excludes ambiguous, weak, unverified, low-confidence and conflicting readings; individual decisions remain possible. |
| `T-AI-066` | U | `apps/api/test/unit/extraction/tileEvidence.spec.ts` (`a`–`d`): crop-before-reader, serial crops/two concurrent legs, empty/all-chrome placeholders, worst-case degradation and refused-layout removal withholding. `packages/domain/test/reviewEvidence.spec.ts` (`e`–`f`): five source tiles survive empty outputs and work collapse; controlled input provenance permits crops without laundering model geometry. `packages/domain/test/overlap.spec.ts` (`g`): adjacent same-height tiles cannot fragment-collapse. `apps/api/test/unit/batchReviewRoutes.spec.ts` (`h`): real review/bulk routes expose input regions and owner-scoped existing-service/library states, omit foreign images, and exclude weak bulk decisions without forbidding explicit ones. |
| `T-AI-067` | C+E | `apps/web/test/tileReview.spec.tsx` (`a`–`e`, `g`–`h`): every original beside its match, optional known-match acknowledgement without additions, verified saved state, discovery wording, other-service and uncertain next steps, same-tile fragments, alternatives correction, unique repeated-work DOM/search IDs and saved-decision counts. `tests/e2e/tileReview.spec.ts` (`f`): Chromium/WebKit at 280/390/1440px, source aspect ratio, no overflow, accessibility, persisted known/new decisions after reload and final summary excluding known titles even when acknowledged. |
...
| `T-UX-155c` | E2E | *(Phone branch redefined in place by TASK-255: uniform priority-control width is a Compact ≥640 px property, since the phone draws content-sized pills; phone posters crop to 10:11 and the heading replaces the identity box in the overlap sweep, which now includes the Added date.)* `tests/e2e/refinedLibrary.spec.ts`: both layouts at 280/390/900/1280/1600px retain uniform 44px priority controls, all eight service marks, 2:3 posters at least 72px wide, bounded metadata and no page overflow. Varied priority, Watching and unidentified/unknown-artwork states cannot overlap row actions. |
...
| `T-UX-161j` | E2E | *(TASK-255: the library it returns to is headed* My Library *below 640 px.)* Real-router cancellation/leave, local previews, partial saved recovery, explicit retry, and responsive accessible geometry. `tests/e2e/captureContinuity.spec.ts`. |
...
| `T-UX-162a`–`l`, `T-UX-162n`–`o` | Web | Reversible individual/bulk decisions; explicit offline persistence, conflict handling and no replay; truthful empty states, known mismatch, secondary rescue, legal two-step re-extraction, expiry, uncertain derived creation and removal intent. `apps/web/test/reviewContinuity.spec.tsx`. |
| `T-UX-162m` | Unit | Corrected identity/display survives confirm, discard and reopen. `packages/domain/test/chosenReviewMatch.spec.ts`. |
| `T-UX-162p` | E2E | Edit saved decisions, return from summary, explicitly save offline intent, and accessible responsive layout at 280/390/1440px in Chromium/WebKit. `tests/e2e/reviewContinuity.spec.ts`. |
| `T-UX-162q` | Web | An old correction cannot overwrite another tab's changed identity, even through explicit conflict override. `apps/web/test/reviewContinuity.spec.tsx`. |
| `T-UX-162r` | Web | Discarded matched and unidentified cards do not contradict their saved decision with an addition consequence. `apps/web/test/reviewContinuity.spec.tsx`. |
| `T-UX-163a`–`g`, `T-UX-163j` | Web | Lost success, verified retry, unreadable outcome, terminal deep links, no-change/empty-group undo, leaving during Apply, another-tab discard and truthful cold-read failure. `apps/web/test/applyOutcome.spec.tsx`. |
...
| `T-LIB-002d` | E2E | *(Phone branch redefined in place by TASK-255: below 640 px the trigger is the tab bar's Search tab, with no toolbar trigger and no expanded state of its own.)* #326 / TASK-241: phone search disclosure reduces toolbar height; wide desktop search stays visible with no hidden-trigger focus loss. Both retain native submit without keystroke requests, clearable active chips, keyboard focus, overflow and axe checks in Chromium/WebKit. `tests/e2e/refinedLibrary.spec.ts`. |
| `T-LIB-003` | Web + E2E | Grid/Compact persists through navigation, reload and browser restart without changing URL, requests, loaded rows or filter/sort/search choices. Missing/invalid/blocked storage retains supported behavior and explains limitations. `apps/web/test/libraryNavigation.spec.tsx`, `tests/e2e/refinedLibrary.spec.ts`. |
| `T-LIB-004` | E2E | An unfinished Netflix review banner stays at least 16px above library heading/actions; Service updates/Add title remain usable without overlap or overflow at 320/640/1280px in Chromium/WebKit. `tests/e2e/refinedLibrary.spec.ts`. |
| `T-POL-001` | Component | Capture stage indicator has exactly one current step, ordered labels, no navigation or fabricated percentage; page integration preserves unresolved/terminal states. `apps/web/test/premiumCatalog.spec.tsx`, `apps/web/test/batchStatusPage.spec.tsx`, `apps/web/test/uploadCheckpoint.spec.tsx`, `apps/web/test/captureContinuity.spec.tsx`. |
| `T-POL-002` | Component | Review metadata is year then type, actual warning signals precede consequences, and labelled raw evidence is preserved without manufacturing confidence. `apps/web/test/reviewPage.spec.tsx`. |
| `T-POL-003` | E2E | Responsive capture stage framing, review evidence/identity/action alignment, full-width 2:3 desktop grid vs 72x108 compact artwork, reduced motion and screenshots. `tests/e2e/guidedCapture.spec.ts`, `tests/e2e/groupedReview.spec.ts`, `tests/e2e/refinedLibrary.spec.ts`; existing overflow, target-size and axe assertions remain. |
| `T-MOCK-001` | U | Service chips share existing multi-service URL state; All clears only services, other query parameters survive and Back restores selection. Pagination remains owned by the existing list route. Pointer focus must not collapse an inline group before the drawer's Done click; keyboard focus-leave behavior remains intact. `apps/web/test/filterBar.spec.tsx`. |
| `T-MOCK-002` | E2E | *(Phone branch redefined in place by TASK-255: the phone navigation is the bottom tab bar, whose one link is Library, and the service chips sit in the filters sheet.)* Desktop sidebar and phone navigation geometry, local serif heading, keyboard-reachable 44px service chips, shared filter-panel state, full catalog reflow and axe. `tests/e2e/refinedLibrary.spec.ts`. |
| `T-MOCK-003` | E2E | Mockup upload framing retains unset service/mode, readable numbered steps, consistent logo tiles, enabled file input and disabled submit before setup. `tests/e2e/guidedCapture.spec.ts`; responsive upload/review serif hierarchy and safety controls remain covered by `T-POL-003`. |
...
| `T-DETAIL-001` | U | Strict display-only TMDB schema and movie/series projection — since #391 one request appends `credits,videos,release_dates` (movie) or `credits,videos,content_ratings` (series) and projects tagline, writers, release/first-air date, status, US certification, seasons, episodes and the trailer; missing versus malformed data and provider failures. Original matching allow-list remains closed. `apps/api/test/unit/clients/titlePresentation.spec.ts`. |
...
| `T-DETAIL-004` | C | Synopsis/credits hierarchy, missing/stale/unidentified states, existing consent dialogs and watch preferences, historical titles, offline controls and failed artwork. `apps/web/test/titleDetails.spec.tsx`. |
| `T-DETAIL-005` | C | Library details links carry query context; direct entry, loading, failed-read retry, missing title and access refusal. `apps/web/test/titleDetails.spec.tsx`. |
| `T-DETAIL-006` | E2E | Responsive detail layout at 320/768/1440px in Chromium/WebKit; complete synopsis/cast, accessible structure, keyboard navigation, modal focus return and remembered filters/sort/Compact across reload and Back. `tests/e2e/titleDetails.spec.ts`. |
| `T-DETAIL-007` (`a`–`g`) | U | #391, the trailer and extra details (`TASK-259`). `a`–`e` `pickTrailer` takes YouTube only, a Trailer before a Teaser, official before unofficial, then the newest; a key that is not a YouTube video id is skipped, never linked; the movie and series projections read the appended `videos`, `release_dates` (US, theatrical first) and `content_ratings`; `f`–`g` a copy cached before #391 (no `trailer` key) is refetched once on access, while `trailer: null` (TMDB has none) is fresh and is not. `apps/api/test/unit/clients/titlePresentation.spec.ts`, `apps/api/test/unit/services/titlePresentation.spec.ts` |
| `T-DETAIL-008` (`a`–`d`) | C | #391 on the Library details page. `a` the trailer is a link out to `https://www.youtube.com/watch?v=KEY` with `target=_blank`, `rel=noopener noreferrer` and a name that says it opens YouTube, and nothing is embedded (no `iframe`/`video`/`object`/`embed`); `b` no trailer means no trailer control, and a teaser says *Watch teaser*; `c` a movie shows its tagline and a *Details* list of Released, Status, Rated and Writers in that order; `d` a series says First aired, Status, Rated, Seasons, Episodes, and a title with none of them has no *Details* section, tagline or trailer. `apps/web/test/waitingDetails.spec.tsx` |
...
> `docs/PRD.md` at every pass rather than incremented by hand, and the
...
`T-META-001` parses `docs/PRD.md`, extracts every `US-nnn AC-n`, and
...
merge. It lives in `tests/meta/acCoverage.spec.ts` and runs in the `meta`
project. ⚠ **This paragraph described the gate for a long time before the gate
existed** — see its row in §12 for what it found on the first run it was
...
| Gate | Threshold | Test |
|---|---|---|
| Title recall (aggregate, **including** the artwork-only fixture) | ≥ **0.95** *(raised from 0.90 — see note)* — **ENFORCED as a gate since TASK-206, not pinned.** Measured **0.9552** (64 of 67); was 0.9403, and `T-AI-030f` fired to force the promotion. ⚠ **THIS WAS THE LAST ENTRY IN THE KNOWN-SHORTFALLS LEDGER, SO THE LEDGER IS GONE** and `T-AI-030f` now fails if one is reintroduced — a future regression must be fixed or its floor renegotiated here, not parked. The cause was §3.1a R2's `ocrSupport === 'exact'` gate treating OCR *silence* as evidence for the model, so a tile captioned `WICKED FOR GOOD` was matched as `Wicked: Part One`. Fixed by §3.1a R3. ⚠ **The 3 residual misses are NOT all reader failures:** two are `louis c k ridiculous`, occluded by the phone nav bar on one screenshot but read correctly on the next — a **per-image measurement artefact**, since `resolveCandidates` collapses across every image in the batch. Only `in the hand of dante` (truncated on screen as `In the sh...of Dante`) is genuinely the reader's. | `T-AI-030` |
...
| Chrome rejection | ≥ 0.80 — **ENFORCED as a gate since TASK-195, not pinned.** Measured **0.8481** (67 of 79); was 0.2361, and `T-AI-030f` fired to force the promotion. The two causes were the §3.2 vocabulary genuinely not covering this corpus **and** step 1's reading-order grouping fusing nav labels before step 3 could match them. ⚠ **IT FELL FROM 0.8750 AT TASK-199 AND THAT IS CORRECT — DO NOT RECOVER IT BY TRIMMING THE ANSWER KEYS.** The owner ruled `HBO` / `HBO ORIGINAL` / `New` chrome, adding seven key entries across the three Max fixtures; the pipeline isolates only four of them, because on `max-saved-mobile-01` and `rotated-01` step 1 merges the badge into the adjacent caption before step 3 can test it — the *same* grouping-before-classification cause TASK-195 fixed for the nav bar, surviving in a second place. The old number was flattered by a key that was silent about badges that were always on the images. | `T-AI-030` |
...
| **…and OCR SILENCE does not excuse it** (§3.1a R3) — a tile captioned `WICKED FOR GOOD` with `ocrSupport: 'none'` must not become `Wicked: Part One`, while an ellipsised or prefix caption still de-truncates and an inference *contained in* a merged caption still wins | must | **`T-AI-043`** (`g`–`j`) |
| **Tile-painted badges are chrome** (§3.2 step 3 R4) — `HBO`, `HBO ORIGINAL`, `New`, **and the vocabulary is never applied to the primary reader**, so a work the model names `New` survives | must | **`T-AI-004`** (`am`–`ap`) |
...
| # | Assertion | Gate |
|---|---|---|
| L1 | Per-image title recall ≥ that image's `minRecall` | in **3 of 3** runs; every **missed expected title printed by name** with the number of runs that missed it |
...
| L5 | False-title rate per run | ≤ **0.10**; every **false title printed by name** with the number of runs it appeared in |
...
**How it is wired (TASK-079b).** `tests/extraction/goldenLive.spec.ts` is
collected by a dedicated Vitest project named **`live`**, selected only by
`npm run golden:live`. ⚠ **It is a project so that it is not INVISIBLE, not
so that it runs:** `T-CI-008` fails any spec file no runner collects, and a
silently dead `goldenLive.spec.ts` is this product's model-drift alarm
disconnected while every gate stays green. The `golden` project still
**excludes** it, the `live` project omits `setupFiles` (every other project
installs the `T-CI-007` egress guard, and this is the one suite whose whole
purpose is to leave the machine), and **`T-CI-007l` asserts the property that
matters**: `golden:live` is the only script selecting `live`, nothing reaches
it transitively through `npm run`, and no script runs a bare `vitest run`
without `--project` — which would collect every project, including this one.
Its bands are `T-AI-051` (`a`–`k`); the report is written **before** any band
is judged, so a failing run still produces the artefact.
...
| AC | Test | L | Assertion |
|---|---|---|---|
| **AC-12** | **`T-PASTE-001`** | C | **The desktop `paste` listener.** A synthetic `paste` event on `document` carrying a PNG in `clipboardData.files` attaches it to the open batch and calls `preventDefault()`. **Three negative cases in the same test, all required:** an event whose `target` is an `<input>` returns **without** `preventDefault()` (text pasting into TMDB search still works); a **text-only** paste returns without `preventDefault()`; and the listener is **removed on unmount** (a paste after navigating away attaches nothing). **Plus the both-sides duplication case (`T-PASTE-001m`), mandatory:** Chromium populates **both** `clipboardData.files` and `clipboardData.items` for one screenshot paste, and `DataTransferItem.getAsFile()` mints a **new `File` on every call**, so an object-identity merge of the two sides delivers every pasted image **twice**. A paste carrying the same single image on both sides must attach **exactly one** file. The test helper must return a **fresh `File`** per `getAsFile()` call — a helper that returns the same instance makes the broken code pass — and the assertion must compare **file names**, because two distinct jsdom `File`s have no enumerable own properties and `toEqual` reports them as equal |
| **AC-13** | **`T-PASTE-002`** | C | **The iOS button path.** Clicking **"Paste screenshot"** calls `navigator.clipboard.read()` **synchronously inside the click handler** (asserted: no `await` or timer precedes the call, so transient activation is intact); the resolved `ClipboardItem`'s `image/png` blob is posted with `ingestSource: 'paste'`. Also §4.0a: a paste before service/mode is chosen **holds** the image client-side and attaches it once the batch exists — it is never silently dropped. **Also the iOS hint (`T-PASTE-002i`, `k`–`n`): it renders when `touch` is passed, and OTHERWISE IS INFERRED from `matchMedia('(pointer: coarse)')` — asserted with no `touch` prop and through `ImageDropzone`, which passes none.** ⚠ Every earlier hint case passed `touch` explicitly, so the inference was never written and the hint was dead in production while both prop comments claimed it existed (product invariant 16). An explicit `touch={false}` must still win over a coarse pointer, and a **width** breakpoint is not a substitute — a resized desktop window is not a phone |
...
| **AC-14** | **`T-PASTE-011`** | C | **Every early arrival survives setup.** TASK-222 holds chosen and dropped files in the local queue, including when service and mode are unset. The three cases prove each file remains present after setup becomes ready, with arrival order and its original ingest source intact. No automatic upload occurs on setup changes. `T-UX-156a/e` additionally prove the retained queue reaches the server on explicit Extract, including clipboard provenance. This replaces the former hold-then-auto-upload callback; never accept a hold-only assertion that could pass while discarding the file. |
...
| **AC-17** | **`T-PASTE-007`** | I | **Every ceiling and guard applies identically to pasted images.** A pasted 48 MP PNG is refused **413 `IMAGE_TOO_LARGE_TO_DECODE`** by the pre-decode guard with the decoder never constructed (the `T-IMG-017` assertion, re-run on the paste path); an 11 MB paste → **413 `IMAGE_TOO_LARGE`**; the 41st image in a batch → **400 `TOO_MANY_IMAGES`** **whether it arrives by paste or upload**; a pasted `MAX_FILES_PER_REQUEST + 1` multipart request is refused **400 `TOO_MANY_FILES_IN_REQUEST`** exactly like upload and lands **zero** rows; an empty paste request is refused **400 `VALIDATION_FAILED`**; `retainUntil` on a pasted row is `uploadedAt + 30 days` exactly like an uploaded one (NFR-019); `image.decode.begin` carries `ingestSource` |
...
| AC-5 | U | `T-AI-049` (new, TASK-204) | **The off-list region (§3.2 step 3b).** `ocr-only` orphans past a `Recommended For You` header are the SERVICE's promotions, not the OWNER's saved list, and are reclassified `chrome-suspected` - **never dropped**. ⚠ **MOSTLY REFUSALS, DELIBERATELY.** Every other cleanup rule reclassifies ONE line on its own evidence; this one reclassifies by POSITION, so a wrong anchor or a wrong axis costs every title on the wrong side of the cut. Asserted: the derived axis is `x` on a rotated page, a header INSIDE the hull yields no cut, outside on both axes yields no cut, no primary-reader hull yields no cut, a primary-reader tile past the header cancels the cut for everything, the header match is whole-line (a title CONTAINING the phrase opens nothing), nothing is dropped, and `unreadable-tile` / `low-confidence` are never overwritten. Seven mutations applied, each caught. |
| AC-5 | U | `T-AI-038` (new) | **The prompt/recording synchronisation gate — the id `specs/ai.md` §2.1a and `prompts.ts` have both promised for months while it existed nowhere.** ⚠ **`check:test-ids` could not catch its absence**: that gate validates ids cited in `docs/backlog.md`, and this one is cited only in a spec and a source comment. ⚠ **WHAT THE ABSENCE COSTS IS INVISIBLE BY CONSTRUCTION.** Every offline gate in `tests/extraction/` replays COMMITTED RECORDINGS of the model's output, produced by one specific prompt. Edit the prompt and not one of them fails — they cannot, because no live call is made — so the whole quality suite goes on measuring a prompt THAT IS NO LONGER THE ONE THAT SHIPS while reporting green. Worse than no measurement, because the numbers still look authoritative. `a` pins a SHA-256 over the system prompt, the user prompt **and `TILE_SCHEMA`**, and fails on any edit to any of them; the only correct response is to re-record (`npm run golden:record` — LIVE, REAL MONEY, MANUAL) and move the pin **in the same commit**. ⚠ **`b` IS THE TWIN, AND WITHOUT IT `a` APPLAUDS AN EMPTY PROMPT** — a hash matches whatever it was computed from, so deleting every rule and re-pinning reaches green; `b` asserts the four load-bearing instructions (`Do NOT guess`, `NEVER omit a tile`, the no-brand rule, `copied EXACTLY`) are still present, because their removal would not announce itself anywhere else — `T-AI-030`'s fabrication rate is scored against recordings and would report the old prompt's number indefinitely. `c` proves the schema is inside the fingerprint and not just the prose, since it ships as a `strict: true` Structured Output and constrains the answer as hard as the instructions do. Both mutations proven: adding one prompt rule fails `a`; deleting `Do NOT guess` fails `a` and `b`. |
| AC-5 | U | `T-AI-056` (new) | **The §4A L7 cost line must name whose price card it used — the defect had already misled a model decision.** `tests/extraction/liveCost.spec.ts`. The live report rendered *"Estimated cost: **$0.3783** at $2.00/1M prompt and $8.00/1M completion"* for the `gpt-6-astra` probe. Those are **`gpt-4.1`'s** rates, and the only record of that fact was a source comment nobody reading the committed report could see. Set beside the incumbent's $0.2024 it reads as *"the frontier model costs 1.9×"* — false: it is the same prices applied to more tokens, and the real multiple is unmeasured and estimated far higher elsewhere in this project. A model choice was being weighed against that number. ⚠ **THE FIX IS NOT TO PRICE EACH ARM, AND THIS ROW MUST NOT BE READ AS ASKING FOR THAT.** L7's ceiling is a regression guard on prompt and token **growth**; holding one price card fixed across arms is exactly what makes two reports comparable on token volume, which is the only thing the guard can see. Re-pricing per arm would silently convert a token-growth alarm into a billing estimate and destroy the comparison — so `c` pins the fixed card as a **property**, asserting the incumbent and a challenger render an identical rate line, and fails if a future change prices them apart. The price card is bound to a named constant `PRICED_MODEL_ID` so the report can state it (`a`) and warn when the measured arm is a different model (`b`). ⚠ **`b` names the specific misreading rather than hedging** — *"may be inaccurate"* would not have stopped the 1.9× comparison that actually happened — and asserts both model names appear, so the reader sees which mismatch it is. `a` asserts the warning is **absent** on the incumbent, because a warning that fires always is a warning nobody reads. `d` pins the arithmetic against hand-computed values rather than a re-expression of the formula, so a swapped prompt/completion rate cannot pass by symmetry; `e` keeps both raw token counts in the report, since the tokens are the real signal and the dollars only scale them. ⚠ **The rendering was EXTRACTED to `tests/extraction/liveCost.ts` to make this testable at all**: `goldenLive.spec.ts` is excluded from every CI project because it calls a paid API, so nothing inside it is ever executed by a gate (`T-CI-008` — an uncollected spec passes by never executing). That is right for the measurement and wrong for the **reporting**, which is the artifact a decision is made from months later. ⚠ **LIMIT, recorded in the file:** these cases exercise the rendering, not the live run — nothing asserts that `reportMarkdown` calls the helper with the *measured* arm's deployment; that wiring is held by the type checker alone. Mutation-proven case by case: removing the warning branch fails `b`; dropping the price-card name fails `a`; pricing the challenger separately fails `c`; swapping the prompt rate for the completion rate fails `d`. |
| AC-5 | U | `T-AI-058` (new) | **Every §4A live band must be ACHIEVABLE by the model in production — checked offline, for free, in CI.** `tests/extraction/bandAchievability.spec.ts` parses the latest committed incumbent baseline (`docs/evaluation/golden-<date>.md`) and fails if any band exceeds what `gpt-4.1` actually measured. ⚠ **THE RULE ALREADY EXISTED AND WAS ENFORCED NOWHERE.** `T-AI-051k` states it for L2 — *"the floor must not exceed the incumbent's measured worst pair"* — inside `goldenLive.spec.ts`, which **no Vitest project collects** because it spends real money, so the guard against an unreachable band only ever ran during the billed run whose bands it protects (`T-CI-008`: an uncollected spec passes by never running). ⚠ **It was also never generalised, and that is why two bands are still red.** L1 has `T-AI-051j`, which pins each floor at or below what the **offline** recordings achieve — a weaker claim, and the gap is the whole defect: replay is deterministic, live resamples, so a floor calibrated on replay can be unreachable live while `j` calls it honest. L2 reached **0.95** exactly that way — a value no arm has ever cleared, production included — and sat permanently red while blocking every model change through §9.7 Stage 3 by a threshold the incumbent itself fails. Measured today, the same derivation is still standing in two places: **L1 `netflix-mylist-desktop-01`** (floor 0.900 from an offline 10/10; live 0.800 in *all three* runs, missing `in the hand of dante` and `wicked for good` every time — ⚠ **since RESOLVED: the floor was lowered to 0.75, just below the measured 0.800, because a floor the shipping model can never reach reports nothing; the two live misses are not forgiven, merely no longer gated by an impossible band**) and **L5 false-title** (ceiling 0.10; live 0.1786 / 0.1321 / 0.0926 — red in 2 of 3). ⚠ **Neither is fixed here and `a` pins them as a CHARACTERISATION, bounded from BOTH sides** — the same device as `T-AI-054e`: the gate fails if a NEW band becomes unreachable **and** fails just as loudly if one of these is silently fixed without the list and its reasoning moving together, because a known-issue list that only shrinks by accident is how the L2 defect survived three revisions of the spec. ⚠ **L5 MUST NOT BE "FIXED" BY RAISING THE CEILING — NOR BY BACKING OUT THE TRUNCATED READS.** `truncated-titles-01` presents deliberately truncated captions, and the reader emits `dr strangelove or how i lear` and `hitchhiker s guide to the` in all three runs, so it is tempting to call each one defect charged to two gates and excuse it the way `goldenScorer.ts` excuses chrome. ⚠ **The report refutes that: `truncated-titles-01` scored recall 1.000 on all three runs** — every full title *was* found, so the stumps appear ALONGSIDE the titles they truncate rather than instead of them, nothing is charged twice, and each stump is a genuine extra junk row (the "one title split across several rows" recognition defect). Backing them out would delete two real defects from the metric; `T-AI-059` therefore builds the exclusion and pins it **shut** for exactly this case, so the obvious-but-wrong fix now fails a test. The L5 breach is real and unexplained by arithmetic — it needs extraction work or an owner-accepted ceiling. ~~Superseded: "part of the numerator is a DOUBLE COUNT the scorer already refuses elsewhere … backing those two strings out moves the measured rates 10/56 → 8/54 = 0.148, 7/53 → 5/51 = 0.098, 5/54 → 3/52 = 0.058 — clearing runs 2 and 3 and still failing run 1." That arithmetic also wrongly shrank the DENOMINATOR; the chrome convention is numerator-only. Both errors are moot: recall 1.000 means there was never a double count to back out.~~ ⚠ **And the baseline is STALE, which outranks both**: `golden-2026-09-18.md` predates `recently added` entering `chromeTerms` — an entry whose own source comment cites *this report* as its evidence, and a false title in that table which today's code would classify `chrome-suspected` and exclude from the numerator. Moving a band against these numbers would calibrate it to a pipeline that no longer exists, which is exactly how L2 got to 0.95; a fresh `npm run golden:live` (manual, ~$0.20) comes first. The band constants were **extracted to `tests/extraction/liveBands.ts`** so the gate and the live suite read one definition rather than two that drift. `c` pins that a floor is judged on the **worst** run, never the best — §9.7 Stage 2 runs three times because one run is not a measurement, and a band cleared by one run of three is cleared by luck. `d` guards the direction of comparison: ⚠ **an inverted floor/ceiling test is the silent failure here** — it reports every healthy band as a breach and every real breach as healthy, and the list it produces still looks entirely plausible. `e` refuses a challenger **probe** as evidence, since `golden-<date>-<slug>.md` measures a reader nobody ships and the repo holds two such probes that sort *after* the baseline and would win a naive `.sort().at(-1)`. `b` is the non-vacuity guard — ⚠ **with a LIMIT found while mutation-proving it and recorded in the file**: the total-emptiness mutation never reaches `b`, because `parseLiveReport` throws on a zero-row table and the suite then dies at **collection**, reporting *"no tests"* and zero failures while the run is red only by exit code — the `T-CI-008` shape in miniature. The throw is the better failure (it names the file and the missing section) so it stays, and `b` covers the subtler half: a parse that matches but drops data. Mutation-proven case by case: judging a floor on the best run fails `c` alone; inverting the floor comparison fails `a`, `c` and `d`; admitting slugged probe reports fails `a` and `e`; capturing only the first of three run columns fails `b`. |
| AC-5 | U | `T-AI-059` (new) | **Declared truncated reads, and the condition that governs them — `tests/extraction/truncatedReads.spec.ts`.** The 2026-09-18 live report shows `truncated-titles-01` emitting `dr strangelove or how i lear` and `hitchhiker s guide to the` in all three runs, and the obvious reading is one defect charged to TWO gates — a missed title (L1) plus a false title (L5) — the same double count `goldenScorer.ts` already refuses for chrome. ⚠ **The report refutes it, and pinning that refutation is why this test exists.** The same image scored recall **1.000 on all three runs**: every full title WAS found, so each stump appears ALONGSIDE the title it truncates, nothing is charged twice, and each is a genuine extra junk row (the "one title split across several rows" recognition defect). Excusing them would have deleted two real defects from the metric and moved L5 for the worst possible reason. So the exclusion is built and then pinned **shut**: `a` asserts that a declared truncation whose full title was found is **still** counted, which is the assertion that stops a future reader of the same report reaching the obvious-but-wrong conclusion. ⚠ **The truncations are DECLARED in the answer key, never DETECTED, and no heuristic could replace them** — each guard one would reach for fails on this very corpus: a word-boundary test rejects `dr strangelove or how i lear`, which cuts MID-WORD, so it rejects the exact case it was written for; a length-ratio test rejects it too (28 of 66 characters); and dropping both guards excuses `true detective`, a prefix of `true detective night country` and a **different real work** the corpus expects on two images. Every heuristic either misses the defect or silently deletes a real false title. `b` holds every declared form to being a strict prefix of the title it names, so a typo cannot quietly excuse an unrelated string. `c` exercises the rule as a pure function in both directions — excused when the full title is missing, **not** excused when it was found, never excused when undeclared. ⚠ **Numerator only, matching the chrome convention**: `aggregate` counts every `title-candidate` in `titleCandidates`, so an excused read stays in the denominator and the rate can never improve by shrinking what it is measured against. `d` pins that the aggregate **names** every excused read rather than dropping it silently, since a rate that improved with no report of why is unauditable. `e` pins the normalisation — an answer key declaring nothing reads as `[]`, never `undefined`. |
| AC-5 | U | `T-AI-057` (new) | **The §9.7 Stage 3 metric set, computed in ONE place — `tests/extraction/readerMetrics.ts`, guarded by `tests/extraction/readerMetrics.spec.ts`.** ⚠ **The metrics §9.7 decides a model on were computed twice, privately, in two spec files that never compare their answers.** `bakeoffMeasured.spec.ts` and `goldenLive.spec.ts` each carried their own `jaccard` / `unstableTitles` / `omissionRecovery`, so the offline bake-off and the live probe could disagree on what a number MEANS while both reporting green — and the report a decision is read from months later carries no record of which definition produced it. Both now import the shared set, and `omissionRecovery` (the one §9.7 row with **no trade and no exception**, REQ-012) is emitted live for the first time as a `<stem>.metrics.json` companion. Every case here pins a **definitional choice where the wrong answer is silent**, never arithmetic. `a`: **L2 is the WORST pair, not the mean** — stability is a floor on how far two runs may diverge, and averaging one bad pair against two good ones dilutes it threefold and inverts the metric at exactly the reader that deserves to fail. `b`: **a single run THROWS rather than returning 1** — returning 1 reports the best attainable score from evidence that cannot produce it, and §9.7 Stage 2 mandates three runs per image, so one run is a broken harness, not a perfect reader. `c`: **two empty sets score Jaccard 1, not `NaN`** — ⚠ this is the zero-yield trap arriving through the stability gate, and its danger is the asymmetry: `NaN >= floor` is false and *looks like* a correct failure, but the equally natural `!(x < floor)` PASSES, so a reader that extracted nothing at all is certified perfectly stable. `d`/`e`: **the L3 denominator is EXPECTED titles, not accepted** — over accepted titles a reader improves its unstable-title rate by accepting more junk, turning a stability band into a reward for fabrication. `f`/`g`: omission recovery is scored over OCR text the reader missed. `h`: **a MISSING recording throws** — with no OCR texts there is nothing recoverable, the rate computes to 1, and the strictest floor in §9.7 is cleared by a measurement that never happened. `i`: artwork-only recall scores **only** the artwork images, and an empty corpus throws for the same reason. ⚠ **Point metrics are taken from run 1, never averaged across runs** (recorded in the file): an average describes a reader nobody ran and can sit above a floor no individual run cleared; run-to-run variation is not discarded, it is precisely what L2 and L3 are for. Mutation-proven case by case, each failing alone: mean-instead-of-worst fails `a`; a single run returning 1 fails `b`; skipping a missing recording instead of throwing fails `h`; dropping the empty-corpus guard fails `i`. ⚠ **§9.7's Outputs clause was AMENDED (owner-approved 2026-09-21) to name the `.metrics.json` companion beside the markdown**, so the artefact is now a required output rather than an undeclared side effect — and `j`/`k`/`l` guard the round trip that makes it worth requiring. ⚠ **The serialisation was EXTRACTED out of `goldenLive.spec.ts` for the same reason `T-AI-056` extracted the cost rendering**: that spec is excluded from every Vitest project because it spends real money, so nothing inside it is executed by any gate (`T-CI-008` — an uncollected spec passes by never running), which is right for the *measurement* and wrong for the *artefact a decision is computed from months later*. `j` pins **equality, not parseability** — the companion exists precisely because the alternative is a human retyping nine floats out of a markdown table, and §9.7 is blunt that *"supplying invented values would yield a fabricated decision carrying a function's authority"*; a round trip that parses but **rounds** reintroduces that quietly, which is why `j` asserts the numbers come back identical rather than merely well-formed. `k` and `l` are about **when** a bad artefact is discovered: `chooseReader` does fail closed on a missing metric (it returns `invalid-input`), but that verdict arrives at the *next* bake-off, against a file written months earlier by 66 billed vision calls, and the run cannot be repeated for free — strict parsing moves the identical failure to the moment of writing, while the model is still loaded. ⚠ **`l` is the case `typeof x === 'number'` would admit**: every comparison against `NaN` is false, so `x < floor` reports no breach and the arm clears a row it never measured — and since JSON cannot hold `NaN` at all (it serialises to `null`), the artefact otherwise loses the distinction between *not measured* and *measured as zero*. The field list is a **type-checked exhaustive record** (`Record<Exclude<keyof ReaderMetrics, 'modelId'>, true>`), so adding a row to `ReaderMetrics` without adding it to the artefact fails `tsc` — the precise defect of `T-AI-045aa`, where `unstableTitleRate` sat in the spec for two days, absent from `chooseReader`, with the suite green throughout. Mutation-proven: rounding the serialiser to 2 dp fails `j` alone; weakening the guard to a presence check fails `l` alone; removing validation entirely fails `k` and `l`. ~~Superseded: "DIVERGENCE, RECORDED NOT RESOLVED: §9.7's Outputs clause names only the markdown report; the `.metrics.json` companion is an addition awaiting the owner's decision."~~ |
| AC-5 | U | `T-AI-055` (new) | **The §5.3a tile crop, scored end-to-end against ground truth — and the case that would have caught the defect.** `tests/extraction/tileCrop.spec.ts` replays the committed recordings through the **real** `crossCheck()` and the **real** `tileCropFor()` and scores every crop against the annotated tiles of `T-AI-054`. ⚠ **`T-AI-041` — eleven unit cases over hand-written boxes — passed throughout the entire life of the defect**, because it proves the arithmetic and says nothing about whether the boxes the pipeline really produces frame anything. Measured here, the previous rule produced **10 crops of which 8 showed under half the tile they named**, median coverage **0.24**; only 2 of 48 thumbnails were useful, and the owner reported it three times before it was found. The rule was one inverted condition: `crossCheck` sets `boxSource: 'llm'` when **no** OCR line corroborated the tile, so `tileCropFor`'s `if (boxSource !== 'llm') return null` offered a crop exactly when the geometry was the reader's unverified guess and refused it exactly when it had been measured. `a` states the owner-facing property as a **guarantee, not an average** — NO crop may show under half its tile — because a thumbnail beside *"is this the right match?"* that frames the wrong artwork actively misinforms a decision the owner cannot check without the screenshot in front of them, and an aggregate would let a few badly-wrong crops hide behind many good ones, which is exactly how the old rule survived. ⚠ **`b` is `a`'s matched pair and neither is meaningful alone**: `a` is satisfied perfectly and vacuously by never cropping, and "never crop" is a live temptation because the uncropped path looks safe — it is also the ORIGINAL complaint, the owner back to finding one row among ~20 tiles. `b` pins the yield from below (≥15 crops, median coverage > 0.75; measured **22** and **0.89**). `c` is the inversion itself. ⚠ **It was strengthened by its own mutation test**: asserted only over the corpus it stayed green with the `boxSource` guard deleted, because `crossCheck` attaches a `tileBox` solely to corroborated items, so today's uncorroborated ones are refused by the *missing-tile* branch and never reach the guard — defence in depth, not a test, and it collapses the moment any other producer of `ExtractedTextItem` sets both fields. `c` therefore also states the rule directly, on an input the corpus cannot currently supply. `d` proves the two halves come from the two different sources, which is the whole design: position from the caption (exact, measured), **size from the reader's tile** (accurate to p25–p75 1.02–1.19) — a crop sized to the caption is a strip of text that shows no artwork, the one thing §5.3a exists to prevent, and it would pass `a` and `b` because a text strip does overlap its tile. ⚠ Crops are paired to ground truth **BY IDENTITY**; pairing geometrically would score each crop against whatever it landed on and always look correct. New rule: refuse unless OCR-corroborated **and** a reader tile is present **and** the caption centre lies inside that tile (the readers agree) — ⚠ **the agreement gate, not the position source, is what removes the misleading crops**; deleting it alone fails `a`. Measured before → after: 10 → 22 crops, 8 → **0** under half, median 0.24 → **0.89**, min 0.00 → **0.59**. Mutation-proven case by case: removing the gate fails `a`; removing the `boxSource` guard fails `c`; sizing the crop from the caption union fails `a`, `b` and `d`. <br> **`e` is the integration counterpart** (`apps/api/test/integration/batchReview.spec.ts`, against a real SQL Server): an uncorroborated reader box is served as **no crop** through the whole route. ⚠ It also proves the WIRING, which no domain test can see — `tileBox` rides inside the `NVarChar(Max)` `boundingBoxes` JSON and must survive `parseBoundingBoxes`, whose type signature did not mention it; dropping it there silently returns every thumbnail to the whole screenshot, and that mutation fails `T-AI-041v`, `T-AI-041w` and `T-UX-154a`. ⚠ A malformed `tileBox` **drops the tile, not the box** — rejecting the box would discard the candidate's only recorded evidence, whereas losing the tile just means "no crop". |
| AC-5 | U | `T-AI-054` (new) | **The tile ground-truth corpus (`tests/fixtures/golden/tiles/`), and the two facts it establishes about the reader's geometry.** ⚠ **Before this set, no measured statement existed anywhere of where the tiles on a golden image actually are**, so every claim about `boundingBox` quality was argued from the extractor's own output — the thing under question. That produced two confidently-reasoned, mutually exclusive diagnoses of ONE owner-reported defect, and both were wrong. 49 tiles across 8 images, hand-annotated and committed. `a`/`b` guard the corpus: a hand-edited fixture cannot place a tile off the image (the exact defect class the corpus detects in the model), and every annotated `title` must exist in the answer key — `title` is not decoration, it is what pairs tile to caption **BY IDENTITY**, because pairing them geometrically fails silently on precisely the layouts that matter, where the caption sits OUTSIDE the tile it names. `c` is the only case that validates the method rather than using it: `truncated-titles-01` is drawn by `images/generate.mjs`, so its tiles are known exactly, and the blind detector reproduces them to a worst-case coordinate error of **0.0007** — ⚠ asserted against the GENERATOR's parameters recomputed in the test, never against the fixture, which compared with itself proves nothing. `d` and `e` pin the measurement along the axis it revealed: box **SIZE** is accurate (p25–p75 1.02–1.19) and box **POSITION** is not (**22 of 46** centres land on their tile; worst drift **2.3 tiles**). That split is load-bearing — it is why a fix may take size from the model and must not take position. ⚠ **`e` asserts the defect is STILL PRESENT and is bounded from BOTH sides** — a characterisation pin, not an endorsement: it fails if the reader silently improves as well as if it degrades, forcing the pin and `docs/evaluation/tile-geometry-2026-09-20.md` to move together so the note cannot quote a figure no longer true of the recordings. `f` holds the two derived images to their documented transform (a resize preserves normalised coordinates; `sharp`'s `rotate(90)` is CLOCKWISE, `(x,y,w,h) → (1−y−h, x, h, w)`), because they are not independent observations — ⚠ deriving the rotated case rather than detecting it is deliberate, since a detector tuned until a rotated image "looks right" is the axis-specific reasoning the corpus exists to refute. Mutation-proven, each case failing only for its own mutation: a tile pushed off the image fails `a` alone; a 1.5% perturbation of the exactly-known image fails `c` alone; boxes inflated 1.6× fail `d`; recordings given perfect geometry fail `e`. `b` and `f` were proven by real failures on first run — they caught three genuine errors in the corpus (two tile titles that disagreed with the answer key, and a wrong provenance claim about which image `rotated-01` derives from). |
| AC-5 | U | `T-AI-060` (new) | **The tile-grid detector measures row geometry FROM THE IMAGE, and refusing is half of what it does.** `packages/domain/src/extraction/tileGrid.ts` + `packages/domain/test/tileGrid.spec.ts`. ⚠ **This exists because we only ever asked the model where things were.** `T-AI-054` measured the reader's own boxes and found sizes broadly right but positions unusable (**22 of 46** centres on their own tile); `T-AI-055` responded the only honest way available — refuse a crop unless OCR corroborated it. That removed the misleading crops but left the owner looking at *"Whole screenshot"* for every artwork-only read, which is exactly the read they most need to eyeball because it is also the fabrication-prone one (§9.2/§9.4, `RSK-021`). Reported live: a five-tile Netflix row where 3 of 5 titles were found and **1 of 3** got a crop. A streaming row is a strictly periodic strip of identical tiles separated by gutters of pure background, so the geometry is **measurable rather than inferable** — and a measurement can be checked, which a reader's guess cannot. ⚠ **Periodicity, not segmentation.** Thresholding column brightness and taking bright runs as tiles was built first and FAILS on real captures because artwork contains its own dark regions: on the owner's capture it split the dark `Nimesh Patel` and `Best of the Best` tiles in two and reported 7 tiles where there were 5. `b` pins that case. ⚠ **The gutters are the signal, not the tiles** — a gutter column is background for the WHOLE band height while a dark patch of artwork is not (measured: gutters peak at 20–22 of 255, darkest artwork column 40–70), hence a per-column **maximum**, never a mean. ⚠ **`a` is the case nature never supplies and is the reason this suite is synthetic.** On a *perfectly* uniform grid lag `2P` aligns every gutter with a gutter just as lag `P` does, the scores separated only by finite-window edge effects. Measured by disabling the subharmonic correction and sweeping raster width at a fixed 100px pitch: the true pitch wins at 400px but **loses at 500px (locks 200) and at 600px (locks 300, a third harmonic)**, yielding two tiles where there are five and six. `a` is built **600px wide for that reason** — an earlier version used 400px and passed with the correction disabled, i.e. guarded nothing, which mutation testing caught. Every real screenshot breaks that tie by accident because artwork varies, so the octave error is invisible to the corpus sweep and visible only to a constructed raster; the subharmonic correction is guarded by `a` alone. `a` also caught two real bugs on first run: the closing cut never lands on the raster edge, so a row whose last tile meets the edge **silently lost that tile** (4 tiles → 3), and the phase argmax returned the gutter's leading column rather than its centre, shifting every tile 4.5px in a 100px pitch. `c` pins band selection — a section header (*"My List"*) is itself a bright run and is the FIRST one, so taking the first analyses letter spacing and locks a period near 40px. `d`–`h` are the refusal guards, one per condition, and they matter more than `a`: `detectTileGrid` returning `null` is what stops this re-introducing the confidently-wrong crops `T-AI-055` removed, so each guard fails if deleted. `i` pins **containment, never nearest** — "nearest tile" always returns something, so an unreliable anchor landing in a gutter would be snapped to an arbitrary neighbour, which is the original defect wearing new clothes. |
| AC-5 | U | `T-AI-061` (new) | **The detector against the REAL corpus — the test that earns the threshold.** `tests/extraction/tileGrid.spec.ts`, driven through the production raster extractor (`apps/api/src/images/luminanceRaster.ts`), so the sweep runs the same code production does. `T-AI-060` proves the algorithm behaves as designed on constructed input; only real captures can show that the accept/refuse threshold sits **in a gap** rather than in the middle of the distribution. Measured: accepts at **1.000** (owner's live capture, 5 tiles — visually verified) and **0.962** (`netflix-artwork-only-01`, 6 tiles — visually verified); refusals at **0.859** and below (`rotated-01` 0.844, `netflix-continue-watching-01` 0.788, `max-saved-mobile-01` 0.625, `low-quality-jpeg-01` 0.569, `blank-no-content-01` 0.534, `netflix-mylist-mobile-01` 0.531). `MIN_GUTTER_SCORE = 0.95` sits in a 0.962/0.859 gap wider than the spacing between any two refusals. ⚠ **Refusals are the point and they are the MAJORITY** — seven of nine. Most of this corpus is not a uniform row, and a change that "improves" the detector by making those return a grid has re-introduced the defect, not fixed one. ⚠ **`b` is the calibration case and the only assertion here independent of anyone's judgement**: `truncated-titles-01` is machine-drawn by `images/generate.mjs`, so its pitch is known **exactly** (440 of 940 px) rather than annotated by eye, and `b` checks the detector against that arithmetic recomputed in the test — never against the fixture, which compared with itself proves nothing. ⚠ **MULTI-ROW WAS BUILT, MEASURED AND REVERTED — do not re-attempt it by relaxing anything here.** Analysing every content band and keeping rows that agree on a pitch is the obvious generalisation and it regressed the sweep: `truncated-titles-01` 2 tiles @ 0.468 → **24 @ 0.040**, `netflix-mylist-mobile-01` refused → 15 @ 0.061, and `blank-no-content-01` and `low-quality-jpeg-01` both became false accepts — while `netflix-mylist-desktop-01`, the two-row fixture the change was built for, stayed refused. The cause is structural: a band of TEXT locks a strong period at its letter spacing and, yielding many narrow "tiles", outvotes the real row under any tile-count-weighted rule. Discriminating a caption strip from a tile row needs a size prior the raster cannot supply, because its axes are scaled independently and true aspect ratio is not recoverable from it. Generalising means carrying the source dimensions and gating on tile aspect. `c` holds every accepted tile well-formed, in-bounds and disjoint left-to-right — an overlap would mean two crops showing the same artwork. |
| AC-5 | U | `T-AI-053` (new) | **A possessive shelf header is chrome (§3.2 step 3).** Reported by the owner from a live Disney+ capture: **"My Watchlist"**, the header of the very list being captured, reached the review pass as a row in "Couldn't identify these", chipped *"The text reader saw this, the tile reader did not"*. ⚠ **`watchlist` was ALREADY in the vocabulary, which is why this needs a named test and not a one-word commit.** The term was present, read as coverage, and could never fire: step 3 is an **exact line match** by design (`T-AI-004g` — a substring test would delete every work whose name contains a UI word), and Disney+ ships the possessive inside the same OCR line. Every service spells its saved list differently, which is why `my list`, `my netflix`, `my stuff` and `my purchases` are each separately enumerated; `my watchlist` joins them. `a` pins both the predicate **and the verdict through `cleanup`**, so the term cannot survive while the pipeline stops consulting it. `b` is **the limit, and the reason this is not a generic `my <term>` rule** — such a rule would fold `my home` onto `home` and suppress a work genuinely called *My Home*; `My Cousin Vinny` must still classify as a title. `c` holds §3.2's `ocr-only` scope: the primary reader stays exempt, because a fixed vocabulary applied to it would suppress a real work named after a UI word. Mutation-proven — deleting the term fails `a` alone, leaving `b` and `c` green. |
| AC-5 | U | `T-AI-052` (new) | **The tile-boundary guards on §3.2 step 1's HORIZONTAL rung (`mergeable`).** Two OCR boxes in DIFFERENT tiles are not merged into one candidate just because their coordinates happen to be close, and the two shapes that legitimately do merge on one line still do. ⚠ **A NEGATIVE GAP IS NOT A SMALL GAP.** `mergeable` tested `gap < OCR_MERGE_GAP` and never tested that `next` is to the RIGHT of `prev`, so a gap of -0.87 — meaning `next` starts 87% of the image width LEFT of where `prev` ended — scored as the most mergeable value available. The row-bucket sort manufactures that pairing whenever a bucket edge falls mid-row, handing one bucket's last item (large x) to its successor's first (small x). ⚠ **`wrapsUnder`, the VERTICAL rung of the same function, has refused both a negative gap and an incomparable height since it was written; the horizontal rung inherited neither, which is why the defect survived four rounds of false-title tuning.** Asserted on geometry copied verbatim from `tests/fixtures/golden/ocr/`: `SOL | MAN ON FIRE` (gap -0.487) is refused, and separately that its HEIGHT ratio PASSES, so the gap guard alone is doing the work; `WHISPER | HAMNET` (gap +0.0215, height ratio 0.41) is refused, and separately that its GAP passes, so the height guard alone is doing the work — the two are mirrors, proving neither guard masks the other. The limits: the truncation marker `The Hitchhiker's Guide to the \| ..` still merges, and a two-word badge still merges. ⚠ **The height floor cannot be raised**: the tightest legitimate merge is 0.6786 and the nearest surviving conflation, `GREENLAND 2 \| TRUE DETECTIVE`, is 0.69 — ABOVE it. No value of the constant separates them, which `f` pins from both sides. Measured effect: 13 of the corpus's 14 horizontal conflations removed; incumbent false-title rate 0.0877 → 0.0545, challenger 0.2464 → 0.2239. Mutation-proven — reverting either guard fails `a` or `c`. |
| AC-5 | U | `T-AI-050` (new, TASK-205) | **The wrapped caption continuation (§3.2 step 1b).** Two OCR lines of ONE caption that wrapped are merged into the whole title; two captions that merely sit above one another are not. ⚠ **THE TWO DIRECTIONS HAVE OPPOSITE COSTS, WHICH IS WHY THE REFUSALS OUTNUMBER THE MERGES.** A wrap left unmerged costs two false titles and a missing one, but every fragment is still on screen; a merge that should not have happened FUSES TWO WORKS into one candidate and the second is unreachable in review - a title LOST. Asserted: the real corpus geometry merges (left edges flush at x 0.350, gap 0.0016) and produces an EXPECTED title; the merged box is the union, so later geometry sees the whole caption; confidence is worst-of, not mean; the next tile's caption at the closest measured distinct-work gap (0.079) is REFUSED; an indented metadata row is refused; a shorter face (a badge) is refused; chrome is refused in either position; an overlapping duplicate read is refused; the ceiling is exercised from both sides; a THREE-line caption chains, which the accumulated-union reference would break; and `llm` items are never merged. Six mutations applied, each caught. |
| AC-5 | U | `T-AI-048` (new, TASK-203) | **The fragment collapse (§7.4a).** An OCR fragment of a caption another candidate holds WHOLE collapses into it, retained and pointing at the survivor. ⚠ Every guard has a DISCRIMINATING TWIN, and the twins are the test: `true detective` ⊂ `true detective night country` must NOT collapse (proximity), `raw` ⊄ `brawl` (token boundary), a cross-image containment must not collapse (same image), `hbo` ⊂ `hbo original` must not (verdict), and a two-host fragment parents onto the MAXIMAL host only. Text containment points both ways in the real corpus, so a suite that only asserts the collapses would pass for a rule that collapses everything and loses real titles. All five guards are mutation-proven. |
...
| — | C | `T-UX-067` | After a failed close, a saved `in-review` read and refreshed summary allow an explicit retry with preserved consent; success clears the failure state. Network failure alone never enables replay or proves nothing changed (`T-UX-163`). Review stays mounted. Pending/removal-consent refusals and authentication remain distinct. ~~Superseded: every 5xx/network failure asserted "Nothing was changed" and immediately re-enabled Apply without checking saved status.~~ |
| — | C | `T-UX-060` | `specs/ux-states.md` §6.1: the review's loading state draws a CARD SKELETON sized from the candidate count carried forward in history state from the batch screen, and a COUNTLESS skeleton on a cold deep-link. Asserted at `apps/web/test/reviewSkeleton.spec.tsx` (`T-UX-060a` … `T-UX-060m`). ⚠ **§6.1 named a source that cannot supply what it asked for**, which is why this id sat in `KNOWN_UNCOVERED` rather than being merely unbuilt: it wanted "section skeletons with their counts already shown from `GET /api/batches/:batchId`", but that payload carries `candidateCount` per IMAGE and nothing per section — sectioning is `sectionForCandidate` (`packages/domain/src/review.ts`) and happens when the review is BUILT, so no per-section count exists before the review has loaded. The owner ruled out both an API change and a second request; §6.1 is corrected in place. ⚠ **The carried count is untrusted input reaching a LOOP BOUND.** History state survives a reload, is restored by the back button, and can be handed to this build by a browser session restored across a deploy — `parseSkeletonCount` validates rather than casts (the rule `parseAppliedState` follows in `containers/ListRoute.tsx`) and caps at `REVIEW_SKELETON_MAX`; `T-UX-060c`/`d` assert both. ⚠ **Anything unusable draws the COUNTLESS skeleton, never a guessed count** — including a `0`, and including a batch where any image has not yet reported (`T-UX-060k`): a placeholder count is a claim about how much was read from the owner's screenshots, and summing an unreported image as zero would under-report it on the one screen that exists to show what was found. |
| — | C | `T-UX-066` | `specs/ux-states.md` §6.14: a 409 `PENDING_ADDITIONS` on the CLOSE renders an inline count — *"N titles still need a decision."* (`reviewPendingAdditions()` in `apps/web/src/copy.ts`) — and moves focus and scroll to the FIRST still-pending card (`apps/web/src/pages/ReviewPage.tsx`, keyed on the candidate ids the 409 names, whose DOM id comes from `reviewCandidateDomId()` in `apps/web/src/components/CandidateCard.tsx`). **Nothing is applied and it does NOT navigate.** Distinct from §6.16: the owner has a decision to make, not a server to retry, so §6.16's "nothing was changed, try again" copy must NOT fire — `T-UX-066d` pins that from this side and `T-UX-067f` from the other. Asserted at `apps/web/test/reviewCloseErrorSiblings.spec.tsx` (`T-UX-066a` … `T-UX-066e`). ⚠ **Why this row exists:** like `T-UX-067`, the id lived ONLY in `ux-states.md`, seen by neither `check-test-ids` nor `check-orphan-tests`, so the state was specced-with-an-id yet wholly unhandled — `ReviewRoute` returned early on every 4xx and a 409 produced no feedback at all. |
| — | C | `T-UX-068` | `specs/ux-states.md` §6.17: offline mid-review shows a banner, dispositions keep working locally, and **Apply changes** is disabled with the reason. ⚠ **Catalogued but deliberately NOT IMPLEMENTED in v1.** There is no `it('T-UX-068' …)` test, and this catalogue row is a *mention*, not an implementation. `tests/infra/status.spec.ts`'s `T-STATUS-001p` probe uses `T-UX-068` as its worked example of an id that is *mentioned but never defined by a test title*; implementing a `T-UX-068` test would break that probe. Do not add one without first reworking `T-STATUS-001p`. Reported as a standing gap on the `T-UX-067` follow-up, not fixed. |
| — | C | `T-UX-069` | `specs/ux-states.md` §6.18: a 401 mid-review shows *"Your session ended. Sign in again — your review is still here."* with **Sign in** returning to this URL and local dispositions preserved. Asserted at `apps/web/test/reviewSessionExpired.spec.tsx` (`T-UX-069a` … `T-UX-069j`). ⚠ **This needed CODE, not copy.** A 401 is intercepted globally inside `apps/web/src/lib/apiClient.ts` (`request`), which redirected before `ReviewRoute` ever saw the `ApiError` — bouncing the owner away from *uncommitted dispositions* with nothing to tell them those survived (they always did: `sessionStorage`, SD-11e / `T-UI-027`). The review now registers a screen-scoped handler (`setUnauthorizedHandler`) that renders the session-ended page instead of redirecting. ⚠ **The scoping is the risk, not the rendering.** A handler left registered after the review unmounts disables the 401 redirect for the WHOLE APP, and the symptom appears on every other screen — `T-UX-069g` asserts the clear and `T-UX-069j` pins that an explicit per-call `onUnauthorized` still wins. The global default is deliberately unchanged everywhere else: a session that expires on a timer must not become an error screen offering a retry that fails identically forever. ~~Superseded: "Catalogued but NOT separately implemented in v1 … the review-local preservation copy is unbuilt."~~ |
...
| AC-2 | I | **`T-AI-004`** | Every candidate is reachable in the review response; all verdicts represented (incl. `inferred-unverified`, `unreadable-tile`); nothing dropped. <br> ⚠ **The unit half is delivered by TASK-057** (`packages/domain/test/cleanup.spec.ts`, `a`–`u`); the `I`-typed review-response half waits on TASK-059. <br> **TWO DEFECTS IN `specs/ai.md` §3.2, RESOLVED IN CODE AND REPORTED, NOT EDITED:** (1) the step order runs the **length gate (2) before "model declined" (7b)**, so every `unreadable-tile` — `rawText: ''`, `inferredTitle: null`, therefore `matchText: ''` — is stamped `chrome-suspected` and buried in §3.3's collapsed "Probably not titles" group, contradicting 7b's "**Never dropped**" and §3.3's main-list thumbnail. The loss is invisible: the tile is technically still on screen, in the group the owner is least likely to open. `unreadable-tile` is therefore decided **first** (`T-AI-004d`). (2) Step 4 says "**> 60 %**" and names `1h 52m` and `S2:E4`; both compute to **exactly 0.60** (3 of 5 non-space characters), so the strict `>` catches **neither of its own two examples** — implemented `>=` (`T-AI-004j`). <br> **Verdict precedence is unspecified in §3.2** and was chosen explicitly: `unreadable-tile` > `chrome-suspected` > `inferred-unverified` > `low-confidence` > `title-candidate`, with `inferred-unverified` deliberately **above** `low-confidence` against the numeric step order, because 7a is the RSK-028 fabrication mitigation and drives a **mandatory thumbnail** (`T-AI-041`) whereas 7 drives a sentence (`T-AI-004e`). <br> A year is **never lifted when nothing would remain** — *1917*, *2012*, *1984* — which would both empty `matchText` into step 8's chrome group and hand the §4.2 matcher a year it invented, costing 0.15 against the real release year. Both failures are silent (`T-AI-004o`, `T-AI-004u`). <br> **TASK-195 extends the unit half with `ai`–`al`.** `ai`/`aj` cover §3.2 step 1's chrome exclusion — a nav bar's labels sit inside the 3 % merge gap, so grouping (step 1) glued them into `NETFLIX Home Shows Movies` **before** the exact-match vocabulary (step 3) ever saw them, and no term could match a string the UI never rendered. `ai` asserts three adjacent labels stay three candidates; `aj` is its limit, asserting a genuine two-line caption on the same screen still merges, so the rule cannot be widened to "never merge on a row containing chrome" and undo `q`. `ak`/`al` cover new step 3a: `Continue Watching for <profile name>` is matched as an anchored **prefix**, and `al` is the discriminating twin proving it is not a substring test — *A Man for All Seasons* survives. Both rules are mutation-proven. <br> **TASK-199 adds `am`–`ap`** for the tile-painted badge vocabulary — `hbo`, `hbo original`, `new` — which read like artwork rather than navigation and so escaped the nav-bar pass entirely; `an` is the safety twin proving step 3 stays `ocr-only`-scoped, which is the only reason a term as short and generic as `new` is tolerable at all. <br> **R6 adds `aq`/`ar`** for the Netflix badges `recently added`, `new season` and `new episodes`, reported from a live owner capture. ⚠ **`aq` IS A CONTENT-SAFETY TEST, NOT A TIDINESS ONE.** Chrome surviving step 3 is not merely shown as an unidentified row — it is handed to the matcher as if it were a title, and a generic two-word phrase matches whatever TMDB ranks first: `New Season` resolved to an unrelated Japanese release whose poster is adult artwork, and that poster was rendered in the owner's review screen. The badge vocabulary is the only thing between a UI label and arbitrary artwork on that screen. ⚠ **`recently added` was ALREADY a named false title in the committed baseline** (`docs/evaluation/golden-2026-09-18.md`, `netflix-mylist-mobile-01`, 1 of 3 runs), so this closes a gap the golden corpus had recorded and nobody had acted on — the §4A report naming false titles did its job and the naming was not read. ⚠ **`ar` IS THE DISCRIMINATING TWIN FOR THE THREE-WORD TERMS, WHICH `ao` CANNOT COVER.** `new` was already a term and did **not** match `new season`, the same lesson `watchlist` taught against `my watchlist`: step 3 is an exact line match, so a shorter term never covers a longer line. The tempting fix is a `new <anything>` prefix rule; `ar` pins *Season of the Witch*, *A New Season of Love* and *New Jack City* as titles so that rule cannot be written. `aq` is mutation-proven (removing the three terms fails it); `ar` correctly stays green under that mutation, because it guards against over-reach rather than under-reach. |
...
| — | I | **`T-AI-039`** | *(R2)* A title the model omitted but OCR saw is recovered as an `ocr-only` orphan candidate — REQ-012 enforced against the model. ⚠ **`d`/`e` (TASK-194) bound step-1c CONSUMPTION, which is the other way this guarantee is lost.** An overlapping OCR line used to be consumed on geometry alone, so a caption contradicting the tile it sits in was deleted *by that tile*, at stage 1, with no record — the model's own misreading suppressing the correction. `d` withholds consumption when the tile's `basis` is `artwork`, i.e. when the model is on record as **not** having read printed glyphs, so the contradicting line survives as an orphan and both readings reach the owner; it is mutation-proven (restore the unconditional `consumed.add` and only `d` fails). ⚠ **`e` is the MEASURED limit on `d` and must not be relaxed into it.** Requiring agreement from every overlapping line regardless of basis — the fix as first written down in `docs/backlog.md` TASK-079 finding (1) — was measured against the golden corpus and moved the false-title rate **0.2500 → 0.4803 with no change to recall or omission recovery**: split captions, episode badges, runtimes and "New episodes" flashes all disagree with the tile subject and would each become a candidate to dismiss. ⚠ **`f`/`g`/`h` (TASK-290) bound the OPPOSITE failure — consumption that does not happen at all.** On the owner's first real Disney+ capture the model emitted a plainly synthetic uniform grid of tile boxes (x roughly right, y shifted down and stretched), so **not one** caption line overlapped a tile, nothing was consumed, and every fragment of a caption the model had **already transcribed** returned as its own review row: one film appeared three times, and correcting two of those fragments onto one work is what then made the batch fail to apply (`T-REV-012bi`). `f` absorbs a fragment whose glyphs already appear as a whole-word run of a tile's own transcription — orphan recovery exists to catch what the model **omitted**, and nothing was omitted here — and asserts the tile is recorded as **corroborated**, since leaving it `ocrSupport: 'none'` would reach `inferred-unverified` for a title both readers read correctly. ⚠ **`g` is the MEASURED limit on `f`, and the wider rule cost a real title.** Word-run containment alone absorbed `True Detective` in `max-saved-desktop-01`, where the owner has saved both it and `True Detective: Night Country`; the model had missed the shorter title's tile, so the OCR line was its only recovery, and aggregate recall fell to **0.9403**, under the §9.2 floor. Franchise and season naming makes that shape ordinary. So `f` additionally requires evidence that OCR **split** a caption: a second unconsumed line holding a different part of the same transcription, stacked directly against the first (`OCR_LINE_STACK_GAP_MAX`). `h` pins that the two halves of one split cannot cancel each other, which a consume-as-you-walk loop would allow. Measured over the whole golden corpus: **three duplicate candidates removed, every pinned metric unchanged.** |
| — | I | **`T-AI-032`** | *(R2)* Fabrication rate ≤ 0.05; every unsupported inference is flagged `inferred-unverified` |
| — | C | **`T-AI-041`** | *(R2)* An `inferred-unverified` candidate renders its **tile thumbnail** beside the proposed title; `unreadable-tile` likewise, for which the tile is the ONLY evidence the row carries (`specs/ai.md` §574 step 7b, `specs/ui.md` §5.3a). Implemented in `apps/web/test/candidateThumbnail.spec.tsx`, which mounts the real `ReviewPage` and drives it through `buildReviewResponse`. <br> ⚠ **THE TEST MUST NOT SUPPLY `thumbnailUrl` ITSELF.** `CandidateCard` accepted the prop and **no caller passed it** — it defaulted to `null`, the render condition was never true, and the thumbnail had never once appeared in the product while the catalogue showed this row as covered. A component test that hand-writes the prop steps over exactly that dead wiring. The assertion must be made against `ReviewPage`'s derivation from `sourceImageIds`, which is the seam the defect lived in. <br> ⚠ **DIVERGENCE RESOLVED — the crop is implemented, client-side.** `specs/ui.md` §5.3a's "**cropped** tile thumbnail" is real: `tileCropFor` (domain) computes the normalised region server-side and `CandidateCard` clips to it with `overflow: hidden` and a scaled `<img>`. No crop *endpoint* was added, and none should be — `GET /api/images/:imageId` still serves whole screenshots, because cropping bytes server-side would put image processing on the request path of a 0.25 vCPU container. The crop is presentational. <br> ⚠ **THE CROP RULE WAS INVERTED FOR THE WHOLE OF THIS ROW'S EARLIER LIFE, AND THESE CASES DID NOT NOTICE.** `tileCropFor` read `if (boxSource !== 'llm') return null`, and `crossCheck` sets `'llm'` to mean *no OCR line corroborated this tile* — so a crop was offered exactly when the geometry was the reader's unverified guess and refused exactly when it had been measured. Every case here passed throughout, because they feed hand-written boxes and therefore prove the arithmetic and nothing about the pipeline's real output; `T-AI-055` measures that and is the guard that matters. The cases were rewritten to the new rule: **position from the corroborating OCR caption, size from the reader's tile, and only when the caption centre lies inside that tile.** ⚠ **`h` is now the INVERSION of its own former self**, as `i` already was: it read *"an OCR box yields NO crop"* — asserting the defect — and now reads *"an OCR box with **no tile** yields no crop"*, which is the surviving true half of that reasoning (a caption strip alone has nothing to be sized to, and cropping to it shows the `rawText` already printed beside it and none of the artwork). `f`, `g`, `i`, `k`, `l`, `m`, `n` moved from `boxSource: 'llm'` to `'ocr'` — ⚠ **left on `'llm'` they would have stayed green while exiting at the first line, testing nothing.** <br> ~~Superseded: "`boxSource === 'llm'` is the real limit and survives untouched (`T-AI-041h`/`x`)."~~ <br> ~~Superseded: "DIVERGENCE, RECORDED NOT RESOLVED: … no crop endpoint exists … The implementation renders the full screenshot. Whether v1 accepts that or requires a real server-side crop is an open decision for the owner."~~ |
...
| AC-5 | I | `T-REV-014` | Correcting to a work with an existing active listing is **applied**, re-classified as already-present, and **withdraws the removal** the uncorrected misread was proposing. ⚠ **THIS ROW USED TO SAY THE OPPOSITE, AND THAT IS HOW THE DEFECT SHIPPED GREEN.** The route answered 409 `DUPLICATE_WORK_IDENTITY` unless the caller sent `confirmDuplicate`, and this row recorded the refusal as AC-5's test — so the mapping agreed with the code while both contradicted the acceptance criterion in `docs/PRD.md`, which reads *"the correction **is applied**, the item is re-classified as already present, moves out of Additions, and closing the batch does not create a duplicate Title."* A test that asserts an inversion of its own AC cannot fail on the defect it names. Three facts condemn the gate: `specs/api.md` §6.18 lists **200, 400 and `BATCH_NOT_IN_REVIEW`** as this endpoint's whole outcome set (the three real `DUPLICATE_WORK_IDENTITY` sites — §6.5 fix-match, §6.10 restore, §6.31 manual add — are each paths that would create a second **Title** row, which this one cannot: `batchClose` resolves through `findTitleByWorkIdentity` and re-uses the row); in a **full update** the condition is the mode's *defining* state, since a re-capture of the whole list means nearly every title in it is already listed; and it was **inescapable**, because `ReviewRoute.matchUnmatched` never sends `confirmDuplicate` and `UnmatchedActions` has no branch for the code, so the owner saw only *"Couldn't save that. Nothing has changed."* ⚠ **It was filed as DATA LOSS, not friction, and `T-REV-014e` is why.** Reconciliation proposes removing every active listing outside the union of the surviving candidates' `resolvedWorkIdentity` — so a refused correction leaves the candidate on the *misread* identity, the real work never joins the union, and **its listing is offered for removal**. The gate manufactured the exact failure product invariant 2 / REQ-006 exists to forbid: a failed extraction of a known title read as a removal. Measured live by the owner on a Disney+ full update. `T-REV-014a` pins the applied write, `b` that a stale `confirmDuplicate` is neither required nor obeyed, `c` that a *removed* listing was never a duplicate (invariant 7), `e` the removal withdrawal plus AC-5's re-classification clause; `T-REV-014d` (unit) pins the write itself, since the two fail to different mutations. `T-SUP-002c` was re-pointed from *"suppression fires before the duplicate gate"* to *"suppression survives the duplicate gate's deletion"* — the ordering question died with the gate, but REQ-071's back door must still be refused. Restoring the gate fails `T-REV-014a/b/c/e` **and** `T-REV-014d`. ~~Superseded: "Correcting to a work with an existing active listing → 409 `DUPLICATE_WORK_IDENTITY` unless confirmed".~~ |
...
| AC-2 | C | `T-REV-016` | "Already on your list" items offer optional persisted Confirm match acknowledgement and secondary Change match, never addition/discard controls. Refused acknowledgement stays retryable; neither append-only nor full-update requires it to reach the summary. `T-REV-012d` excludes known pending/confirmed/corrected readings from additions; deliberate identity correction is covered by `T-UX-162g`. |
| AC-3 | I | **`T-UI-006`** | Append-only review retains known extracted titles with the true count; only removals are omitted |
...
| AC-4 | I/C | `T-LIST-023`, `T-UI-016`, `T-UX-139`, `T-UX-144` | AND across dimensions, OR within a dimension; labelled dropdown fields expose current selections, and URL-driven pickers/chips preserve unrelated selections and ordering |
| AC-5 | C | `T-UX-013`, `T-UX-139` | Zero-match is distinct from an empty library; chips remain visible with nonzero results, each removes only its dimension/value, and Clear filters removes dimensions plus q without resetting sort/view |
...
| AC-6 | I/C | **`T-LIST-026`/`T-UI-024`**, `T-UX-138`, `T-LIB-001` | Recently added reverses to Oldest additions in one press; ordering stays deterministic. #328 extends persistence across browser restarts for the complete destination. `T-UI-024o`–`q` assert remembered return URLs, explicit-filter link authority and field defaults. The label must never describe a different order from the API request. |
...
| AC | L | Test | Assertion |
|---|---|---|---|
| AC-1 | I/C | `T-FRESH-010`, `T-UX-143` | Service updates exposes each factual last-completed date and service-preselected upload link |
| AC-3 | I/C | `T-FRESH-012` | Opening Service updates shows "never updated", not an error |
| AC-4 | I | `T-FRESH-013` | Abandoned/failed batches never update `serviceState` |
| AC-5 | C | `T-FRESH-014`, `T-UX-143` | If dates cannot be computed, the list renders and degradation remains visible outside the closed disclosure; every service link is still reachable inside |
...
| AC-7 | I | **`T-FIX-005`** | *(SD-06)* An active suppression **migrates** to the corrected identity when fix-match re-points the work, and the response reports the migration. Without it, correcting a match silently resurrects a work the owner had dismissed. Also listed under US-030, where it has no criterion of its own |
...
| — | I | **`T-FIX-005`** | *(SD-06, new)* An active suppression on the old identity migrates to the new one and the response reports it. **Its criterion is US-028 AC-7**, where it is now mapped; kept here because the behaviour lives on this endpoint |
...
| AC | L | Test | Assertion |
|---|---|---|---|
| AC-1 | I | **`T-MANUAL-001`** | A chosen TMDB work + service becomes a row on `GET /api/titles` with that badge, dated today. Asserted through the LIST, not the handler's own return value |
| AC-2 | I | `T-MANUAL-002` | The same work on the same service is 409 `DUPLICATE_WORK_IDENTITY`; listing and title counts are unchanged |
| AC-3 | I | **`T-MANUAL-003`** | The same work on a second service yields ONE row with two badges — `titleWasCreated: false`. REQ-005 has no merge affordance, so a second row is unrepairable |
| AC-4 | I | **`T-MANUAL-004`** | A suppressed work is 409 `WORK_SUPPRESSED` with `unsuppressHref`, and **zero TMDB calls** are made. The gate ORDER is the assertion |
| AC-5 | I | `T-MANUAL-005` | TMDB miss → 404 `TMDB_WORK_NOT_FOUND`; TMDB transport failure → 502 `TMDB_UNAVAILABLE`. Title and listing counts stay 0 on both |
| AC-6 | I | **`T-MANUAL-006`** | A body carrying `dateAdded: '2019-01-01'` still stores today. The dangerous shape is accepting the field and dropping it, so the STORED value is asserted |
| AC-7 | I | `T-MANUAL-007` | A row seeded at `2026-04-02` gaining a badge today keeps `sortDateAdded: '2026-04-02'` (product invariant 6) |
| — | U | **`T-MANUAL-023`**, `T-MANUAL-030` | *(reachability)* The affordance is on the LIST page — not in the empty state, not in `/upload` — and is not rendered at all when the container wired no handler. ⚠ Present-but-dead is indistinguishable from working to every other kind of test, so the absence is what is asserted |
| — | U | **`T-MANUAL-024`**, `T-MANUAL-025` | The service picker has **no default** and refuses to submit without one, and the body carries `tmdbId`/`mediaType`/`service` and nothing else. A default would write a badge the owner never chose |
| — | U | `T-MANUAL-026`, `T-MANUAL-027`, `T-MANUAL-028`, `T-MANUAL-029` | Success refetches the list; a 409 duplicate does **not** (nothing was written); a 409 suppression renders the `unsuppressHref` way out; offline the button is disabled with the reason as text |
| — | U | **`T-MANUAL-032`**, `T-MANUAL-033` | The §6.30 body parser accepts a CLOSED field set — `name` and `dateAdded` are not merely dropped, they are never read — and refuses a non-object body, a non-positive-integer `tmdbId`, and a `mediaType`/`service` outside its permitted set. `dateOnlyUtc` truncates to **midnight UTC**, asserted on a late-evening instant, the only case where a local-time truncation shifts the day. Implemented in `apps/api/test/unit/routes/manualListEdits.spec.ts` |
| — | U | **`T-MANUAL-034`**, `T-MANUAL-035` | The handler's arms over real HTTP with the store and TMDB mocked: a new work writes a title **and** a listing with `createdByBatchId: null` on both; a work already on another service gains a **badge, never a second row**; a long-held `sortDateAdded` is **not** pulled forward (product invariant 6) while a later or null one is; a `removed` title is re-created rather than resurrected (invariant 7); the **suppression gate runs before TMDB is ever called**; a TMDB miss and a TMDB outage both write nothing; a raced unique violation is a 409 `DUPLICATE_WORK_IDENTITY` rather than a 500, and a store error that is **not** a unique violation is not disguised as one. ⚠ `isUniqueViolation` is kept REAL and driven with a genuine SQL `number: 2627` — stubbing it would make that fork pass whatever was thrown. Implemented in `apps/api/test/unit/routes/manualListEdits.spec.ts` |

### US-048 — Remove a title from my list by hand
| AC | L | Test | Assertion |
|---|---|---|---|
| AC-1 | I | **`T-MANUAL-008`** | The row leaves `GET /api/titles` and its listings are `removed`; the response reports `suppressed: false` |
| AC-2 | I | **`T-MANUAL-009`** | `suppression.count()` is 0 after a removal, and re-adding the same work succeeds rather than hitting the suppression gate. This row is the whole distinction from US-027 |
| AC-3 | I | **`T-MANUAL-010`**, `T-INV-012` | Title and listing rows still exist and the listing is in `GET /api/removed`, `restorable: true`. `T-INV-012` guards the source half — no `prisma.*.delete()` in `manualListEdits.ts` |
| AC-4 | U | `T-MANUAL-014`, `T-MANUAL-015` | `toRemovedItem` derives `removedBy: 'owner'` from a NULL batch id and `'batch'` otherwise, so the two readings cannot drift |
| AC-5 | I | **`T-MANUAL-011`** | Every `removedListingIds` entry restores through the existing `POST /api/listings/:id/restore`; both badges return. Restoring a SIBLING listing of an already-active title is not a duplicate |
| AC-6 | I | `T-MANUAL-012` | Already-removed → 409 `TITLE_NOT_ACTIVE`; suppressed → 409 `WORK_SUPPRESSED` with `unsuppressHref`; a foreign or unknown id → 404. No listing state moves on any path |
| AC-7 | I | `T-MANUAL-013` | A two-badge row removes both services in one call and logs two independently restorable removed-view entries |
| — | U | **`T-MANUAL-016`**, `T-MANUAL-022`, `T-MANUAL-030`, `T-MANUAL-031` | *(reachability)* The row menu offers **Remove alongside Not interested**, not instead of it — the whole point of the feature, and the assertion that stops a later tidy-up collapsing the two decisions into one item. Disabled offline with the reason as text; absent entirely when unwired; and — the case the all-or-nothing test cannot see — **absent when only PART of its wiring is present**, since the dialog needs both the call and its undo and a third item that opens nothing is the exact present-but-dead defect `rowMenu.spec.tsx` exists to close |
| — | U | `T-MANUAL-017`, `T-MANUAL-018` | Opening the dialog is a confirm step that writes nothing, and confirming removes the row the menu was opened from and only that row |
| — | U | **`T-MANUAL-019`** | Undo restores **every** `removedListingIds` entry, one call each. A two-badge undo that restored only the first would return the row with a badge silently missing |
| — | U | `T-MANUAL-020`, `T-MANUAL-021` | 409 `TITLE_NOT_ACTIVE` reads as *already done*, not as a failure; any other failure puts the row back, so nothing is hidden that was not removed |
| — | U | **`T-MANUAL-036`**, `T-MANUAL-037` | The §6.32 handler's arms with the store mocked: an unknown or foreign id is a **404, never a 403** (`T-SEC-002d` — a 403 confirms the row exists); a suppressed work is refused with `unsuppressHref` and **writes nothing**, because suppression sits above listing state in a separate table and a `title.state` check alone would let it through; a title with no active listings is a **409, not a 404**, since the owner may be looking at it in the removed view; a two-badge title soft-deletes **both** listings with `removedByBatchId` and `removedByGroupId` NULL; and the response reports `suppressed: false` — the whole distinction from US-027. `T-MANUAL-037` asserts the title state is **derived from the listings as they now stand**, not assigned: given a sibling listing this handler did not touch, the title stays `active` and `sortDateAdded` is recomputed from what remains. Implemented in `apps/api/test/unit/routes/manualListEdits.spec.ts` |
...
| AC-2 | S | **`T-META-002`** | **The twenty** PRD-mandated invariants — **every numbered row of the §6 table** (sixteen today) plus US-016 AC-6, US-021 AC-6, US-026 AC-6, US-038 AC-5 — each resolve to an existing, passing test. `tests/meta/invariantCoverage.spec.ts`. ⚠ The gate **reads the §6 table** rather than a hard-coded count, and `T-META-002b` pins both end rows, so a parser that silently truncates fails. ~~"The nine PRD-mandated invariants (§6 rows 1–5 …)"~~ *(Superseded: §6 has not had five rows for a long time — its own heading was already corrected from "eleven" to "sixteen" for this same drift, and this row was never reconciled. Rows 6–16 include `T-INV-013`, `T-IMG-017` and `T-AI-036`; a gate honouring "rows 1–5" would have ignored all eleven.)* |
...
| AC-2 | U | `T-WAIT-012` | The discovery capture runs the **same** pipeline, proven structurally rather than by agreement on a fixture: no module on the extraction path reads any discovery/service discriminator (`a`), the entry point accepts no source so no caller can select a variant (`b`), extraction is started unconditionally at both call sites (`c`), and no rival pipeline module exists (`d`). `e` is the positive control — the discriminator is still read at batch creation and close, so `a` cannot pass by the feature having evaporated. `tests/infra/discoveryPipelineParity.spec.ts` |

| AC-3 | I | `T-WAIT-003` | A closed discovery batch creates **no** `ServiceListing`, changes no service badge count and leaves the combined list byte-identical (`a`, `b`); `d` also proves the waiting work is absent from the **removed** log, so "invisible to the list" was not achieved by making it look deleted. `apps/api/test/integration/discoveryClose.spec.ts` |
| AC-4 | I | `T-WAIT-002` | Reconciliation never runs: a second capture omitting a previously-seen title proposes no removal (`a`), while `b` — **the discriminating case** — shows the same omission in a Netflix full-update *does*, and `c` shows the store itself refuses a full-update discovery batch |
| AC-5 | U/I | `T-WAIT-004`, `T-WAIT-003` | A work already in the combined list produces **no** intent and is REPORTED rather than silently dropped — `T-WAIT-004a` on the decision, `T-WAIT-003c` end to end through the store |

~~*(US-040 AC-2 is landed with the extraction path; until its test exists it remains declared in `KNOWN_UNMAPPED`.)*~~ **Superseded at TASK-126: `T-WAIT-012` now maps US-040 AC-2 and the id has been removed from `KNOWN_UNMAPPED`.**

### US-041 — Curate the rental page down to what I actually want
| AC | L | Test | Assertion |
|---|---|---|---|
| AC-1 | U | `T-WAIT-005` | A discovery review pass renders (`service: null`), routes **every** extracted candidate to a section including already-listed works, and leaves every disposition `pending`. Service captures likewise retain known matches. |
| AC-2 | I + U | `T-WAIT-006` | Discarding a discovery candidate creates a `Suppression` keyed on canonical **work identity**, and the discarded work becomes no title and no intent. |
| AC-3 | I | `T-WAIT-006` | The suppressed work is absent from the next capture's review pass **entirely** — the check is before record creation. |
| AC-4 | I | `T-WAIT-006` | The rationale is asserted by the named test: the same page captured twice with a discard in between, verified empty of it. |
| AC-5 | I | `T-WAIT-006` | The **discriminating case** — the same discard in a Netflix pass writes no suppression, so the behaviour is scoped to discovery sources. |
| AC-6 | I | `T-WAIT-007` | A suppression failure inside the close rolls the whole batch back: no suppression, no intent, no title, and the batch is still `in-review` and retryable. |
...
| AC-3 | I + U | `T-AVAIL-003` | A `flatrate` hit on a service the owner has is **flagged with an invitation**, and the work stays out of the combined list: `T-AVAIL-003a` asserts the flag, the absence from `GET /api/titles`, and that **no `ServiceListing` was invented**. ⚠ **`T-AVAIL-003b` is the load-bearing negative** — membership *and ordering* of a combined list seeded with three real listings are byte-identical across the refresh; `T-AVAIL-004b` makes the same comparison against an EMPTY list, where any ordering claim is vacuous. `T-AVAIL-003c`/`d` pin the rendering: the flag names the service and links to `/upload`, and a rent-only answer is not flagged |
| AC-6 | U | `T-AVAIL-006` | **The sentence the data can support.** NOT KNOWN (`null`, `a`) and ASKED-AND-NOBODY (`[]`, `b`) render the *same* bounded claim about the owner's own services with the as-of date attached; a never-checked row says *"Not checked yet."* rather than inventing one (`d`). ⚠ **`T-AVAIL-006c` is the point** — the forbidden claim *"not streaming anywhere"* is asserted absent from the **whole document** across all three data shapes, because every other assertion here passes just as happily against a build that renders it. `e` asserts the rule against the exported `availabilityLine` directly, since what is under test is which sentence is CHOSEN |
| AC-7 | I + U | `T-AVAIL-007` | TMDB unreachable is a **200 rendered from last-known availability**, with its original as-of date and an unobtrusive note — never blank, never an error page (`a`). The store is asserted untouched: a failed lookup that wrote `null` would erase a known-good answer and, under Trap 4's rendering rule, silently downgrade the row. `T-AVAIL-007b` is the discriminating half — a wholly successful render does **not** raise the note, without which `a` passes against a hard-coded `true` |
| AC-9 | E + U | `T-AVAIL-009` | The **JustWatch** attribution is rendered unconditionally on the waiting view, asserted in a real browser on **both** the populated and the empty screen with a non-trivial painted box (`a`). ⚠ Rendering it beside the rows — the natural, wrong implementation — fails only on the empty screen, which is the one a casual check skips. Like the TMDB obligation, its absence is invisible from inside the product: nothing errors, nothing logs, the screen looks healthy |

### US-043 — Browse and clear the waiting list
| AC | L | Test | Assertion |
|---|---|---|---|
| AC-1 | E + U | `T-WAIT-010`, `T-AVAIL-006` | Each waiting row carries the work's name, year, poster, the storefront it was discovered on and the date it was discovered, plus the availability sentence `T-AVAIL-006` governs |
| AC-2 | U | `T-AVAIL-003` | A work now `flatrate` on a service the owner has is flagged in the list with a link to add it — `T-AVAIL-003c` |
| AC-3 | I | `T-WAIT-008` | **Graduation.** Capturing the work on Netflix enters it in the combined list by the ordinary path and its intent leaves the waiting view (`a`). ⚠ The same test asserts a batch-mate's unrelated intent is left `waiting`, without which it passes against a build that satisfies every intent on every close; `c` adds the empty-set case |
| AC-4 | U | `T-WAIT-009` | "Not interested" routes the row's `titleId` to the **same** `suppressTitle` endpoint every other surface uses (`a`), because suppression is keyed on canonical work identity server-side and a waiting-specific client path is a second thing that can disagree with the first. `b` the row leaves on success; `c` a failure keeps it and offers a retry |
| AC-5 | I | `T-WAIT-008` | The satisfied intent is **retained for ever**, with its discovery provenance intact (`b`). ⚠ "It left the waiting view" is satisfied just as well by a `DELETE`, which is invisible in every other assertion — the row is simply gone and every count still adds up |
| AC-6 | E + U | `T-WAIT-010` | The empty view names what it is for and its action is a **reachable link** to `/upload`, not prose (`a`/`b`). `c` keeps "you are waiting on nothing" and "we could not find out what you are waiting on" distinct — they look identical on screen and mean opposite things; `d` announces the loading state; `e` is the discriminating half, a populated view showing the row |
...
| AC-2 | U | `T-UX-103` | Direct metadata children follow year → type → runtime → genres, matching the approved 2026-09-16 layout (`specs/ui.md` §2.2). |
...
| AC-4 | U | `T-UX-127`, `T-UX-141` | Three full-name chips plus `+n` expansion; names wrap without clipping or ellipsis. The limit is a count, not a measured line. |
...
| AC | L | Test | Assertion |
|---|---|---|---|
| AC-1 | U | `T-UX-110`, `T-UX-141` | A single column of horizontal poster/details rows is the narrow base for both preferences; CSS source assertions complement real-browser no-overflow checks. |
| AC-2 | U | `T-UX-111`, `T-UX-141` | Default Grid gains columns through a `min-width` rule; Compact remains one column, with horizontal cards in both. |
| AC-3 | U | **`T-UX-112`**, `T-UX-141` | One row tree preserves identical metadata, badges, server order, actions and pending/offline restrictions; Grid/Compact changes presentation only and survives query changes. |
| AC-4 | U | `T-UX-110`, `T-A11Y-001` | No horizontal scroll at 320 px. |
| AC-5 | U | `T-CSS-004`, `T-UX-142` | Approved dark tokens are pinned and contrast is recomputed on actual surfaces; dark surface foreground on accent fill passes, text is ≥4.5:1 and interactive boundaries ≥3:1. |
...
| AC-2 | U | `T-UX-132`, `T-UX-143` | Phone is List/Upload/More; desktop is List/Upload/Batches/More. Every other nav route remains in overflow; one nav landmark and consistent breakpoint rules remain. |
...
| AC-5 | U | `T-UX-118`, `T-UX-143` | Upload stays first-class; Service updates reveals factual service-preselected links even when dates are unavailable, with degradation visible outside. Show facts, never nag (REQ-039/A46). |
...
| AC-6 | U | `T-UX-120`, `T-UX-138` | Longest runtime is the inactive default; active reverses to Shortest runtime. Labels never describe date order. |
...
| AC-5 | U | `T-UX-127`, `T-UX-141` | Three chips plus `+n` expansion; full genre names wrap without truncation. Active-filter genres are hoisted and the count limit yields to them, so four active genres show four chips. |
...
| AC | L | Test | Assertion |
|---|---|---|---|
| AC-1 | U | `T-UX-113` | Both controls mount inside `.list-controls` on the real page (`a`); changing filters retains direction (`b`). #328 supersedes the separate direction-only session preference: the navigation boundary persists one complete destination, while controls continue to read/write the authoritative URL. |
...
| AC-3 | U | **`T-UX-128`**, `T-UX-146`, `T-UX-138` | Six native complete-order buttons in fixed field order inside the chooser, exactly one `aria-pressed`; the selected accessible name states the current order and the reverse action; each row spells its direction in words beside a decorative arrow (`T-UX-146c`). No separate direction segment. ⚠ **`T-UX-128a`/`d` read the row's `.sort-option__name`, not its text content** — the chooser names the field and the direction separately, so asserting text content against a complete-order label would fail on a correct implementation and invite someone to "fix" the component instead.<br />~~Six **visible** native complete-order buttons in fixed field order, exactly one `aria-pressed`; selected accessible name states current order and reverse action. No separate direction segment.~~ (Superseded 2026-09-17: the orders moved into a chooser, `specs/ui.md` §2.1 item 2 / §10.1.) |
...
| AC-5 | U | **`T-UX-131`**, `T-UX-116`, `T-UX-146` | One interaction from the toolbar (`T-UX-131a`) and one from the default page (`T-UX-116a`), performed on the dedicated **reverse button**, which `T-UX-131b` asserts is visible and operable with the chooser shut and which `T-UX-146h` asserts never opens the chooser. ⚠ This is not pedantry: a collapsed `<select>` of combined options passes a one-`click()` assertion because jsdom does not model the press that opens it — and, since 2026-09-17, so does the chooser. The reverse button is the only control that keeps this AC true, so **`T-UX-131b` is what stops it being deleted as a duplicate of the selected row.**<br />~~One interaction from the control (`T-UX-131a`) and one from the default page (`T-UX-116a`), with the target visible **without opening anything first** (`T-UX-131b`), where the target was the selected order button itself.~~ (Superseded 2026-09-17: same requirement, now carried by the reverse button.) |
...
| AC-7 | U | `T-UX-115`, `T-UI-024o`–`q`, **`T-UX-146i`**, `T-LIB-001` | Remembered complete order reaches the query string before list reads and the mark agrees (`115a`, `T-LIB-001a`). Explicit field links use their own default (`115b`) and explicit direction wins (`115c`). URL leads and persistence follows; choosing a field must not lose it during a competing preference reconciliation (`146i`). #328 intentionally retires the direction-only session fallback that rewrote older history entries. |
...
| AC | L | Test | Assertion |
|---|---|---|---|
| AC-1 | U | `T-CSS-006`, `T-CSS-007`, `T-UX-142` | Closed type scale, minimum primary-content size and Segoe UI/Aptos system stack; no downloaded font. |
| AC-2 | U | `T-UI-030`, `T-A11Y-016`, `T-UX-143` | Closed 16-icon inline set including Brand/Grid/Compact, inherited colour, decorative and named modes. |
...
| AC-5 | U | `T-UI-031`, `T-A11Y-006`, `T-UX-139`, `T-UX-143` | Modal dialog focus trap remains; nonmodal disclosures allow normal Tab, dismiss/restore correctly, and field associations/generated IDs stay accessible. |
...
| `T-INV-014` | Every table has an `owner_id` column, and every index that serves an **owner-scoped query** leads with it (`specs/data-model.md` §16.2). **Two kinds of index are exempt and are enumerated by name in `tools/check-owner-leading-indexes.mjs`:** single-column **foreign-key** indexes, which exist to serve joins and referential-integrity checks rather than owner-scoped reads and are made worse at that job by a leading `owner_id`; and `uploaded_image_retain_until`, which serves the **30-day blob purge** — one of the exactly two permitted non-owner processes (product invariant 5), so it is deliberately not owner-scoped. ~~"Every table has an `owner_id` column and every index leads with it."~~ ⚠ **That form was not true of the schema and never had been**: 12 indexes did not lead with `owner_id`, all of them correctly. It was also unimplementable as a remedy — `T-MIG-001` forbids `DROP INDEX`, so the 12 could not have been rewritten even if the rule had been right. ⚠ The exemption is an **allow-list, not a predicate**: a new index is a violation until it is named, so the narrowing cannot be used to wave through tomorrow's genuinely owner-scoped index. |
...
| `T-INV-016` | A non-empty `title.duplicate_ack_seq` is written in **`routes/fixMatch.ts` and `routes/listings.ts`, and nowhere else** (§16.4). Both are `updateTitle()` calls on a title that already exists: fix-match re-points a title's work identity, and restore brings a listing back under a work identity that already has an active title. In each case the title's own id is used as the sequence value, which is what keeps the filtered unique index `title_one_active_per_work` — keyed on `(owner, work_identity, dup_ack_seq)` — from firing against the existing active row whose `dup_ack_seq` is `''`. ~~"A non-empty `title.duplicate_ack_seq` is written in `createTitleAllowingDuplicate()` and nowhere else (§16.4)."~~ ⚠ **That form named a function that does not exist**, and could not be implemented as written: there is no create-allowing-duplicate path anywhere in the product, because **both** writers update an existing title. Making the code match the rule was checked and rejected — it would have meant renaming a restore path into a "create". ~~"The acknowledged-duplicate `dup:` work-identity prefix is applied in `createTitleAllowingDuplicate()` and nowhere else."~~ ⚠ **That form passed vacuously**: it grepped for a `dup:` prefix that appears nowhere in the codebase, that `WORK_IDENTITY_RE` rejects, and that `title_match_coherent` rejects at the database (all verified). See `docs/task-017-schema-findings.md` §2 D-2. ⚠ **This rule has now been wrong twice** — once vacuous, once unimplementable — which is why the third form is enforced by a checker (`tools/check-duplicate-ack-writers.mjs`) that is itself proven against planted violations rather than only against a clean tree. |
...
| **`T-INV-023`** (new, TASK-017) | The coverage **exclusion list** may not become an untested-code loophole. `apps/api/src/repository/**` is excluded from the §1 thresholds because coverage is bound to CI job 4 (`test:unit`), which has no database, while that code is exercised in full by the `integration` project (§3.3a) — counting it in the unit run would report 0% for well-tested code and drag the floor down for everything else. The risk is what the exclusion permits *later*: once a directory is outside the thresholds, code added to it, or tests deleted from it, still pass every coverage gate, and the exclusion silently changes meaning from "measured elsewhere" to "not measured at all". So `T-INV-023b` fails if any excluded repository module is not imported by an integration spec, and `T-INV-023c` fails if the exclusion list ever grows beyond the two process entrypoints and this one directory. ⚠ **`T-INV-023d` (TASK-127) closes the other half of the same loophole.** `a`–`c` guard the exclusion *list*, but the threshold **numbers** were unguarded: an agent facing a red coverage gate could edit `95` down to `70` and every CI check would stay green, turning the standing rule — *a shortfall is fixed with a test, never with a lowered threshold or a widened exclude* — into a prohibition nothing enforces. That is `T-INFRA-016` restated: a rule no test asserts is a comment. `d` does **not** hardcode the numbers; it parses the §1 table above and the `thresholds` block in `vitest.config.ts` and fails unless they **agree**, so this table stays the single source of truth and lowering a floor requires editing the spec too — a visible, reviewable act. Both sides carry a positive control asserting 3 parsed rows, so a reformat that breaks either parser fails loudly instead of comparing two empty sets. |
...
| **`T-DM-002`** | `workIdentityForTmdb` produces the documented `tmdb:<movie\|tv>:<id>` shape and **throws `RangeError`** on a non-positive or non-integer id rather than minting a malformed identity; `workIdentityForUnmatched` produces exactly 16 lowercase hex characters, is derived from the **normalised** text (so case, spacing and punctuation variants of one tile collapse to one work), injects **no year (SD-05)**, and separates distinct works. Implemented in `packages/domain/test/identity.spec.ts` (TASK-015). ⚠ `T-DM-002g` **pins the digest of `'Dune'`** (`unmatched:ab906be12ca6d9f2`, independently verified against `crypto.createHash('sha256').update('dune')`): the hash, the slice width and the normaliser are jointly load-bearing, because changing any of them silently orphans **every stored unmatched identity and every suppression keyed on one** (product invariant 1). That must break a test before it breaks data. `T-DM-002h` asserts `WORK_IDENTITY_RE` rejects the near-miss forms — uppercase hex, a short digest, a long digest, a `film` media type, id `0`, and leading/trailing whitespace or newline. |
| **`T-DM-020`** | Every owner document type (`title`, `uploadBatch`, `uploadedImage`, `suppression`, `extractionCandidate`, `serviceState`) round-trips through its own schema unchanged **and** is accepted by the `ownerDocument` discriminated union, so a type reachable in code but missing from the union cannot pass. Implemented in `packages/domain/test/schemas.spec.ts`. ⚠ `T-DM-020c` asserts an unexpected key is a **parse error, not a silent strip**: stripping hides a producer/consumer mismatch until it matters, which for this app means a write that quietly discards a field. |
| **`T-DM-021`** | **Suppression is never a title state.** `TITLE_STATES` is exactly `['active', 'removed']`, and the title schema rejects `state: 'suppressed'`. Implemented in `packages/domain/test/schemas.spec.ts`. This is the schema-level enforcement of product invariant 1 / REQ-071 / PRD R-5: suppression is the existence of a `Suppression` row keyed on the canonical **work identity**, so a row-scoped flag would be silently bypassed the moment a title reappears as a brand-new row (`L1`/`A33`), appearing to work and then quietly stopping. |
| **`T-DM-022`** | Match state, work identity and TMDB metadata are **one fact, not three fields**: a `matched` title requires a `tmdb:` identity **and** TMDB metadata; an `unmatched` title round-trips carrying its raw and normalised source text and is **rejected without it** (so an unmatched row can always be shown to the owner and re-matched); and a malformed `workIdentity` is refused at the schema boundary. Implemented in `packages/domain/test/schemas.spec.ts`. |
| **`T-DM-023`** | A title carries **at most one listing per service** — a second Netflix listing is rejected while one listing per service round-trips — and a title with **no** listings is rejected, since its derived state and `sortDateAdded` are undefined (see `T-INV-010`, which asserts the derivation throws on the same shape). Implemented in `packages/domain/test/schemas.spec.ts`. |
| **`T-DM-024`** | TMDB metadata is stored as TMDB reports it: an **empty** `genres` array is valid and means "TMDB carries no genre" (US-019 AC-6 — never defaulted, never filled in), and `posterPath` must be a TMDB **path**, not a fully-qualified image URL, so the base URL and size remain the client's choice. Implemented in `packages/domain/test/schemas.spec.ts`. |
| **`T-DM-025`** | The uploaded-image row records **how the image arrived and what was stored**: all three ingest sources (`paste`, `upload`, `drop`) round-trip and a fourth is rejected (product invariant 16 — paste was *added* to file selection, not swapped in); `fileName` round-trips and may be neither blank, whitespace-only nor over-long; and `uploadedFormat` accepts `heic` while the **stored** `format` does not, because HEIC is transcoded to lossless PNG on ingest (REQ-077, ADR-0008) — a persisted `format: 'heic'` means the transcode was skipped. ⚠ `T-DM-025e`/`f`/`g` assert the **two size ceilings are different units and must not be aliased**: a lossless PNG transcode of a compliant 10 MiB HEIC routinely stores at ~30 MiB, so the stored byte count is bounded by `MAX_STORED_IMAGE_BYTES` (the largest sanctioned raster) while the **uploaded** byte count is bounded by the 10 MiB upload ceiling. Bounding the stored size by the upload ceiling made a legal upload unrepresentable, and nothing would have found it until a real phone photo arrived. Implemented in `packages/domain/test/schemas.spec.ts`; see §29.3(b). |
| **`T-DM-026`** | Batch invariants: `completedAt` is set **if and only if** the batch is `applied` (neither an applied batch with no completion time nor a draft carrying one is representable), and `provenance` round-trips its `created`/`modified`/`removed` arrays with the **pre-batch** value of every modified attribute intact. Implemented in `packages/domain/test/schemas.spec.ts`. The `before` values are load-bearing beyond undo: REQ-075's refusal enumeration reads straight out of these arrays (REQ-068 / US-031 AC-6), so a `before` lost in the round trip becomes an undo that cannot explain itself. |
| **`T-DM-027`** | Extraction candidates: an **empty `rawText` is allowed only for an unreadable tile** (`cleanupVerdict: 'unreadable-tile'`, with no inferred title, identity or classification) and is otherwise rejected — REQ-012 drops no candidate, so an unnameable tile must still become a reviewable candidate rather than vanishing; a candidate may cite **more than one** source image after overlap collapse; and a candidate citing **no** source image is rejected, since a candidate with no provenance cannot be shown against the screenshot it came from. Implemented in `packages/domain/test/schemas.spec.ts`. |
...
| **`T-UI-025`** | C | `ReviewPage` renders a **"Confirm all N" control** per confirmable section (SD-11a, `specs/ui.md` §5.4), where **N is the number of still-pending candidates**, not the section's size; the control **disappears** once nothing is left to decide, and it is **absent from "Already on your list"** even though the API accepts that section — `T-REV-016` prohibits addition/discard controls there, with deliberate identity correction covered by `T-UX-162g`. US-012 AC-2 and `T-REV-011` assert the confirm grammar at the API; nothing asserted the one-tap control that makes a 200-title first import something other than 200 taps. ⚠ A bulk confirm must **skip already-decided rows**: overwriting a `discarded` row would silently undo a decision the owner had already made. |
| **`T-UNM-013` (new, TASK-200)** | C | The **§5.3 per-card controls exist on the ADDITIONS section** — Confirm, Change match, Discard on **every** addition card, not just the bulk "Confirm all N" of `T-UI-025`. `a` all three render on **each** card (per card, not once per section — asserting one control somewhere in the section passes over the exact shape that was broken); `b` discarding one addition patches **only** that candidate and leaves its neighbour still decidable; `c` the labels are the addition words, not the unmatched ones; `d` confirming sends the §6.18 confirm and the card says so; `e` a **refused** patch leaves the card pending and says nothing changed; `f` an already-decided card reports the decision instead of offering it again. ⚠ **This was found in use, not in review.** A real Netflix upload read the two-line `SOL LEVANTE` treatment as both `SOL LEVANTE` and a fragment `LEVANTE`; nine good rows could not be accepted without also accepting the fragment, because `ReviewPage` passed `renderCard` only for `unmatched`. `CandidateCard` had accepted an `actions` node all along and `PATCH …/candidates/:id` already took `discarded`, so **every component-level test passed throughout the entire period the affordance did not exist** — these cases mount the page. ⚠ **`TASK-199` does not subsume this:** extraction accuracy lowers how *often* a false extra appears and can never make it never appear. ⚠ **The keep label is not shared with `T-UNM-010`** — an addition already carries a resolved match, so "Keep as unidentified" would tell the owner a correctly identified title was unidentified. |
| **`T-UI-026`** | C | The candidate list is **virtualised above 100 items in a section and rendered plainly at or below it** (SD-11c). Both directions are asserted, because each fails silently in the opposite environment: virtualising unconditionally renders a subset of a nine-item section wherever layout is not computed, and never virtualising is invisible until the 500-candidate batch `T-PERF-002` measures. `T-PERF-002` is an `E` test on a real browser and cannot distinguish "fast because windowed" from "fast because small". |
| **`T-UI-027`** | C | Dispositions are **persisted to `sessionStorage` under `nextup.review.<batchId>`** and re-read on mount (SD-11e), so a refresh mid-review does not lose the pass — **but the server wins**: a cached value applies only over a candidate the server still reports `pending`. Asserted with the key shape, a malformed payload degrading to an empty cache rather than throwing, and a throwing storage (Safari private mode) leaving the screen renderable. ⚠ `localStorage` would outlive the batch, the tab and the sign-in, and replay a stale decision over a later batch. |
...
| **`T-BOUND-001` (new, TASK-181)** | C | A **render crash never becomes a blank page** (`ux-states.md` §1). A 200 of the wrong shape throws mid-render, React unmounts the tree, and the owner is left on white with no message, no nav and nothing to retry. `a` a throwing screen renders `BOUNDARY_TITLE`/`BOUNDARY_BODY` in a `role="alert"`; `b` the body states nothing saved was changed (§1's ASM-029 rule — a crash is exactly when the owner must guess whether their action half-applied); `c` the **shell survives**, so every other route stays one tap away, which is why the boundary wraps the `<Outlet />` and not the root; `d` navigating away **clears** it — without a reset key React latches the message over a route that is fine; `e` **Try again** recovers a transient failure without a page load; `f` a healthy screen is untouched, the discriminator that `a`–`e` all pass without. ⚠ **This shipped three times** — `/batches/:id`, the review route, and the all-routes a11y sweep — each time mistaken for a bad test stub and corrected in the stub. `T-A11Y-001c` and `expectStyledAndRendered` were green throughout, because `role="main"` exists for the instant before the throw propagates, so no sweep can be strengthened into this assertion. |
| **`T-UX-030`** | C | The `⋮` row menu (`specs/ui.md` §2.2/§2.3) **reaches** the suppress-confirm state (`ux-states.md` §3.1) and the fix-match dialog, driven from `ListPage` and never from a component in isolation. **`a`** the `⋮` opens a menu naming both actions; **`b`** "Not interested" opens the §3.1 confirm step and suppresses nothing yet; **`c`** confirming calls the server and hides that row only; **`d`** "Fix match" opens a dialog whose search reaches TMDB; **`e`** an unmatched row goes straight to fix match with no menu step; **`f`** cancelling opens nothing and changes nothing; **`g`** the menu acts on the row it was opened from, not the first row; **`h`** no dead affordance is offered when the container wired no handlers; **`i`** the fix-match write lands on the row the menu was opened from, with the exact request body; **`j`** a row is hidden only once the **server has confirmed**, asserted **while the request is in flight** — an end-state assertion cannot distinguish this from an optimistic hide, because the row is restored when the error arrives. ⚠ **Every case must mount the page, not the dialog.** `T-UI-020` and `T-FIX-010` both passed for months while `FixMatchDialog` was imported only by its own test and `apiClient.fixMatch` did not exist: a component test that mounts the component under test can never discover that nothing mounts it, and `T-API-010` is one-directional by design, so a missing client method is asserted by nobody. |
...
| **`T-AI-023`** | I | `POST /api/batches/:batchId/manual-entry` (§6.20) creates an owner-supplied candidate on an **open** batch — the manual-entry fallback for the artwork-only tile that carries no readable text — returns it in the review response, and rejects entry on a closed batch. Also asserts the two 409s (`WORK_SUPPRESSED` on the identity, `ALREADY_IN_BATCH`), that a **discarded** candidate for the same work does NOT block the entry (that is the ordinary path through a mis-read tile), and that a TMDB miss or outage writes **nothing**. `T-UNM-010` covers actions on an *unmatched* candidate, not creation of a new one, and `T-AI-041` covers only rendering the untitled tile. ~~`POST /api/batches/:batchId/candidates` …~~ ⚠ **The path in the struck-through text never existed.** This row and `docs/backlog.md` named `…/candidates`; `specs/api.md` §6.20 — which is authoritative for the API surface — names `…/manual-entry`, and neither route was implemented, so nothing failed. |
| **`T-REV-018` (new, TASK-067)** | U | `parseManualEntry` (`packages/domain/src/candidatePatch.ts`) — the pure half of the §6.20 body. Accepts `{ tmdbId, mediaType }` for both media types; refuses a non-positive or non-integer `tmdbId`; refuses `mediaType: 'person'` **specifically**, because TMDB's multi-search returns people and a client that passed one through would add a human being to the watchlist as though it were a work; refuses a client-supplied `disposition` (a manual entry is `confirmed` by definition, so honouring `discarded` would write a row the owner cannot act on and report it back as their own decision); and refuses a client-supplied `name`, because the name is read from TMDB at the route and SD-05 keeps text out of identity. |
| **`T-UI-028` (new, TASK-067)** | C | `ManualEntryPanel` — search TMDB, then add the chosen work **on an explicit press**. ⚠ **This replaces `T-UI-012` in TASK-067's row, which was already green and owned by another task** (service deep links, `titleRow.spec.tsx`): building the manual-entry UI under it would have marked the task done against tests that never touched it. Asserts the panel renders and says what it is for; that searching alone adds nothing (REQ-014); that the id is what gets sent (SD-05); that `WORK_SUPPRESSED` and `ALREADY_IN_BATCH` each get their **own** message rather than a generic failure; that an empty result set is stated rather than left blank; and that a failed search **clears stale hits**, since results left under a failure message can still be added from a search the owner is no longer looking at. Two further cases assert the **mount** on `ReviewPage` — a component nobody renders is the TASK-076 defect in miniature — including the negative: a half-wired page (search with no add) renders no panel at all. |
| **`T-BATCH-017` (new, TASK-076)** | U | `GET /api/batches/:batchId` (§6.15) **exists, is registered and answers**, and carries the fields the status page cannot work without. ⚠ **This route was absent for the whole of TASK-059's life while the SPA polled it every two seconds** — `apiClient.getBatch` and `BatchStatusRoute` were correct, `T-UX-007`/`T-UX-008` asserted the render against a stub, and the real request 404'd, so US-006 AC-1 could never complete. `a` an unknown batch is a **404, not a 500**; `b` every image carries an `/api/images/...` href and an availability **derived** from `retainUntil` by the same predicate the byte route uses, with `blobPath` absent (`T-SEC-003`); `c` `progress` is present while `submitted`/`extracting` and **absent once applied**, from the *same* stored stats — a finished batch reporting progress is US-006 AC-1 read backwards; `d` the extraction error **code** is returned and the server's message is not, so one failure does not get two voices; `e` `degradedExtraction` and `crossCheck` are carried, which is the gap `apiClient.ts` had recorded and which leaves `T-UX-008`'s banner underivable without them. |
| **`T-BATCH-018` (new, TASK-076)** | U | The batch-detail **predicates**, asserted directly rather than through a response. `readProgress` returns **`null`, never `{0,0}`**, for stats that were never written — a zeroed pair renders as *"0 of 0 screenshots read"*, which is a claim, where the absence renders as nothing, which is the truth; it also rejects a non-numeric pair and survives unparseable JSON without throwing, because the status page is what an owner opens when something has already gone wrong. `a` `changedNothing` is **false when only a modification happened** — §9.5's sentence must not be told over a batch that corrected a match. `b` `provenanceTitleIds` de-duplicates across all three arrays, since a title created *and* modified by one batch is one title. `c` only `submitted` and `extracting` are in flight — `in-review` is not, and a progress bar there would be a lie. |
| **`T-API-010` (new, TASK-076)** | S | **Every `/api/...` path the SPA client calls is a route the API registers**, compared mechanically between `apps/web/src/lib/apiClient.ts`'s own source and the live Express router. ⚠ This exists because the product shipped a poll against a 404 for months and **both suites stayed green**: the web tests stub the client so the request never leaves the process, and the API tests assert routes that exist, so an **absent** route is asserted by nobody. A hand-kept list would not have helped — it would be maintained by the same person who forgot the route. `a` carries a **non-vacuity guard on both sides**, since a refactor that broke either extraction would otherwise compare two empty sets and pass; `b` is the parser's negative control (interpolation must normalise the way Express does, or every parameterised path reads as "missing" and the assertion gets deleted instead of the bug); `c` mutation-proves the comparison reports an unregistered path; `d` names the batch route explicitly, because the sweep is only ever as good as its extractor. **Only client → server is asserted**: `GET /api/images/:imageId` is reached from an `<img>` href and never through the client, so a bidirectional check would fail on correct code and be silenced. |
...
| **`T-META-001` (now implemented)** | S | **Every `US-nnn AC-n` in `docs/PRD.md` is mapped in §9, and every test id §9 cites really names a test.** `tests/meta/acCoverage.spec.ts`, cases `a`–`g`. ⚠ **This document has asserted since §1 that this gate was running — "the AC mapping in §9 is the real gate" — and named it again in §3, §10 and the §9 US-039 row. It did not exist**, in any form: the one gate NFR-003 depends on was itself the sixth disguise, an id living only in a spec. On its first run it found **43 unmapped criteria** in the two shapes the prose promised to prevent: two whole epics (US-040…US-046, the rental and IMDb-rating stories, 42 criteria) whose §9 sections were never written, and **US-028 AC-7** — a single criterion silently dropped from a story that otherwise looked complete, on the load-bearing suppression-migration invariant, whose test `T-FIX-005` was catalogued under US-030 with `—` in the AC column. Both halves of that mapping existed and had never been joined. It also found **14 cited ids that name no test**. ⚠ Two ratchets, both shrink-only and both failing in BOTH directions (`KNOWN_UNMAPPED`, `KNOWN_PHANTOM_CITATIONS`), so closing a gap forces its line out. ⚠ **`e` shipped vacuous on the first run and `g` is why it does not now:** the baseline lists phantom ids as string literals, the scan reads every collected spec including this gate's own source, so each literal was counted as its own implementation and the phantom set collapsed to empty. The fix was the **predicate, not an exclusion list** — an id counts only in `it`/`test`/`describe` **title position**, which `T-META-004` guarantees is where every real id sits. Excluding the file instead was tried and is strictly worse: it blinds the gate to its own tests, and did — `T-META-001` reported *itself* as a phantom. | §1, §3, §10 prose |
| **`T-META-002` (now implemented)** | S | **Every invariant in the §6 non-negotiable core resolves to a test that exists.** `tests/meta/invariantCoverage.spec.ts`, cases `a`–`f`. Scope is **every numbered row of the §6 table** (sixteen today) plus the four criteria §9 names outside it — twenty invariants, not the nine the US-039 AC-2 row used to claim. ⚠ **§6 is the table of properties "whose failure is **silent**", so until this gate existed that table was itself free to fail silently** — a row could name a test nobody had written and the only signal would be the absence of a failure. On its first run it found one: **§6 row 10**, *"the mixed-undo refusal enumerates everything, never truncated"* (US-033 AC-5), cites `T-UNDO-006` and no such test existed; `apps/api/src/services/batchUndo.ts` confirms it from the other side, shipping `truncated: false` with an empty payload pending TASK-114. ⚠ **The scope stated in §9 was stale and building to it literally would have produced the wrong gate** — see the US-039 AC-2 row. The gate therefore **reads the table** rather than a hard-coded count, and `T-META-002b` pins a floor plus both end rows so a parser that silently truncates fails rather than passes. ⚠ `a` is a shrink-only ratchet failing in BOTH directions, and `f` additionally requires each baselined entry to still name a real §6 row, so a renumbered row cannot leave a dead excuse behind. Ids count only in **title position**, the same predicate as `T-META-001`: under bare occurrence `T-UNDO-006` is "implemented" by the gate's own comment about it. | §6, §9 US-039 AC-2 |
| **`T-AI-009`** | U | The Read extractor requests `features=['Read']` only, and the extractor result type carries **no service field** — `'netflix'`/`'max'` appear nowhere under `extraction/` (`specs/ai.md`). Service attribution is the owner's, declared on the batch; an extractor that guessed it could relabel a title onto a service the owner never uploaded. <br> **`g`/`h` (new, TASK-057) — the ONE exemption, and the property that buys it.** `specs/ai.md` §3.2 step 3 lists the chrome vocabulary verbatim and it **includes `hbo max`, `max` and `netflix`**, because those words are printed on the screenshots: an OCR orphan of a Netflix page header reads exactly `NETFLIX`, and dropping the terms turns it into a `title-candidate` — a false title, the rate `T-AI-030` measures. That is a real conflict with RULE B, and it is resolved by **exempting `packages/domain/src/extraction/chromeTerms.ts` by path — never by moving the file out of the scanner's reach**, which would defeat the ban rather than answer it. RULE B bans the READER being told which service it is looking at; a fixed vocabulary of on-screen words prompts nothing and branches on nothing. `g` makes that verifiable rather than asserted: the exempt file **imports nothing**, and a module with no imports has no reader, no request and no batch in scope, so it cannot be made service-conditional without first failing this case. `h` proves the import scan catches a planted import. `d` additionally asserts the exempt path still exists, so a rename cannot make the exemption vacuous. <br> **`s` — the two §9.7 bake-off escape hatches, and the first test either has ever had.** `LlmVisionExtractorOptions.tokenParam` and `.temperature` both carry the instruction *"production must not set this"*, and until `s` that instruction was a comment. `tokenParam` shipped with the bake-off harness and had **no test of any kind**; `temperature` joins it so the Stage 0 `temperature: 0` disqualifier can be *measured* rather than only argued about — every model newer than `gpt-5.4` rejects `temperature: 0`, so that rule currently excludes every future frontier candidate, and a rule that cannot be tested cannot be justified. Both hatches change §2.1a's contract, which §9.5's whole golden apparatus rests on. `T-AI-009k`'s default-value pin is **not** sufficient: it stays green while a production caller passes an override explicitly, which is the only way the hatch can do harm. So `s` asserts both halves — an override really reaches the request body (a seam that silently did nothing would let an investigation file incumbent numbers under another arm's name and "prove" whatever was already believed), and no file under `apps/api/src/**` sets either key. ⚠ **The static half was itself fixed by mutation.** Its first form anchored the key to a line start, which finds it only in a multi-line object literal — and `configFromEnv.ts` builds its config on **one** line, so the exact call the gate existed to stop passed straight through it. It now strips comments and matches unanchored. | `specs/ai.md` |
...
| **`T-AI-045` (TASK-168)** | S/U | The **primary-reader bake-off protocol** in `specs/ai.md` §9.7: `a` arms differ only in deployment name, with the same prompt, schema, `detail`, `max_completion_tokens`, `temperature` and `seed`; `b` shared expected answers and OCR; `c` model-scoped recordings cannot overwrite the incumbent's evidence. `d` the pure, total decision function retains the incumbent on a mixed, equivalent or inconclusive result; **cost is reported, never decisive**, confirmed by the owner on 2026-09-17. A challenger must meet every floor, be no worse on every compared quality metric, and show a measurable improvement. `e` a sub-two-title delta on the 11-image corpus is **"no measured difference"**, not a win. `f` neither recorder nor bake-off runs in a workflow. <br> **`g` (`x`, `y`, `z`, `aa`, new) — the Stage 0 / Stage 3 tables match §9.7 AS REVISED ON 2026-09-19, and this claim exists because for two days they did not.** §9.7 moved `temperature: 0` out of Stage 0 into a Stage 2 *measurement* and added an **L3 unstable-titles** row to Stage 3 to carry the guarantee the parameter gate had stood proxy for. `chooseReader.ts` was not updated until 2026-09-21 — while its own header claimed to transcribe §9.7 "row for row". ⚠ **THE SUITE WAS FULLY GREEN THE ENTIRE TIME, AND THAT IS THE POINT OF THESE FOUR.** It pinned the stale values: `T-AI-045k` asserted `['stabilityJaccard', 0.94]` as a *breach*, which is only a breach against a 0.95 floor **that §9.5 has never contained**. The tests were not merely blind to the drift, they actively certified it — the F-001 defect class §5 exists to prevent, reproduced inside the gate meant to catch it. Every assertion below is therefore written against the **spec's** number, and three use the **real measured figures** so a silent revert fails on evidence rather than on taste. `x` a challenger reporting `temperatureZero: false` and otherwise passing is **not** disqualified, and `REQUIRED_CAPABILITIES` does not contain the key — the stale gate refused `gpt-6-astra` (which accepts only `temperature: 1`) for a rule the owner had deleted, spending zero images on a candidate that meets every quality floor and is the *most stable* arm measured. §9.7's own words: *"A gate cannot be justified by a property its own subject fails and its excluded candidates satisfy."* `y` the field **survives on `ReaderCapabilities` as reported-not-required** — deletion is the tidy-looking wrong fix, because which arm was sampling is what makes the Stage 2 stability numbers interpretable at all; demotion is not removal. `z` ⚠ **the L2 floor is 0.75 and the case that proves it runs the INCUMBENT AGAINST ITSELF at its real measured 0.7692** — at the stale 0.95 `chooseReader` returned `floor-breach`, i.e. it disqualified `gpt-4.1` for being `gpt-4.1`, so that row could only ever produce one answer and protected nothing; `z` also pins the floor still biting at 0.74, so the fix is a re-base and not a removal. `aa` ⚠ **L3 binds on BOTH halves — ≤ 0.05 absolute AND ≤ the incumbent — and L2 alone is the wrong landing place, which is exactly why §9.7 added this row beside the Stage 0 move.** L2 counts false-title churn, i.e. noise, so a challenger that **drops titles the owner really saved** can post a *better* Jaccard; `aa` hands it a perfect 1.0 stability with a worse unstable-title rate and requires `incumbent-stays`. **All three repairs are mutation-proven and each is caught by exactly the case written for it:** restoring `temperatureZero` to `REQUIRED_CAPABILITIES` fails `x` alone; reverting the floor to 0.95 fails `z` alone; deleting the L3 row fails `aa` **and** `k` (`k` now carries the L3 breach too, so the absolute floor cannot vanish unnoticed). `y` is additionally guarded by `tsc` — removing the field is a type error at every fixture. | `specs/ai.md` §9.7 |
...
| **`T-AI-051` (new, TASK-079b)** | S | **The §4A live quality suite (`npm run golden:live`, `tests/extraction/goldenLive.spec.ts`) — MANUAL, and it spends real money.** Three live runs over the eleven golden images against the live `gpt-4.1` deployment and the live Vision endpoint, asserting the §4A bands and nothing else: `a` L1 per-image recall ≥ that image’s `minRecall` in 3 of 3; `b` L2 pairwise Jaccard of the normalised accepted-title sets ≥ 0.75 (re-based from 0.95 on 2026-09-19 — see `k`); `c` L3 unstable titles ≤ 5 % of expected, **each printed by name** in both the report and the failure message; `d` L4 fabrication rate ≤ 0.05 per run; `e` L5 false-title rate ≤ 0.10 per run; `f` L6 artwork-only recall ≥ 0.80 in 3 of 3; `g` L7 whole-run cost ≤ $0.50 from reported token usage. ⚠ **`i` IS ASSERTED FIRST AND EVERY OTHER NUMBER DEPENDS ON IT.** A run that reached no provider scores recall 0 — but *also* false-title 0 and fabrication 0, so `d` and `e` **pass**. Two bands agreeing on an empty run is not evidence, it is the shape of a broken harness, so `i` requires one LLM call and one OCR call per image, non-zero prompt tokens and a non-empty accepted set before anything else is believed. ⚠ **`h` EXISTS BECAUSE THE REPORT IS WRITTEN BEFORE ANY BAND IS JUDGED.** A run that fails a band is the run whose numbers matter most; a report emitted only on success would discard exactly those, and a drop between two reports is the only early warning of model drift this product has. ⚠ **`j` KEEPS `MIN_RECALL` HONEST.** The floors are hand-declared bands, not the offline measurements — the reader is sampled, so pinning them would fail on the model’s own variance and teach the owner to ignore the suite. `j` replays the committed recordings (free) and fails if any floor exceeds what they already achieve, so the table cannot drift into aspiration and turn L1 red on a good day; it also asserts the table covers exactly the corpus, and that at least eight floors are above zero so a table of zeroes cannot pass vacuously. ⚠ **`netflix-continue-watching-01`’s floor is 0 ON PURPOSE** — `T-AI-030b` records `found: 0` for its single expected title, and a floor above a known offline shortfall would fail every live run for something that is not drift. ⚠ **THE SUITE IS A VITEST PROJECT (`live`) SO THAT IT IS NOT INVISIBLE, NOT SO THAT IT RUNS.** `T-CI-008` fails a spec no runner collects, and a silently dead `goldenLive.spec.ts` is this product’s model-drift alarm disconnected while every gate stays green. `T-CI-007l` therefore asserts the property that actually matters — that `golden:live` is the **only** script selecting the `live` project, that no script reaches it transitively through `npm run`, and that no script runs a bare `vitest run` with no `--project` (which would collect every project, including this one). ⚠ **L1's MISSES AND L5's FALSE TITLES ARE PRINTED BY NAME, for the reason L3 already was.** A ratio says the reader wobbled but not on **what**, and the name is the only part anyone can act on. The first real run made the cost concrete: `netflix-mylist-desktop-01` sat at 0.800 against a 0.900 floor **identically in all three runs** — a reproducible shortfall, not sampling noise — and the report could not say which titles were lost. Named, the same run showed that most of the false-title mass is three deterministic patterns (a subtitle variant duplicating an accepted base title, a truncated prefix duplicating an accepted full title, and shelf-header chrome), each of which is a §3.2 rule someone can write. ⚠ **THE SUITE CAN NOW MEASURE A NON-PRODUCTION ARM, AND THAT CAPABILITY CAME WITH A LATENT WAY TO DESTROY THE BASELINE.** `NEXTUP_AOAI_DEPLOYMENT` was always honoured, so pointing the suite at another deployment was already possible — but the report was still written to `golden-<date>.md` and still headed `gpt-4.1`, so one investigation run would have **overwritten the committed baseline with another model's numbers under the incumbent's name**. That file is the one thing §4A exists to preserve: the drop between two reports is the only early warning of model drift this product has, and it is readable only while every report describes the arm that produced it. The arm is therefore named — deployment, `temperature` and token parameter — the name is stamped into both the header and the filename, a non-default arm gets a `golden-<date>-<slug>.md` path it cannot collide on, and the report carries a banner saying its band failures are facts about that arm rather than regressions. ⚠ **The slug keys on temperature as well as deployment**, because the question the seam was built for is asked by running the *same* deployment at two temperatures, and a deployment-only slug would file both answers under one name and lose the comparison. ⚠ **`k` IS THE L2 ANALOGUE OF `j`, AND L2 IS WHERE THE BAND TABLE ACTUALLY WENT WRONG.** `j` has always kept `MIN_RECALL` at or below what the committed recordings achieve; L2 had no such check and sat at **0.95, a value nothing has ever reached — production included**, whose measured worst pair is 0.7692 (`gpt-5.4` 0.8205, `gpt-6-astra` 0.8750). It looks calibrated against the *offline* replay, where the recordings are fixed and Jaccard is 1.0 by construction, then applied to a *live* run that resamples every time — the exact mistake §4A already documents for `minRecall`. The cost was not cosmetic: a band that is always red cannot tell a healthy run from a regression, it teaches the owner to skip the failure, and through `specs/ai.md` §9.7 Stage 3 it silently blocked **every** model change by a floor the incumbent itself fails. The floor is re-based to **0.75**, just under the incumbent's measured worst pair on the same "next band below" convention `MIN_RECALL` uses, and `k` parses the committed baseline's `## L2` section and fails if the floor ever exceeds the worst pair recorded there. ⚠ **`k` READS A LIVE REPORT, NEVER THE OFFLINE RECORDINGS** — replay is deterministic, so its Jaccard is 1.0 and would "justify" any floor whatsoever, which is the mistake being undone. ⚠ **It asserts exactly three pairs first**, because an empty match list would make `Math.min` return `Infinity` and pass for any floor at all. ⚠ **The report had no L2 section until 2026-09-19**, which is how a permanently-red band went unexamined for so long: the numbers lived only in `b`'s failure message, visible when the band broke and invisible when it passed. L2 is now written to the report next to L3 — read together, L3 empty with L2 below 1.0 means the churn is **entirely in false titles and never in the titles the owner sees**. Reports predating that section make `k` fail by design; the fix is to re-run the incumbent, which by `h` writes the report before any band is judged, **not** to hand-type pair labels into the old evidence file. | §4A, `specs/ai.md` §9.5, TASK-079b |
| **`T-CI-007`** | I | An outbound-blocking proxy observes **zero** requests during the CI test run. Egress in CI means a test is really an integration test against someone else's service: it fails on their outage, passes on their cached response, and quietly breaks the offline guarantee `T-AI-033` depends on.  **`o`–`r` (added with TASK-056): the guard hooks `fetch` AND `http.request`/`https.request`, and the two are not equivalent. `msw` mocks `fetch` by REPLACING it, so a mocked fetch never reaches the guard at all — which is why the TMDB suite needed nothing here. It cannot do that for `http.request`: to hand back a `ClientRequest` it must call the real one, having swapped the socket underneath, so the guard sees a request to a public hostname that no packet will ever leave for. The Azure SDKs speak `https.request` (`@azure/core-rest-pipeline`), so without a seam a fully-recorded, offline extractor suite is indistinguishable from a live one — `T-AI-033` could not exist. `registerMockedHost()` records such an attempt as `mocked`, never as `blocked`. The seam is only safe while it is narrow, so all three properties are asserted: `o` it works, `p` deregistration restores blocking (a registration that outlived its `msw` server would wave a REAL call through, silently, for the rest of the run), and `r` nothing outside `tests/fixtures/msw/**` calls it.** ⚠ **`s` GENERALISES `l` TO THE BILLABLE ENTRY POINTS THAT ARE NOT VITEST PROJECTS.** `l`'s entire mechanism is "which script selects `--project live`", so it is structurally blind to `golden:record` and `probe:stage0` — both plain `node tools/*.mjs`, both calling Azure OpenAI directly, both costing real money, and until `s` neither was stopped from appearing in a workflow. Committing `tools/stage0-probe.mjs` for the §9.7 Stage 0 gate is what made the gap concrete: the fix for "the first Stage 0 was ad hoc and left no artefact" put a third billable entry point one `run:` line away from CI. `s` asserts both halves — transitively unreachable from every CI entry script (`npm run test` is a chain, and a billable script three links down is invisible to a direct grep), and named in no workflow file under **either** spelling, since a gate that looked only for `npm run probe:stage0` would wave `node tools/stage0-probe.mjs` straight through. The failure this prevents is silent in the worst way: a workflow that burns quota produces a **green** run and a bill nobody reads until the budget alert fires. | §2 prose |
...
| **`T-SEC-034` (new, TASK-142)** | S | The **production audit gate suppresses by named exception, never by blanket**. `tools/check-audit.mjs` collects only **high and critical** advisories, ignores bare-string `via` edges (they are dependency paths, not advisories — inventing an id from one makes the gate permanently and unfixably red), fails on **any advisory not explicitly listed**, and — the half that matters — fails on **any listed exception that is no longer reported**. An allow-list that only suppresses is a permanent hole that outlives its reason; this one deletes itself the moment upstream ships a fix. `d` requires every entry to carry an id, a date and a justification over 200 characters, because the point of the list is the reasoning, not the silence. `f` asserts CI actually **calls** the gate and has not reverted to a bare `npm audit --omit=dev`, and `g` that the non-blocking full-tree report survives so dev-tooling findings stay visible. ⚠ Accepted exception `GHSA-ggr8-5vv4-36mx`: npm reports its "fix" as `prisma@6.12.0`, which is a **downgrade** from the installed `6.19.3` presented in the same field as an upgrade. **`h`–`i` added after the gate misfired in CI:** `h` an audit that **did not run** is rejected rather than read as a clean tree — npm's quick-audit endpoint returned `400` while being retired, `npm audit --json` still printed parseable JSON carrying an `error` and **no `vulnerabilities` key**, and the empty result made every exception look stale, so the gate told the reader to **delete reviewed security reasoning** because "upstream has fixed it"; the same empty shape would equally have waved through an unfixed critical, which is why a transport failure must never be read as a statement about the tree. `i` a transient outage is **retried** and a persistent one still **fails closed** — a gate that goes red on somebody else's outage is a gate that gets switched off, but passing after the last attempt would call an unchecked production tree green. | TASK-142, `NFR-004` |
...
| **`T-INFRA-012` (new, TASK-157)** | S | The **memory-observability alerts exist, only ever send email, and still match the log lines the app writes** (`A43-M5`, `infra/alerts.bicep`). `a`–`c`: all three rules are present — one `Microsoft.Insights/scheduledQueryRules` (`decode-abandoned`, the PRIMARY signal) and exactly two `Microsoft.Insights/metricAlerts` (`replica-restart`, `memory-pressure`). `d`: the action group carries **email receivers only** — an `automationRunbookReceivers`, `webhookReceivers`, `logicAppReceivers` or `azureFunctionReceivers` entry turns an alert into an actuator, which `REQ-028` forbids and which deploys perfectly cleanly. `e`: an **unknown** receiver collection fails rather than being silently permitted, so a new Azure receiver type cannot arrive as an allowed default. `f`: every rule description names **`docs/runbooks/scale-up-memory.md`**, because a notification that says "memory is high" without saying what to do is a nag, and `A43-M4` exists so the owner is not working it out during an incident. `g`: the compiled query contains the **exact** `image.decode.begin` / `image.decode.end` literals exported from `packages/domain/src/logEvents.ts` — ⚠ this is the quiet failure the whole rule exists around: rename the constant, leave the KQL alone, and the app still logs, the rule still runs, the deployment is still clean, and the query simply never matches again with nothing to tell anybody. `h`: **no `${...}` survives into the compiled template** — a Bicep multi-line string is VERBATIM and does not interpolate, so `'''…${x}…'''` compiles clean, deploys clean and watches nothing. `i`: no prohibited automation resource type appears. | TASK-157, `A43-M5`, `REQ-028` |
...
| **`T-INFRA-013` (new)** | S | **Nothing finished is left unreachable from the SPA.** ⚠ This gate exists because the identical defect shipped **three times** and was found by hand each time: `GET /api/batches/:batchId` (route built, client method absent — TASK-076); `FixMatchDialog` (component finished and fully tested, imported by nothing but its own test, so US-030 AC-1 `must` had no reachable implementation); and `apiClient.fixMatch` (did not exist at all while `T-FIX-010` and `T-UI-020` both stayed green). **No existing gate can see this class and none is at fault:** `T-API-010` closes client→server parity **one-directionally by design** (`GET /api/images/:imageId` is reached from an `<img>` href with no client method, so a bidirectional sweep would fail on correct code); a component test that mounts the component under test can never discover that nothing mounts it; an a11y or tap-target sweep counts an inert button as happily as a working one; and coverage counts a line reached from a *test* as covered. `a`: the method list is read from source, not assumed — a floor and two known members, so a reformat that breaks the regex fails loudly instead of letting `b` pass vacuously over an empty list. `b`: every client method is referenced somewhere under `apps/web/src/**`, against a **ratcheted baseline** of genuinely-unreached methods (`getTitle`, `removeBatchImage`; ~~`restoreListing`~~ **discharged** — the gate was right and the first diagnosis was wrong: TASK-099's restore control *was* built, but `routes.tsx` mounted `RemovedPage` bare with no props, so nothing could reach it; `RemovedRoute` now wires it), each a separately-tracked gap — the gate's job is to stop a fourth. `c`: a baseline entry that has since been wired must **leave** the baseline, or the ratchet loosens by one every time a gap is closed. `d`: every module under `components/` and `pages/` is imported by another `src` module, with **no baseline at all** — this property was RED before the row menu was wired, and an unmounted screen is a feature the owner cannot reach. ~~⚠ **Both matchers are deliberately loose** (any reference counts, not just a call): a reachability gate that produces one false positive gets a baseline entry added to silence it and then protects nothing. It is a smoke alarm, not a proof of liveness.~~ **Superseded for matcher `b` only.** Its looseness cost a shipped dead feature: `undoBatch`/`undoRemovalGroup` appeared solely in the PROP DECLARATIONS of `ListPage` and `BatchAppliedNotice`, which satisfied a bare-identifier match while nothing ever called them, so `BatchAppliedNotice` — US-017 AC-1, `must` — never rendered once. `b` and `c` now require an actual **call form** `` `\.method\s*\(` ``. ⚠ **The tightening was verified against the real defect, not assumed:** with `ListRoute`'s `client.undoBatch(batchId)` replaced by a no-op, the old matcher stayed GREEN and the new one reports `['undoBatch']`. Matcher `d` stays loose, where a false positive is still the worse failure. `e`: **the gate's own regression test** — a prop declaration, a destructuring binding, a `on`-prefixed JSX attribute and a type member are each asserted NOT to match, and four call spellings (including one split across lines) asserted to match, so the matcher cannot quietly loosen again with every other case in the file still green. ⚠ **The generalisable lesson survives the fix: a static gate proves a CALL EXISTS IN A FILE, never that a CHAIN IS CONNECTED at runtime** — only `T-DATA-011`, which drives the real containers together, could see the defect. | TASK-102, `T-API-010` |
| **`T-INFRA-014` (new)** | S | **No spec may name a source path whose file demonstrably lives somewhere else.** ⚠ This is the fourth proven disguise of this repo's dominant defect class — not a wrong assertion, but an instruction that points at a file which does not exist, so an agent following it literally builds a **duplicate** that passes its own tests. It shipped: `specs/ai.md` §2.1c named ~~`apps/api/src/extraction/crossCheck.ts`~~ for a module that lives in `packages/domain`, and the gate found **six more** on its first run across four documents. Most damning, `specs/testing.md` itself had already flagged the `pixelGuard.ts` divergence and written *"should be settled before TASK-145 is built"* — TASK-145 was then built, the spec never followed, and the warning cost nothing because nothing enforced it. **The obvious property is wrong and must not be substituted:** *"every path in a spec exists"* fails on the large volume of correctly-specified unbuilt work (`apps/api/src/extraction/suppressionGate.ts`, stage 4/REQ-071), which turns the gate into a backlog tracker that gets silenced. The narrow property that works is three-part — the cited path does **not** exist, its **directory does**, and a file of that **basename exists elsewhere** — which is exactly the "moved and the spec did not follow" signature and structurally cannot fire on unbuilt work (`d`). `c`: `~~struck-through~~` text is stripped, or correcting a path **in place** (the §5 house rule) would itself break the build and the only green path would be deleting the history of the mistake. `a`: non-vacuity — index size, a known basename resolving to exactly one path, spec count, citation count, **and every `CODE_ROOT` matching at least one citation**; that last clause was added after mutating `apps/api/src` out of the roots left the corpus total above its floor and `a` green, so an entire root could stop being scanned silently. `e`: **the positive control, and the reason this row is worth reading.** `b` and `d` are both satisfied by a detector that returns nothing at all — mutating `elsewhere.length === 0` to `\|\| true` blinded the gate completely and **every other case stayed green**, reproducing in the gate's own code the vacuous-pass shape it was written to prevent. `e` drives the detector over synthetic text containing a known-moved path, asserts it fires, and asserts it stays quiet on the corrected path so it is not merely "reports everything". | TASK-102, `specs/ai.md` §2.1c |
| **`T-INFRA-015` (new)** | S | **No live document may cite a repository document that is not there.** Every spec in this repo was generated by a separate authoring app rooted at `artifacts/`, and that prefix survived into the committed text: **56** citations pointed at paths like `artifacts/PRD.md`, none of which exist here, so every one was a dead link an agent would follow into nothing. ⚠ **`T-INFRA-014` could not catch these and is not at fault:** its property is *path absent + **directory present** + basename exists elsewhere*, and that middle clause — the very thing that stops it flagging correctly-specified unbuilt work — keeps it silent when the whole `artifacts/` directory is absent. Right for code, wrong for documents, hence a second gate. `d` asserts this one ignores non-markdown paths, so the two can never produce duplicate contradictory failures on the same citation. `a` is the non-vacuity floor (corpus size, citation count, and both roots represented). `e` keeps the baseline honest: an entry must still be cited **and** still be missing, so a superseded citation that gets deleted cannot linger as permanent permission. ⚠ **The baseline is for DEAD TEXT ONLY** — its single entry sits inside an ADR revision block that a later revision reversed, and repairing a path inside superseded narrative would present history as live guidance, which is the F-001 defect this project already hit. `c` is the positive control **and the reason this row is worth reading**: its first three assertions inject their own `exists()`, which proves the pure detector works and proves **nothing** about the predicate actually wired into the scan. Mutating the real one to `() => true` left `c` green and was caught only by `e`, and only because the baseline happens to be non-empty — with an empty baseline the entire gate would have gone vacuously green. `c` now asserts the real predicate in both directions. | `docs/PRD.md`, ADR-0003 |
| **`T-INFRA-016` (new)** | S | **A typecheck that no script invokes is not a gate — it is a file.** ⚠ `tests/e2e/**` is compiled by **Playwright's esbuild loader, which STRIPS types and never checks them**, and the root `npm run typecheck` (`tsc --build`) references only `packages/domain`, `apps/api`, `apps/web` and `scripts` — so for the whole life of the e2e suite **nothing has ever typechecked it**. That is invisible in the worst way: a Playwright run is green against a stub whose builders contradict the very contract types they are declared with, and a reviewer seeing `: TitleListResponse` on a builder reasonably concludes drift is caught. It is not; the annotation is decoration until something runs `tsc` over it. `a`: the root `typecheck` script **chains** `typecheck:e2e`, so every existing caller — CI's `1 · lint` job, `docs/getting-started.md`, and every lane agent's gate list — picks the alarm up without being told. `b`: `typecheck:e2e` really points at `tests/e2e/tsconfig.json`, or `a` passes while the chained command checks nothing. `c`: that tsconfig sets `noEmit` and is **absent from the root `tsconfig.json` `references`** — `tsc --build` demands a referenced project be `composite`, and a composite project writes `.tsbuildinfo` and declarations for a directory that produces no build output, so the correct wiring is deliberately *outside* the build graph and a future "tidy-up" that moves it in would break the build. `d` is the positive control and the only assertion that proves the alarm can fail: the e2e tsconfig's `include` really resolves to the spec files, so the check is not passing over an empty file set. ⚠ This is the same principle as `T-CI-008` (a spec no runner collects never runs) applied to a **typecheck** rather than a spec file, and it arrived the same way — found while bounding the `T-E2E-001` stateful stub, where the type annotations were the *entire* drift alarm. | `T-CI-008`, `tests/e2e/journey.spec.ts` |
| **`T-INFRA-017` (new, TASK-193)** | S | **The sibling of `T-INFRA-016`, and the larger half of the same hole.** That gate closed `tests/e2e/**`; **nothing typechecked the other ~240 Vitest specs.** `tsc --build` walks the root `references`, and every referenced project includes only its own `src/**`; `typecheck:e2e` covers the Playwright directory; `apps/api/test/**`, `packages/domain/test/**`, `apps/web/test/**` and `tests/**` fell between them. ESLint does not typecheck, so a spec could name a variable that does not exist, pass every static gate, and fail only if that line happened to execute — which is exactly how `T-AI-017u`/`v` (TASK-192) reached CI with `where: { ownerId, batchId }` in a file binding the owner as `owner`, surfacing as a `ReferenceError` inside a seven-minute integration job. ⚠ **It is worst where no runner reaches.** `T-CI-008` catches a spec *file* outside every collected path; it cannot catch a **collected** spec holding a `describe` that never executes — a `.skip`, a guarded branch, an error path no case supplies. A type error there is invisible for ever, and static checking is the only thing that sees it. The gate found real drift on its first run, including `w`/`h` written as `width`/`height` in `resolveCandidates.spec.ts`. `a`–`d` mirror `T-INFRA-016` case for case, including `d` as the positive control, because `tsc --noEmit` over an **empty** file set exits 0 for ever. ⚠ **Two extra assertions guard this one's specific failure modes.** `a` also pins `tsc --build` *and* `typecheck:e2e`, so the new link cannot be added by displacing an old one. `c` asserts `noImplicitAny` is **not** disabled: the paths still outside `include` (`tests/infra/**`, `apps/web/test/**`, ~237 errors) are dominated by implicit-any callback parameters, and turning the check off would "clear" them by blinding the alarm to its loudest class — the same shape as lowering a coverage threshold instead of writing a test (`T-INV-023d`). Scope is deliberately partial and **may only grow**. | `T-INFRA-016`, `T-CI-008`, `tsconfig.tests.json` |
| **`T-INFRA-018` (new)** | S | **The Node major is one decision written in four files, and nothing asserted they agreed.** `.nvmrc` (read by every CI job via `node-version-file`), `engines`, the digest-pinned `Dockerfile` base image, and `@types/node` all encode the same runtime; they were kept in step by a comment in `.github/dependabot.yml` and by whoever remembered. ⚠ **The drift is silent in the worst direction:** if `.nvmrc` moves and the `Dockerfile` does not, CI typechecks, lints and runs ~240 specs against an engine the container never ships — twelve green checks describing a build that does not exist, with the break deferred to production. `b` closes that. `c` refuses a range that merely *includes* the pinned major (`>=20`, or an open upper bound): an `engines` that accepts anything is documentation, not a pin. ⚠ **`d` is deliberately ONE-SIDED, and that asymmetry is the point.** Types *behind* the runtime cost only visibility of new APIs; types *ahead* of it compile against methods the deployed container does not have, so `tsc` passes and the failure lands at run time — the same shape as the `@types/mssql`/`mssql` pair. That is why the Node 20 → 22 move took the three runtime pins **without** `@types/node`, which is a legitimate state this gate must permit rather than a lag it should nag about. `a` is the positive control and is load-bearing: every other case compares two parsed values, and `undefined === undefined` reports perfect agreement between two files the test has quietly stopped reading. Same principle as `T-INFRA-005` for the memory/decode-guard pair — one setting expressed in two places is one setting, and a test has to say so. | `T-INFRA-005`, `T-INFRA-007d`, `.github/dependabot.yml`, `docs/runbooks/update-dependencies.md` §5 |
| **`T-META-006i` (new)** | S | **An id that appears in a spec file only as a string LITERAL is not credited as implemented.** ⚠ This is the **ninth** proven disguise of the dominant defect class, and it inverted the meaning of a gate for months. `check-orphan-tests.mjs` credited an id found **anywhere** in a `.spec.*` file, and several gates record their known gaps as arrays of id literals inside exactly such files — so **the act of writing down that a test was missing is what made the id look tested**. `T-AUTH-003`, `T-UX-053`, `T-UX-054` and `T-UX-062` occur in precisely one place in the entire suite, as literals in `uxStateCoverage.spec.ts`'s `KNOWN_UNCOVERED`, and the gate reported all four as owned and advised striking them from the baseline. Sharpening the predicate to **title position** surfaced nine hidden orphans. ⚠ **`T-META-006d` cannot catch this and is not at fault:** it plants a genuine `it()`, which both predicates credit; only the negative half discriminates, which is why this case asserts the literal is *excluded* and the real `it()` alongside it is *included* in the same fixture. ⚠ The companion failure was `T-META-006e`, named *"BASELINE_ORPHANS may only shrink"* while asserting `<= 60` — a **high-water mark, not a ratchet**. The baseline had been worked down to 26 and the ceiling never followed, leaving 34 free slots, one of which was used with CI green. Both are now pinned exactly. | §22.1, `T-META-001e` | ⚠ This is the **sixth** proven disguise of the dominant defect class, and the only one no existing gate could see: an id that exists **solely in a spec**. `check:test-ids` walks backlog → catalogue, so an id cited by no task is invisible; `check:orphans` walks outward from tests that exist, so an id with **no test at all** is unreachable by construction (and it reports an id as *owned* when merely **defined** here — a strictly weaker property than *asserted*, a vocabulary collision worth knowing about); `check:test-locations` only asks whether a **file** is collected. All three were green while **46 of the 105 ids** in the state matrix had no assertion anywhere, every one of them on an already-shipped route. That is not hypothetical — it is how `T-UX-067` was found, the 5xx-on-close state whose rejection handler was empty on the **irreversible** full-update path. The gate is a **shrink-only ratchet**: `KNOWN_UNCOVERED` may lose entries and never gain them, and `a` fails when a baselined id becomes covered, so closing a gap forces the line out. ⚠ **It nearly shipped vacuous itself:** the baseline lists ids as string literals and the scan reads every spec file, so the gate counted its own baseline as coverage and collapsed the uncovered set to empty — passing perfectly while measuring nothing. `f` asserts the self-exclusion is still in force. `b` and `c` are non-vacuity floors on the id parser and the corpus walk. | `specs/ux-states.md` §4–§9 |
| **`T-META-009` (new, TASK-126)** | S | **The mirror of `T-META-006`: a test that RUNS must be a test that was SPECIFIED.** `check-orphan-tests.mjs` guards `defined ∧ ¬cited ∧ ¬implemented` — an AC written down and never built. **The inverse was guarded by nothing.** `check:test-ids` checks that ids the *backlog* cites are defined; `T-META-001` checks every AC has a test; `T-META-004` checks the id is in title position. None asks whether an id CI is *enforcing* corresponds to a written requirement. **35 do not**, and **10 of those appear in no `specs/**` or `docs/**` file at all.** The clearest is `T-META-007` — a six-case ratchet over the whole `specs/ux-states.md` matrix, with its own positive control, cited by name in `check-orphan-tests.mjs` as design precedent, credited in `batchStatusPage.spec.tsx` with catching a real bug — and absent from this document, which NFR-003 makes the definition of done. Delete it and no gate notices. ⚠ **"Defined" is strict on purpose: a defining ROW, not a passing mention** (`e`). 25 of the 35 *are* mentioned — in `specs/ux-states.md`, `specs/ai.md`, `specs/data-model.md`, and in this file's own prose — while having no row in the AC→test mapping; letting a sentence count would read the debt as paid. So `f` splits the baseline instead of averaging it: the 10 ghosts need a row *written*, the other 25 need a row *moved*. Both lists are shrink-only and pinned **exactly in both directions**, so documenting one forces the list corrected in the same change; `d` fails if a baselined id stops being undefined, so neither becomes a museum of fixed bugs. ⚠ `a` is a positive control and load-bearing: `b`, `d` and `f` are set differences against a scanned corpus and pass **vacuously** on an empty scan — the mutation that left `T-META-008b` green. ⚠ **Two of the 35 were resolved in the same change rather than baselined, so the pins read 33 and 8.** `T-META-007` was DEFINED (see its row above) — the §12 row for this gate names it while explaining the finding, which by itself would have moved it out of the ghost set on a technicality, and describing a hole is not filling it. `T-FAKE-006` was EXCLUDED: it is not a test but a synthetic id quoted inside a string literal in `orphanTests.spec.ts`, planted into a scratch tree to prove that scanner distinguishes a real `it()` title from an id in a comment. `T-FAKE-\d+` is an established convention for exactly that, used in four gate self-tests. Baselining it would have been worse than noise — a ratchet entry that can never legitimately be paid down invites someone to write a spec row for a test that does not exist. `a` proves the exclusion is both in force and doing work. Fourth instance of one pattern, after `T-INFRA-016`, `T-CI-008` and `T-META-008`. | `T-META-006`, `T-META-008`, `T-INFRA-016`, `tools/check-orphan-tests.mjs` |
| **`T-UX-031` (TASK-126)** | S | **§3.2 suppress, already suppressed (idempotent 200)** — the dialog reads *"'{name}' was already on your Not interested list"*, distinct from a first-time hide, and offers **Close only**. ⚠ The second half is the load-bearing one: an idempotent 200 changed nothing, so offering **Undo** would invite the owner to "undo" a no-op and, if wired to the real unsuppress path, would silently UNDO THE ORIGINAL SUPPRESSION they never asked to reverse. `specs/ux-states.md` §3.2 defined this state and its id; nothing mapped it here, so it sat in `T-META-007`'s `KNOWN_UNCOVERED` — this row and `apps/web/test/suppressDialog.spec.tsx` retire that entry together. | `T-UX-030`, `T-META-007`, `specs/ux-states.md` §3.2 |
| **`T-UX-032` (TASK-126)** | S | **§3.3 fix match, searching** — typing is **debounced to ONE request for the final query**, *"Searching…"* shows while in flight, each result carries poster/name/year/type, and an empty result **QUOTES THE QUERY BACK** (*"No results for '{q}'"*). ⚠ Debounce is asserted as *one* call for rapid typing, not "fewer than N": a per-keystroke implementation still returns correct results and looks fine by hand, while spending a TMDB request per character. ⚠ Quoting the query back is not cosmetic — a bare *"No results"* leaves the owner unable to see that OCR handed the search a mangled title, which is the failure this state exists to surface. Defined in `specs/ux-states.md` §3.3 and unmapped here until now; retired from `T-META-007`'s `KNOWN_UNCOVERED` with `apps/web/test/fixMatchDialog.spec.tsx`. | `T-META-007`, `specs/ux-states.md` §3.3 |
| **`T-UX-036`** | C | State 3.7 **Fix match succeeded AND a suppression was migrated** — the migration is stated **when it happened, and only then**, alongside the success message rather than instead of it. ⚠ Both directions are load-bearing and only one was ever asserted: `T-UI-020i` drives the response that reports a migration and checks the notice appears, so rendering the notice **unconditionally** passed all 22 cases in that file while telling the owner their *"not interested"* mark had been moved on **every** fix-match, including the ones where they never made one. Product invariant 1 is what makes this matter — suppression is keyed on canonical work identity, so a fix-match genuinely can move it and the owner has no other way to learn that it did; claiming it falsely is the same defect facing the other way. Defined in `specs/ux-states.md` §3.7 and unmapped here until now; retired from `T-META-007`'s `KNOWN_UNCOVERED` with `apps/web/test/fixMatchDialog.spec.tsx`. | `T-META-007`, `specs/ux-states.md` §3.7 |
| **`T-META-007` (TASK-126)** | S | **Every state in `specs/ux-states.md` is either asserted by a test, or is on an explicit shrinking list of known gaps.** ⚠ **This row is retrospective: the gate has been running in CI for some time and was defined in NO specification** — it was found by `T-META-009`, and its absence is the exact defect that gate exists to prevent. `specs/ux-states.md` gives all ~105 UI states a test id, and **no pre-existing gate could check that those ids led anywhere**: `check-test-ids.mjs` walks backlog→spec and cannot see an id no task cites; `check-orphan-tests.mjs` starts from tests that exist, so an id with *no* test is by construction invisible to it; `check-test-locations.mjs` only asks whether a spec FILE is collected. So an id could describe a `must` behaviour on a shipped route, have no assertion at all, and every gate stayed green. Not hypothetical — that is how `T-UX-067` (§6.16, 5xx-on-close) was found: the batch close shipped with an **empty rejection handler**, so its failure path was silent. `T-UX-060` was the same shape reversed — `data-testid="review-loading"` shipped in `ReviewPage` with no assertion naming it. **It has since been closed** (`apps/web/test/reviewSkeleton.spec.tsx`), and closing it uncovered a second failure mode this gate does not catch on its own: §6.1 named `GET /api/batches/:batchId` as the source of counts that endpoint has never carried, so the state was not merely unbuilt but **unbuildable as specified**. A ratchet can tell you an id is unasserted; it cannot tell you the spec behind it is wrong. ~~Superseded: "`T-UX-060` is the same shape reversed — … and no assertion names it."~~ `a` is a ratchet failing in **both** directions: a new uncovered id fails it, and an id that is now covered but still listed fails it too — the second half is what stops the baseline going stale and becoming a list nobody prunes. `b`/`c` are corpus controls; `d` drives the detector over synthetic input and asserts it both FIRES on a gap and STAYS QUIET on a covered id, because "reports everything" is as broken as "reports nothing" — added after `tests/infra/specPaths.spec.ts` shipped with a blinded detector that left every case green. `e` proves the baseline is real, unique and sorted. ⚠ `f` proves the file EXCLUDES ITSELF from the scan it feeds, which is load-bearing: `KNOWN_UNCOVERED` writes every gap id as a literal, so without the exclusion the scan finds each one in this very file, concludes it is asserted, and the uncovered set collapses to empty with `a` green. | `T-META-006g`, `T-META-009`, `specs/ux-states.md` |
| **`T-META-008` (new, TASK-126)** | S | **Cross-file uniqueness of SUFFIXED test ids — documented as guaranteed, owned by a named gate, and enforced by nothing.** `tools/eslint-rules/test-id-naming.js` sees one file at a time and says so, then delegates: *"Cross-file uniqueness and the AC→test mapping are enforced by `T-META-001`."* `T-META-001` maps ACs to ids and fails on an AC with no test or an id absent from the suite; it has **never** checked that a suffixed id is used once. **65 collisions accumulated behind that sentence.** The worst is `T-AI-036b`, which names *"issues both legs in PARALLEL"* in `hybridExtractor.spec.ts` and *"a missing OCR leg is NOT degraded"* in `runExtraction.spec.ts` — two unrelated properties under one name, which destroys the entire justification for putting the id in the title (*"a failure that says `T-REV-006` failed names the acceptance criterion directly"*). ⚠ **A bare base id in several files is legitimate and is NOT reported:** `T-AI-036` is typed `I` and is deliberately asserted at three levels, so `e` fails if the regex is ever widened from `{1,2}` to `{0,2}` and starts reporting the suite intended multi-level coverage as a defect. The 65 are a **shrink-only ratchet** — not a claim they are harmless, but a refusal to guess which file owns which letter across 65 judgement calls, the same reasoning `BASELINE_ORPHANS` gives. `c` pins the size exactly in **both** directions so resolving one forces the list to be updated in the same change; `d` fails if a baselined id stops colliding, so the list cannot become a museum of fixed bugs. ⚠ `a` is the positive control **and it is load-bearing, proven not assumed**: with the file walk mutated to match nothing, `b` — the actual gate — **passed vacuously** on an empty scan, and only `a` and `d` failed. That is `T-CI-008` applied to a scanner rather than a runner. | `T-META-001`, `T-CI-008`, `tools/eslint-rules/test-id-naming.js` |
...
| **`T-UX-002`** | C | State 2 **Any failed read**: no `fetch` rejection path ends without a rendered message, and the message is paired with a remedy. ⚠ This is an **exhaustiveness** claim and it is the one thing the six per-screen failure tests cannot make. `listRoute.spec.tsx`, `removedRoute.spec.tsx`, `suppressedRoute.spec.tsx`, `batchesOffline.spec.tsx` and the review suites each assert their own failure copy **more strongly** than this id does — and every one of them is silent about the screen added next week whose author forgets one, which is invisible in a green suite. The expected set is therefore read off `ROUTES` in `apps/web/src/routes.tsx`, already exported as data for `T-ATTR-002`/`T-A11Y-001`, and **which** routes read is discovered by observing `fetch` rather than declared, so neither list can drift; `T-UX-002d` is the non-vacuity floor that stops that discovery exempting every case at once (6 fetching routes today, asserted at >= 4). The rejection modelled is `fetch`'s literal one (`TypeError: Failed to fetch`), which has no envelope to read a message out of and is therefore the case where a screen is likeliest to render the raw error or nothing at all. Defined in `specs/ux-states.md` §2 and unmapped here until now; retired from `T-META-007`'s `KNOWN_UNCOVERED` with `apps/web/test/failurePaths.spec.tsx`. | `T-META-007`, `specs/ux-states.md` §2, `specs/ui.md` §12.2 |
| **`T-UX-015`** | C | State 2.6 **the count**: the *"Showing X of Y"* line **never fabricates a total** (NFR-018). ⚠ This was a live, measured product defect, not a missing test. The list is served one page at a time (`DEFAULT_PAGE_LIMIT = 50`, `apps/api/src/pagination.ts`), and `ListRoute.tsx` rendered `total={unfiltered.length}` — the rows **in hand** — flat. A probe mounting the real container against a response of 50 items plus `nextCursor` produced the literal string **`Showing 50 of 50`**: an owner with 300 titles was told they had 50. That is the data-loss misreading US-019 AC-5 exists to prevent, and §2.6 forbids it in bold. The count now renders `of at least 50` whenever the response carries a `nextCursor`, via `AT_LEAST_PREFIX` (whose **trailing space is part of the constant**; `T-UX-015d` asserts it, because trimming it yields `of at least50`). **The sentinel half now ships too** (`T-UX-015f`-`r`, `apps/web/test/loadMore.spec.tsx`): `useCursorPages` accumulates pages, `LoadMoreSentinel` carries both affordances §2.1 item 4 requires, and the bound is retired once the last page is in hand. Pages **append, never replace** — the obvious wrong fix passes every request-shaped assertion while deleting the fifty rows the owner was reading — and they are **discarded whenever page 1 changes identity**, which a retry and the reconnect refetch do without changing the query string. Three of the thirteen mutations run against this id **survived** and forced new cases: a failure clearing accumulated pages (invisible to a test that fails on page 2), the reset ignoring a fresh page 1, and a fake `IntersectionObserver` that fired without ever observing anything. ~~"⚠ THIS COVERS ONE HALF OF §2.6 ONLY. The load-more sentinel ... is **not built**; `ListRoute` still never advances the cursor, so titles past the first page remain **unreachable in the UI**, and no backlog task covers the remainder."~~ — true for one PR. ~~"`T-UX-073` remains the same gap in the REMOVED view."~~ — also superseded; the removed view ships in the same PR, see the row below. `T-UX-015b` pins the exact defective string so the regression cannot return unnoticed; `T-UX-015c` and `T-UX-015e` are counterweights proving the qualifier is **conditional** — without them the whole id passes by hedging unconditionally, which is a different lie. `T-UX-015a` drives the assertion through the real container so the wiring, not just the component, is covered. Retired from `T-META-007`'s `KNOWN_UNCOVERED` with `apps/web/test/listCount.spec.tsx`. ⚠ **`T-UX-015q` WAS FLAKY AND FAILED ON `main` — a green PR run, then `expected undefined to be defined` on the merge commit, with no code in between.** The race is in the test, not the component: `mountList()` resolves as soon as the first row is on screen, but the sentinel's `observer.observe(element)` runs in an effect *after* that render, once the ref is attached, so there is a window in which rows exist and nothing is being observed yet. Reading `FakeObserver.live()` synchronously samples that window on whichever run happens to be slow — a coin toss that lands heads often enough to look deterministic locally. Both cases now await a `liveObserver()` helper. ⚠ **Polling is not the same as tolerating absence:** `waitFor` still fails if the observer never appears, so deleting `observer.observe(element)` — which stops the auto-loader in every real browser — kills these cases exactly as before; the wait removes a timing assumption, not an assertion. ⚠ **`T-UX-015r` WAS STRENGTHENED, NOT MERELY DE-FLAKED, AND HAD BEEN PASSING VACUOUSLY.** It fired through `FakeObserver.live()?.fire()`, which does *nothing* when no observer has attached — and its closing assertion, `expect(live()).toBeUndefined()`, is satisfied by that same absence. The case could therefore report PASS having never fired anything and never exercised the disconnect, because an observer that was never attached is trivially "not attached afterwards". Awaiting the observer first is what makes the final line mean *it stopped* rather than *it never started*; the mutation confirms it, since removing `observe()` now fails `q` **and** `r`, where previously it failed only `q`. Verified 30/30 consecutive clean runs. | `T-META-007`, `specs/ux-states.md` §2.6, `specs/ui.md` §2.1 |
| **`T-A11Y-006`** | C | `specs/ui.md` §9 focus order: **dialogs trap focus, restore it to the trigger on close, and close on `Escape`.** ⚠ **All three dialogs shipped with `aria-modal="true"` and none of the behaviour it promises.** That pairing is worse than an unlabelled `div`: `aria-modal` tells assistive technology the rest of the page is inert, so a screen-reader user who tabs out of an untrapped dialog lands in content their software has been told does not exist, with nothing to signal they have left — the attribute was a claim the code did not honour. Restoring focus is not a courtesy either: every one of these dialogs opens from a control part-way down a long list, and unmounting without restoring drops focus to `<body>`, so the next Tab restarts at the top of the document. `useDialogFocus` (`apps/web/src/lib/useDialogFocus.ts`) supplies all three behaviours; `onDismiss` is held in a ref because call sites pass inline arrows, and a changing dependency would re-run a setup that **moves focus** on every render. `T-A11Y-006c` pins Escape to **cancel, never confirm** — this is the product's only confirmation (REQ-020) and it authorises deletions, so the universal "get me out" gesture must not be the destructive one. `T-A11Y-006f` covers §6.12, where every control is disabled mid-close and the dialog's own `tabIndex={-1}` must hold focus rather than let it fall to `<body>` at the moment the page claims to be inert. ⚠ **`T-A11Y-006e` is the case that keeps this true**: the behavioural cases each mount a dialog someone remembered to wire and are silent about the fourth dialog added later, so the expected set is **read off the source** — every component asserting `aria-modal` must call the hook. An earlier version searched for the bare identifier and a mutation deleting the call while leaving the import **survived**; it now requires `useDialogFocus(`, because an unused import is exactly what half-finished wiring leaves behind. Eight mutations, all killed. ⚠ **PARTIAL, AND THE RATCHET FORCED THE RETIREMENT** (`T-META-007f` fails when an id is both asserted and declared a gap): `specs/ux-states.md` §1 *"Focus after a state change"* — success moves focus to the `role="status"` region, error to `role="alert"` — is carried by this same id. The **success half is now closed** by `T-A11Y-006h`–`n` in the same file: `useOutcomeFocus` moves focus to the outcome for the three **action** outcomes the owner chose — the review close (`BatchAppliedNotice`), the fix-match confirmation (`FixMatchDialog`) and the suppression (`SuppressDialog`). ⚠ Do **not** widen it by focusing every live region: there are 53 across 23 components and most are **ambient** (`OfflineBanner`, `FreshnessStrip`, `FilterBar`'s count, the load-more busy message), so focusing them would yank focus on every poll — a worse defect than the one being fixed. `T-A11Y-006m` reads the source and fails in BOTH directions (a fourth component adopting the hook, or one of the three dropping it); `T-A11Y-006n` pins the ambient four. ⚠ `T-A11Y-006j` guards the dependency array: without it the hook re-focuses on every render, taking focus off the **Undo** button the owner is pressing. ⚠ **STILL OPEN:** the ERROR half (focus to `role="alert"`). ~~Superseded: "remains **unasserted** … Which regions are action outcomes is an owner-level design decision."~~ Retired from `T-META-007`'s `KNOWN_UNCOVERED` with `apps/web/test/dialogFocus.spec.tsx`. | `T-META-007`, `specs/ui.md` §9, `specs/ux-states.md` §1 |
| **`T-UX-073`** | C | State 7.4 **Partial** on `/removed`: *"Page 1 + load-more sentinel"* (`specs/ui.md` §2.1 item 4). ⚠ **This is the list most certain to outgrow one page, and the one whose truncation is most easily misread as loss.** By product invariant 7 a reappearing title becomes a brand-new row, so the removal log only ever grows and legitimately holds several entries for the same work; capped silently at `DEFAULT_PAGE_LIMIT = 50` it stops at the most *recent* removals, and the older ones — exactly what an owner opens this screen to find — are simply not there. On the one screen whose purpose is to prove nothing was lost, that is the US-019 AC-5 misreading delivered by the reassurance itself. `RemovedRoute` now accumulates pages through the same `useCursorPages` hook as the list and renders the same `LoadMoreSentinel`. ⚠ The sentinel sits **outside** the results-vs-empty branch here, unlike on the list: the removed view filters server-side by `q`, so a search matching nothing on page 1 may still match on page 2, and hiding the control in that branch strands the owner on a screen that is empty only because it is incomplete (`T-UX-073d`). Four mutations were run and the third **survived** the file as first written — deleting the `!loading` guard changed nothing observable, because through the route `hasMore` is false whenever page 1 is in flight, so `T-UX-073e` passed on that coincidence rather than on the guard it names. `T-UX-073f` drives `RemovedPage` directly with the contradictory `loading` + `hasMore` pair the guard exists to refuse: the state a **refetch** reaches, where "Load more" would page a cursor belonging to the query the owner just left and append its rows to a list about to be replaced. Retired from `T-META-007`'s `KNOWN_UNCOVERED` with `apps/web/test/removedLoadMore.spec.tsx`. | `T-META-007`, `specs/ux-states.md` §7.4, `specs/ui.md` §2.1 |
| **`T-UX-001`** | C | State 1 **"Never an indefinite spinner"**: a pending request that passes **1200 ms** gains `SlowResponseNotice` (`role="status"`, *"Still working…"*), and one that passes **70 s** becomes an **error with a remedy** (`role="alert"` + **Retry**), with a **`waking`** phase at **15 s** in between (`role="status"`, *"Waking the database…"*, **no Retry**). ⚠ **Nothing existed.** `SlowResponseNotice` was not a component that needed improving — **TASK-143 renamed it in the SPEC** (from `ColdStartNotice`, because ADR-0003 Rev 3 pins `minReplicas = 1`, so there is no cold start to name as the cause and *"Waking things up…"* would have been a confident, specific, **wrong** explanation) while it had never been built at all: §1's central promise was documented, justified, renamed for accuracy, and **enforced by nothing**. A hung list read showed *"Loading your list…"* for ever, with no way to learn it had stalled and nothing to retry. ⚠ **The 15 s state is the id, not the 1200 ms one.** *"Still working…"* is always true while a request is pending, so on its own it is an indefinite spinner that has learned to talk: it can never report that something went wrong. `T-UX-001d`/`e` pin the exit — assertive role, a Retry, and the words *"Nothing has been changed"*, which is §1's ASM-029 defence, since a timeout is precisely when the owner cannot tell whether the request half-applied. ⚠ **Two weak tests were found by mutation, not by review.** Every threshold was first written `advance(SLOW_AFTER_MS - 1)` — a form that **moves with the constant it exists to pin**, so setting the source to `0` made the test advance `-1` ms and still pass; the literals `1200`/`15000` are now written out and the constants asserted against them. And the absence assertions checked only `slow-response`, so a component rendering the **stalled** block early — a worse defect — slipped straight through. ⚠ `T-UX-001h` exists because `T-UX-001f` looked sufficient and was not: deleting `clearTimeout(stalled)` from the hook's cleanup **survived the entire first battery**, because `f` lets the first attempt run the full 15 s, so the leaked timer has already fired and nothing is left to leak. The leak only bites when the first attempt settles **early** — the common case — where the abandoned timer reports a stall on a request that is healthy. Retired from `T-META-007`'s `KNOWN_UNCOVERED` with `apps/web/test/slowResponse.spec.tsx`. <br> ⚠ **THE 15 s EXIT WAS ITSELF A DEFECT, AND THE OWNER HIT IT.** Staging's Azure SQL is serverless with `autoPauseDelay = 60` (ADR-0003 Rev 3) — paused deliberately, to bill nothing while idle — and the first request after a pause triggers a **30-60 s resume**. `SQL_CONNECT_TIMEOUT_MS` (60 s) waits that out, so the request genuinely **succeeds**; this machine declared it stalled at 15 s and showed *"This is taking longer than it should"* with a Retry that could only hit the same wall. Observed live 2026-09-18: `resumedDate` 12:54:34Z, every read failing to 12:55:44, then recovery with no intervention. A healthy, expected, billed-for state was reported as a fault, and the remedy offered made it worse. `T-UX-001i`/`j`/`k` and `T-UX-010f` are the fix. ⚠ **`waking` was INSERTED, not substituted** — §1 still requires the wait to terminate in something actionable, and a "waking" notice that never ends is the indefinite spinner wearing new copy; `T-UX-001d` keeps the exit and moves it to 70 s, which `T-UX-001a` pins as **greater than the server's 60 s connect timeout**, because below it the rule reports failure for requests about to succeed. ⚠ **THIS IS NOT `ColdStartNotice` RETURNING.** TASK-143's reasoning is untouched and still correct: the **app** never cold-starts. The **database** does, and is a different cause — so `T-UX-001j` requires the word *"database"* in the copy, guarding against the nearest wrong fix, which is to reuse the old app-flavoured wording and silently re-assert a cause that cannot occur while looking like a tidy-up. `T-UX-001i` also pins **no Retry** on the waking phase: there is nothing to retry, only something to wait for, and a Retry restarts the very wait it appears to escape. `T-UX-001k` sweeps the whole 15-60 s window rather than sampling one instant, because the defect *was* a threshold and a threshold nudged a few seconds the wrong way passes any single-point test. ⚠ `T-UX-001h` was extended to assert the **waking** timer is cleaned up too — it is armed at exactly the old stalled threshold, so checking only `slow-stalled` would leave the new timer untested at the moment it is easiest to forget. Mutation-proven: reverting the stall threshold to 15 s fails `a`, `i`, `j`, `k` and `T-UX-010f`; announcing `waking` as `role="alert"` fails `i`; app-flavoured copy fails `j`; rendering nothing for `waking` fails `i`, `j` and `T-UX-010f`. <br> ~~Superseded: "one that passes **15 s** becomes an **error with a remedy**".~~ | `T-META-007`, `specs/ux-states.md` §1, ADR-0003 Rev 3 |
| **`T-UX-010`** | C | State 2.1 **Loading** on `/`: exactly **six row skeletons**, all `aria-hidden`, under one `role="status"` wrapper named by `LIST_LOADING_BODY`. Real service updates/search/filter/sort/view controls remain mounted; **no freshness-strip or filter-bar skeleton duplicates**. `FilterBar.countPending` hides result and runtime-hidden counts until the response supplies them. `a` retains the six-row shape, `b` the hidden skeleton/status semantics, `c` absence of never-uploaded empty state; `d`/`e` still prove real-page slow-request/retry wiring rather than a probe-only timer, and **`f` does the same for the new `waking` phase** — the list is where the resume defect was actually reported, the owner opening the app after a quiet spell and being told *"Couldn't load your list."* Every probe-driven `T-UX-001` case passes with `ListPage` never calling the hook, which is exactly the state this screen was once in. `T-UX-141h`/`i` supplement this with pending-picker and selected-genre preservation. Implemented in `apps/web/test/slowResponse.spec.tsx`. | `T-META-007`, `specs/ux-states.md` §2.1, `specs/ui.md` §2.1 |
| **`T-UX-021`** | C | State 2.13 **Submitting (row action)** on `/`: *"the affected row dims with an inline spinner; **the rest of the list stays interactive**"*. ⚠ **The state was modelled and rendered nowhere.** `SuppressDialog` has reported `'pending'` since TASK-102 and `ListPage` has stored it in `rowStates` ever since — it simply never reached the row, so a write in flight looked identical to an idle list, and the row the owner had just acted on gave no sign it had heard them. ⚠ **The second half of the sentence is the requirement.** A list-wide `busy` flag satisfies *"the affected row dims"* — it dims that row too — while doing precisely what §2.13 forbids in the same breath; on the owner's real list that reads as the app having frozen, and the natural response is a reload, the one action that can lose the write still in flight. `T-UX-021c` is the **only** case that fails under that implementation, which is why it asserts the **other** row rather than the affected one. ⚠ **`aria-busy` and a worded spinner, not just opacity.** The dim alone is a colour-only state (`specs/ui.md` §10.2 forbids colour as the sole carrier) and "greyed out" is indistinguishable from "saving" at a glance; the label names the row's **state**, not the action, because the row does not know which §2.3 action is in flight and claiming *"Removing…"* over a **Fix match** would be a confident, specific, wrong description of a write the owner cannot see. ⚠ **`T-UX-021g` restates a guard `ListPage` already holds** — `pending` dims and **never hides**, since the optimistic-then-reconcile behaviour TASK-102 rejected would leave a failed write's row hidden while it is still on the list. ⚠ **`T-UX-021h` is the wiring case and is not redundant**: every other case hands `TitleList` a set the test built, so all of them pass with `ListPage` deriving `pendingTitleIds` from nothing — the state the screen was in. It drives the real §2.3 menu → confirm path with a suppress that never settles, the only way the pending phase is observable. Seven mutations killed by exactly the named cases, alongside a **negative control** (a comment-only edit) that correctly survived. Retired from `T-META-007`'s `KNOWN_UNCOVERED` with `apps/web/test/rowPending.spec.tsx`. | `T-META-007`, `specs/ux-states.md` §2.13, `specs/ui.md` §10.2 |
| **`T-UX-017`** | C | State 2.8 **Partial data — TMDB stale** on `/`: rows render from **stored** metadata and the affected ones carry a subtle chip *"Details may be out of date"*, while the owner can still do *"everything, normally"*. ⚠ **THIS ID LOOKS LIKE A DELETED FEATURE AND IS NOT — read product invariant 8 before touching it.** A46 dropped the staleness nudge whole (no threshold, no nag, no derived `stale` state) and `specs/ui.md` §2.1 item 1 lists *"the stale chip"* among the things it dropped — but that sentence is about **how long ago the owner last uploaded**, which is `FreshnessStrip`'s subject. This chip is the other sense of the overloaded word: **TMDB unreachable, or over the 5 s refresh budget, while serving this page** (NFR-014, REQ-076, `specs/api.md` §6.4), which is still required. Deleting it as A46 scope removes the metadata pipeline's only user-visible signal. ⚠ **The flag had been on the wire the whole time.** `apps/api/src/routes/titles.ts` emits `metadataStale` on **every** item and `apiClient` casts parsed JSON straight through, so the value has been arriving in the browser since the refresh landed and being dropped for want of a field on `TitleListItem` — a defect no API test could see (the server was right) and no component test could see (each built its own fixture). `T-UX-017f` is the only case that fails if the value cannot travel. ⚠ **`true` never means the data is missing or wrong**, and `T-UX-017d` is the case that pins it: §6.4 says *"the list never fails because of TMDB"*, so the row renders in **full** — poster, genres, year, rating, date label — and merely gains a chip. Hiding the genres when stale kills `d` **alone**; an implementation that blanked or errored the row would pass every other case here. ⚠ **Not a live region** (`T-UX-017h`): §2.8's own *"user can"* column is *"everything, normally"*, so there is nothing to interrupt for and nothing for the owner to do — the refresh retries itself on the next view. ⚠ **`T-UX-017g` keeps the two chips independent**: an unmatched title still has stored metadata TMDB can fail to confirm, and writing them as one `unmatched ? … : …` branch drops the stale signal on exactly the rows whose data is least trustworthy. ⚠ **`T-UX-017i` reads the stylesheet** because jsdom applies none: the muted styling is *"subtle"* per §2.8, and without it the chip is indistinguishable from `Unidentified`, which reports a different kind of problem. Nine mutations killed by exactly the named cases, alongside a comment-only **negative control** that correctly survived. Retired from `T-META-007`'s `KNOWN_UNCOVERED` with `apps/web/test/metadataStale.spec.tsx`. | `T-META-007`, `specs/ux-states.md` §2.8, `specs/api.md` §6.4 |
| **`T-UX-016`** | C | State 2.7 **Populated**: the combined list renders **one row per title** with a badge per service, **in the order the server sent**, collapsing nothing of its own. ⚠ `TitleList.tsx` has always carried the rule — *"THIS COMPONENT DOES NOT SORT, GROUP OR DEDUPE... a client-side `sort()` here would silently disagree with the cursor the API pages on"* — and until this id **no test imported `TitleList` or queried `title-list` at all**: documented, justified, enforced by nothing. Every case asserts something the component must NOT do, because a client-side reorder is invisible on one screenful of data and only misbehaves at the page boundary, on the owner's real list. Protects product invariant 6 (title-level date order is the EARLIEST date-added across the title's listings, computed in `packages/domain/src/ordering.ts`) and invariants 1/7 (the work-level collapse is the server's, keyed on canonical work identity — a client dedupe by name would delete a remake or its original). Defined in `specs/ux-states.md` §2.7 and unmapped here until now; retired from `T-META-007`'s `KNOWN_UNCOVERED` with `apps/web/test/titleList.spec.tsx`. | `T-META-007`, `specs/ux-states.md` §2.7, `specs/ui.md` §2.1 |
| **`T-UX-040`** | C | State 4.1 **`/upload` initial**: with no service and no mode chosen, the **attach area stays enabled** and the blocking reason `SUBMIT_NEEDS_SELECTION` — *"Choose a service and a mode first."* — sits on **submit**. ⚠ This id could not be implemented until the SPEC was corrected, and the contradiction sat one row above it: §4.1 said the attach area was *disabled* with that reason, while §4.0a (added at **A45**) says a paste arriving before selection is **held client-side, not discarded** (`T-PASTE-002`). A disabled attach area cannot hold anything. The code has always implemented §4.0a — `ImageDropzone.tsx` contains **no `disabled` at all** — so §4.1 was corrected in place with the superseded wording struck through. ⚠ The sentence itself did **not** change; only the control it explains moved, which is why `T-UX-040c` asserts its **location** and not merely its presence — asserting the text alone passes under both the old and the new wording. `T-UX-040b` is deliberately wider than `T-UX-040a`: the old wording would most naturally be re-implemented by disabling the surrounding fieldset or the drop target rather than the file input. The cost of regressing this is not cosmetic — it discards the owner's screenshot for the most ordinary sequence in the product, pasting before picking a service, and the screenshot is often no longer on the clipboard to paste again. Retired from `T-META-007`'s `KNOWN_UNCOVERED` with `apps/web/test/uploadInitial.spec.tsx`. | `T-META-007`, `specs/ux-states.md` §4.1, §4.0a |
| **`T-UX-044`** | C | State 4.6 **Ceiling breached**: the size and count ceilings are refused with a reason that **names the actual number and the limit**, and that reason is **rendered**, not merely returned. ⚠ Distinct from `T-UX-042b`/`c`, which call the pure `reviewFiles()` and assert its return value: §4.6 is a rendered state, and before this id only the unsupported-**format** rejection was ever asserted as reaching the screen. ⚠ And the ceiling is **stateful** — `reviewFiles(files, alreadyAccepted)` is only as correct as the count the component hands it, so `reviewFiles(files, 0)` lets the owner walk past `MAX_IMAGES_PER_BATCH` over successive attaches while every pure-function case stays green. One case must therefore attach twice. Defined in `specs/ux-states.md` §4.6 and unmapped here until now; retired from `T-META-007`'s `KNOWN_UNCOVERED` with `apps/web/test/imageDropzone.spec.tsx`. | `T-META-007`, `specs/ux-states.md` §4.6 |
| **`T-UX-090` (imported at TASK-076)** | C | State 9.1 **Loading**: `/batches` renders skeletons rather than an empty history. An empty list and an unloaded list are the same pixels and opposite facts — one says *"you have never uploaded anything"*, which for this owner would be false and alarming. | `specs/ux-states.md` §9.1 |
| **`T-UX-091` (imported at TASK-076)** | C | State 9.2 **Empty**: *"You haven't uploaded anything yet."* with a route to `/upload`. Asserted against the named copy constant, not a substring. | `specs/ux-states.md` §9.2 |
| **`T-UX-092` (imported at TASK-076)** | C | State 9.3 **Populated**: one card per batch carrying date, service, mode, status and the count triple *"Created 6 · Modified 0 · Removed 3"*. ⚠ `created` counts **creations, not `batch_change` rows** — a `title_created` row and its `listing_added` sibling are one creation (`data-model.md` §3.7), so a card summing both kinds shows double what the detail page then lists, and each number looks individually plausible. | `specs/ux-states.md` §9.3 |
| **`T-UX-093` (imported at TASK-076)** | C | State 9.4 **Detail — provenance**: the created, modified and removed lists are rendered **in full**, each entry naming and linking to its title (US-031 AC-2/AC-3/AC-4). ⚠ **Nothing is summarised away.** This view is the owner's only account of what an import did to their list; a *"and 12 others"* here is the product declining to answer the question it exists to answer. The entries carry title **names**, not ULIDs, and the title's **current** state, so a title since removed or suppressed reads as such (US-033 AC-6). | `specs/ux-states.md` §9.4 |
| **`T-UX-094` (imported at TASK-076)** | C | State 9.5 **Detail — a batch that changed nothing**: the sentence *"This upload didn't change anything."* is rendered **explicitly**, not implied by three empty panels. ⚠ The flag is computed server-side and sent, because a batch that only **modified** something did change something, and deriving the rule in the SPA as well would put it in two places that can disagree. Three empty panels are indistinguishable from a failed load. | `specs/ux-states.md` §9.5 |
...
| **`T-UX-045` (imported at TASK-178)** | U | State 4.7 **Populated**: the submit control is enabled **only** with a service, a mode and at least one image, and while it is disabled the **reason is rendered as text**, not merely encoded in the `disabled` attribute (`specs/ux-states.md` §4.7, §4.1). A disabled button with no visible reason is the single most common dead end in this flow — the owner cannot tell a broken app from a missing choice. Asserted against the exported pure `submitBlockedReason`, so the two branches cannot drift from the rendering. | `specs/ux-states.md` §4.7 |
| **`T-UX-046` (imported at TASK-178)** | U | State 4.8 **Submitting**: the whole form is disabled and the **"Don't close this tab"** warning is shown for the duration of the request (`specs/ux-states.md` §4.8). Without it a second press re-posts the same batch, and closing the tab mid-upload loses images the owner believes are attached. | `specs/ux-states.md` §4.8 |
| **`T-UX-047` (imported at TASK-178)** | U | State 4.9 **Success**: a successful submit **navigates to `/batches/:batchId`** — the id the server returned, not a remembered one — and posts exactly once (`specs/ux-states.md` §4.9). Staying on `/upload` after a successful close leaves the owner with no route to the extraction they just started. | `specs/ux-states.md` §4.9 |
| **`T-UX-048` (imported at TASK-178)** | U | State 4.10 **409 `OPEN_BATCH_EXISTS`**: the server's message is rendered **verbatim** with **both** actions — "Go to it" (navigating to the batch named in the envelope's `details`, never a guess) and "Discard it and start again" (discarding the **other** batch, then retrying) — **without replacing the screen**, so the owner's selection and held images survive (`specs/ux-states.md` §4.10). Offering only one action strands the owner on whichever branch they did not want; replacing the screen silently discards work they can no longer recover. | `specs/ux-states.md` §4.10 |
| **`T-UX-043` (imported at TASK-178)** | U | State 4.5 **All rejected**: no thumbnails, the rejection list rendered, submit still disabled (`specs/ux-states.md` §4.5). ⚠ The load-bearing half is **where the rejections come from**: partial acceptance returns **201 with `rejected[]` in the body**, but when *nothing* is accepted the API throws with the first rejection's own status and puts the array in the error envelope's `details.rejected` (`api.md` §6.12, `apps/api/src/routes/batchImages.ts`). A client that reads only the success body shows an **empty** rejection list on the one request where every single file failed — the worst case rendered as the least informative screen. | `specs/ux-states.md` §4.5 |
| **`T-UX-074` (imported at TASK-099)** | C | State 7.6 **Submitting (restore)**: clicking Restore dims the row with a spinner and the control is replaced by an `aria-busy` indicator, so the owner knows the request is in flight and cannot double-submit. | `specs/ux-states.md` §7.6 |
| **`T-UX-075` (imported at TASK-099)** | C | State 7.7 **Success (restore)**: the row moves out of the list AND a `role="status"` announces *"'{name}' is back on your {service} list, with its original date ({date})."* — **naming the original date** is what makes US-025 AC-2 visible. | `specs/ux-states.md` §7.7 |
| **`T-UX-076` (imported at TASK-099)** | C | State 7.10 **Error — 409 `LISTING_NOT_REMOVED`**: *"'{name}' is already back on your list."* with a refresh button. The listing was restored by another path (e.g. undo); the owner's view is stale. | `specs/ux-states.md` §7.10 |
| **`T-UX-034` (imported at TASK-099)** | C | State 7.8 / §3.5 — **409 `DUPLICATE_WORK_IDENTITY`**: the dialog offers **"Yes, keep both"** (which re-sends with `confirmDuplicate: true`) and **Cancel**. The retry parameter is the load-bearing half — without it the second request hits the same unique index. | `specs/ux-states.md` §7.8 / §3.5 |
| **`T-UX-035` (imported at TASK-099)** | C | State 7.9 / §3.6 — **409 `WORK_SUPPRESSED`**: the dialog shows *"You marked '{name}' as not interested. Stop ignoring it first?"* with **"Stop ignoring and continue"** (which calls unsuppress THEN retries restore — **order is load-bearing**) and **Cancel**. | `specs/ux-states.md` §7.9 / §3.6 |
...
### 12.5 Imported by the `T-META-009` sweep — ids that RUN but were defined nowhere here

`T-META-009` asks the question `T-META-005` and `check:orphans` never did:
**does an id that CI is already enforcing correspond to a written
requirement?** It found 33. The nine `T-DM-*` entries were §12.1's class and
are recorded there; the 24 below are §12.3's class, one layer worse — most are
fully specified in `specs/ux-states.md`, with wording, controls and the state's
number, and were simply never imported into the document NFR-003 makes the
definition of done.

⚠ **A row in a sister spec is not a definition of done.** A UX state defined
only in `specs/ux-states.md` is a design decision; the test that proves it is
delivered belongs in a work order's exit criteria, and this file is where a
reviewer looks for that. `T-META-007` already asserts the reverse direction —
every state in the §2-§9 matrix has an id — so the two gates now close the loop:
a state cannot lose its test, and a test cannot lose its state.

| Test | L | Asserts | Anchored in |
|---|---|---|---|
| **`T-AI-019`** | U | The cross-check thresholds are **named constants, ordered, and in range**: `OCR_SUPPORT_EXACT > OCR_SUPPORT_PARTIAL > 0`, both `≤ 1`, and `OCR_BOX_OVERLAP_MIN` in `(0, 1]`. Implemented in `packages/domain/test/crossCheck.spec.ts`. Ordering is the load-bearing half — inverted, every partially-supported candidate is promoted to `exact` and the corroboration signal silently reads as stronger than the evidence. | `specs/ai.md` |
| **`T-CI-006`** | S | **Every GitHub Action is pinned to a commit SHA.** `a` no workflow uses a mutable tag; `b` mutation-proves a planted `actions/checkout@v4` is caught. Implemented in `tests/infra/supplyChain.spec.ts`. A tag is mutable — `@v2` can be re-pointed at new code by anyone who can push to that repository, inside a workflow holding `GITHUB_TOKEN` and the `ghcr.io` push. | `specs/security.md` |
| **`T-CI-008`** | S | **No spec file may live where no runner collects it.** `a` an uncollected file is flagged, `b` a fully-collected tree passes, `c`/`d` the Playwright roots (`tests/e2e/**`, `tests/smoke/**`) are exempt while `tests/infra` and `tests/extraction` are **not** — `tests/` is no blanket exemption; `e` every exemption names its owning runner, because one without a reason is indistinguishable from a silenced orphan; `f` the failure message says the tests **DO NOT RUN**, or it reads as a layout preference; `g` the real repository has no orphan, pinning `tests/web/` and `tests/images/` by name. Implemented in `tests/infra/test-locations.spec.ts` and `tools/check-test-locations.mjs`. ⚠ The defect it exists for is in `specs/testing.md` §11 and product invariant 21: six test files were pointed at directories no Vitest project collects, and a canary asserting `1 === 2` was reported inside a fully passing run. | §11, invariant 21 |
| **`T-CI-010`** | S | **The Dependabot notices workflow cannot execute dependency code.** `a` it never installs anything; `b` it runs `tools/check-licences.mjs` directly and that script imports only `node:` built-ins; `c` it is gated on the author being `dependabot[bot]` **and** on the head branch being in this repository, not a fork; `d` mutation-proves the install matcher catches a planted `npm ci` while leaving a plain `node` step alone; `e` the token is `contents: write` and **not** `pull-requests: write`; `f` it asserts the dirty set is exactly `THIRD-PARTY-NOTICES.md` before pushing; `g` it is neither scheduled nor an auto-merge. Implemented in `tests/infra/dependabotNotices.spec.ts`. ⚠ **The workflow holds a write token on a `pull_request_target` trigger — the classic Actions escalation shape.** It is safe for exactly one reason: it never installs, so no `postinstall` from the bumped package ever runs. ⚠ **That reason is one line from being untrue and the line looks like a fix** — a maintainer adding `npm ci` "so the script has its dependencies" (it has none) converts a file-copy into arbitrary code execution holding `contents: write`, and `T-CI-006`, `T-CI-005d` and lint would all still pass. | `specs/security.md` |
| **`T-INFRA-003`** | S | **Ingress refuses plaintext**: the committed Bicep has no ingress violation, `allowInsecure` is `false`, and `c` mutation-proves that turning it on is caught. Implemented in `tests/infra/infra.spec.ts`. Beyond confidentiality this is functional — product invariant 19: `navigator.clipboard` does not exist on `http://`, so the primary ingest affordance simply vanishes over plaintext. | `specs/security.md` §2 |
| **`T-SEC-006`** | U | **The owner id comes from the authenticated principal, never from the request.** `c` drives `GET /api/removed?ownerId=o_somebodyelse` and asserts the store was called with the principal's id. Implemented in `apps/api/test/unit/routes/removedRoute.spec.ts`. A query-supplied owner id is the whole allow-list defeated by a URL, and every other auth test still passes. | `specs/security.md` §3 |
| **`T-UNDO-005`** | I | **US-033 · a refusal writes NOTHING.** After a mixed-undo refusal returns `409`, the owner partition is identical: the list snapshot is unchanged, the `batchChange` count is unchanged, and the batch is still `applied`. Implemented in `apps/api/test/integration/batchUndo.spec.ts`. The enumeration is read-only by construction; a refusal that partially applied would be the one failure mode undo exists to prevent, and it would leave no record of itself. | `docs/PRD.md` US-033 |
| **`T-UX-003`** | U | **The offline state, asserted on every route that can mutate** (§2.12, §4.11, §7.11, §9.11): `a`–`e` on `/batches`, `f`–`g` on `/suppressed`, `h`–`l` on `/removed`. Mutating controls are disabled; the reason is **visible DOM text, not a `title=` or `aria-label`**; cached rows stay readable so browsing is never blocked; a request already **in flight** when the connection drops keeps its submitting state and invents no offline reason; and `Refresh` is disabled because it would trigger a document reload. Implemented in `apps/web/test/batchesOffline.spec.tsx`, `suppressedPage.spec.tsx` and `removedPage.spec.tsx`. | `specs/ux-states.md` §2.12, §4.11, §7.11, §9.11 |
| **`T-UX-050`** / **`T-UX-051`** | U | §5.1/§5.2 — a `submitted` batch reads as **queued** with both counts, an `extracting` batch reads as **running**, and the two are not interchangeable. Implemented in `apps/web/test/batchStatusPage.spec.tsx`. | `specs/ux-states.md` §5.1, §5.2 |
| **`T-UX-052`** | U | §5.4 — a batch that has finished extracting **navigates to that batch's review**, and a still-running batch does **not** navigate. Implemented in `apps/web/test/mutatingFlows.spec.tsx`. The negative case is the one that matters: an eager navigation lands the owner on a review of nothing. | `specs/ux-states.md` §5.4 |
| **`T-UX-055`** | U | §5.7 — an `IMAGES_PURGED` batch offers **"Upload new screenshots"** and **never a retry** (US-034 AC-5). Implemented in `apps/web/test/batchStatusPage.spec.tsx`. The screenshots were deleted by the 30-day lifecycle purge (`NFR-019`), so a retry is an action that cannot succeed however many times it is pressed. | `specs/ux-states.md` §5.7 |
| **`T-UX-056`** | U | §5.8 — offline **while polling**: the banner appears, the poll **pauses** and resumes on reconnect **without waiting for the next tick**, a request already in flight when the connection drops **invents no error**, and an genuinely online failure is still reported. Implemented in `apps/web/test/mutatingFlows.spec.tsx`. | `specs/ux-states.md` §5.8 |
| **`T-UX-057`** / **`T-UX-058`** | U | §5.9/§5.10 — the degraded-reader banner **reaches the review page** and is **not dismissible**; §5.10 is the milder consequence, so removals are **still offered** when only the cross-check reader was unavailable; and a healthy read carries **no banner**. Implemented in `apps/web/test/reviewPage.spec.tsx`. Pairs with `T-AI-036`, which asserts the pipeline half: under §5.9 removals are withheld entirely, because an incomplete read would propose removing titles that are still on the list. | `specs/ux-states.md` §5.9, §5.10, ADR-0001 R2 |
| **`T-UX-064`** | U | §6.12 — while the close is in flight the apply control **states it is applying**, **discard is disabled too**, a second press issues **no second close**, a failed close **re-enables** the retry, the removal dialog **survives its own confirm** with controls disabled, and a `409 REMOVALS_NOT_CONFIRMED` **re-opens the dialog enabled**. Implemented in `apps/web/test/reviewApplying.spec.tsx`. The double-press case is the reason this exists: a full-update close is one transaction over one service, and a second one is not idempotent from the owner's side. | `specs/ux-states.md` §6.12 |
| **`T-UX-070`** | U | §7.1 — the removed view renders **row skeletons** while loading, rather than an empty list that reads as "nothing was ever removed". Implemented in `apps/web/test/removedPage.spec.tsx`. | `specs/ux-states.md` §7.1 |
| **`T-UX-080`** / **`T-UX-081`** / **`T-UX-082`** / **`T-UX-083`** / **`T-UX-084`** | U | The suppressed view, states §8.1-§8.6: skeletons while loading (not an empty list); an empty state that explains **nothing is marked not interested** and points at the `⋮` menu; `Back` as **router navigation, not a document reload**; populated rows rendered from `displaySnapshot`; a submitting state that dims **only the affected row**; and success that **restates the un-suppression caveat** — *"This doesn't bring back anything that was removed"* (US-029 AC-4) — as a status rather than dropping it. Implemented in `apps/web/test/suppressedPage.spec.tsx`. | `specs/ux-states.md` §8.1-§8.6 |
| **`T-UX-095`** / **`T-UX-096`** | U | §9.6/§9.7 — the initiating card shows **"Undoing…"**, is disabled, and **other cards stay actionable**; a second click fires **no second undo**; success renders the counts **from the response** with honest pluralisation at one, and a link to `/`; and both `parseUndoResult` and `formatUndoneSummary` are asserted directly, so the sentence has a single rule. Implemented in `apps/web/test/undoOutcomes.spec.tsx`. | `specs/ux-states.md` §9.6, §9.7 |
| **`T-UX-098`** | U | §9.10 — a `409 BATCH_ALREADY_UNDONE` shows the **settled-fact** message with a refresh, kept **distinct** from a failure: `isBatchAlreadyUndone` recognises only that code, an unclassified failure keeps the batch **retryable**, a non-`ApiError` network fault routes to the same retryable state, the retry is a **real button**, it re-issues undo with the **same batch id** against a two-batch history, and a successful retry lands in §9.7 with the server's counts. Implemented in `apps/web/test/undoOutcomes.spec.tsx`. Conflating "already undone" with "undo failed" invites the owner to retry a settled fact for ever. | `specs/ux-states.md` §9.10 |
| **`T-UX-100`** | U | **REQ-105 · the row menu is a CHILD of its own row.** Opening the `⋮` on a row asserts the `role="menu"` is inside that row's `<li>`, that it is **not** a sibling of the list, and that opening a second row's menu moves the mount rather than leaving two. Implemented in `apps/web/test/rowDefects.spec.tsx`. This is the defect the owner reported: the menu was mounted at page level, so it rendered at the bottom of the screen instead of beside the title it acts on. ⚠ The DOM-ancestry assertion is the whole test — jsdom performs no layout, so any position assertion would pass vacuously. | `specs/ui-refresh.md` §3.1 |
| **`T-UX-101`** | U | **REQ-105 · the menu is anchored, not floating.** `.title-row__actions` carries `position: relative` and `.row-menu` carries `position: absolute` with `right: 0` and a `top` clear of the trigger, asserted by reading `apps/web/src/index.css` as a file. Implemented in `apps/web/test/rowDefects.spec.tsx`. A child that is not positioned is still laid out in flow, which reproduces the reported bug with the correct DOM. | `specs/ui-refresh.md` §3.1 |
| **`T-UX-102`** | U | **REQ-106 · the row meta line is one wrapping line with separators.** `.title-row__meta` is a wrapping flex row and a `span + span::before` rule supplies the `·`. Implemented in `apps/web/test/rowDefects.spec.tsx`. ⚠ Asserted as a CSS rule, not as text: jsdom does not render `::before`, so the character never reaches `textContent` and a text assertion would pass whether or not the rule exists. | `specs/ui-refresh.md` §3.2 |
| **`T-UX-103`** | U | **REQ-106 · the meta fields are ordered year → type → genres**, matching `specs/ui.md` §2.2, asserted on the rendered row. Implemented in `apps/web/test/rowDefects.spec.tsx`. | `specs/ui.md` §2.2, `specs/ui-refresh.md` §3.2 |
| **`T-UX-104`** | U | **REQ-107 · the actions column is top-aligned.** `.title-row__actions` carries `align-items: flex-start`, so the `⋮` sits level with the title rather than centring itself against a two- or three-line body. Implemented in `apps/web/test/rowDefects.spec.tsx`. | `specs/ui-refresh.md` §3.3 |
| **`T-UX-105`** | U | **REQ-108 · the row body has breathing room**, `--space-2` rather than `--space-1`, and the menu shadow is a `:root` token. Implemented in `apps/web/test/rowDefects.spec.tsx`. ⚠ The token exists because `T-CSS-003c` forbids an `rgb()` literal outside `:root` — a shadow written inline would have evaded `T-CSS-003b`'s hex rule by changing notation. | `specs/ui-refresh.md` §3.4 |
| **`T-UX-106`** | C | **REQ-109 · a corrected card shows the identity the owner chose** — corrected name, corrected poster, corrected year — and the **rejected** name is gone from the card entirely. Implemented in `apps/web/test/correctedMatchVisible.spec.tsx`. ⚠ Every fixture keeps the rejected guess in `alternatives`, because that is the fact the old code fell back to; with an empty alternatives list these cases pass over the defect. ⚠ `f` re-asserts that the raw extracted text is still beside the match — REQ-109 must not cost `T-REV-013`, the evidence that makes a correction checkable. ⚠ `g` asserts the **display fields reach the handler**: without them the server stores no name and the whole requirement silently reverts while every other case still passes. | `specs/ui-refresh.md` §3.5 |
| **`T-UX-107`** | C | **REQ-109 · the correction survives a re-render from SERVER state.** Implemented in `apps/web/test/correctedMatchVisible.spec.tsx`. ⚠ **This is the load-bearing case and it must not click.** The original defect was invisible in the click path — the component held the name locally and read correctly — and appeared only on a render driven by the refetched payload, where the name became `null` and the copy fell back to a generic sentence. These cases mount with `disposition: 'corrected'` already set, i.e. exactly what the server returns after the PATCH. ⚠ `d` pins the **honest degradation**: a correction the server has no name for still reports itself, unnamed, rather than interpolating an empty string. | `specs/ui-refresh.md` §3.5 |
| **`T-UX-108`** | C | **REQ-109 · the correction is announced in a live region**, and the announcement **names** the corrected title. Implemented in `apps/web/test/correctedMatchVisible.spec.tsx`. ⚠ A live region saying "Matched to the title you chose." announces that something happened without saying what — for a screen-reader user that is the same defect sighted owners hit, with no poster to fall back on. | `specs/ui-refresh.md` §3.5 |
...
| **`T-UX-120`** | U | **REQ-037 · runtime complete orders** use Longest runtime / Shortest runtime (`apps/web/test/sortControl.spec.tsx`). Inactive runtime selects `desc`; active reverses. Returning to dateAdded removes the default sort key. Both values are URL-owned; #328 remembers them together at the navigation boundary. |
| **`T-UX-121`** | U | **REQ-119 · runtime display** is `1h 55m` for film and `45m/ep` for TV, before genres in `Year · type · runtime · genres` (`apps/web/test/titleRow.spec.tsx`, `packages/domain/test/titleRuntime.spec.ts`). `/ep` remains mandatory: the stored TV runtime is one episode, not the whole series. |
...
| **`T-UX-123`** | U/I | **REQ-035 · the runtime bucket filter** (`apps/web/test/filterBar.spec.tsx`, `packages/domain/test/titleRuntime.spec.ts`, and `apps/api/test/integration/titleRuntime.spec.ts`). Selecting a bucket writes `runtime`; half-open boundaries put 60 in `60-90`, 90 in `90-120`, and 120 in `over120`. An inclusive upper boundary would double-count titles. Null/zero/negative runtime satisfies no bucket, separately asserted from the known-runtime partition. Unknown tokens are dropped client-side but rejected by the API (`T-API-021`); chips use labels, not tokens. Cases `o`/`p` pin canonical-only membership and legacy `60-120` expansion/deduplication. |
| **`T-UX-124`** | U | **REQ-035 · the hidden-unknown disclosure** renders with the SERVER's count while a runtime filter is active, and not at all without one (`apps/web/test/filterBar.spec.tsx`). ⚠ **This is product invariant 2 in a new place** — nothing leaves the owner's list without telling them: a runtime filter silently drops every title TMDB never supplied a runtime for, and without this line the list just gets shorter. ⚠ **The count cannot be computed in the browser**: the excluded rows were never sent, so anything derived from `items` counts what *survived* the filter — it would read `0` on every list and look right in every fixture. ⚠ `0` and `null` both render nothing but are **different facts** and are asserted separately. ⚠ `role="status"`, because a sighted owner sees the list shrink and a screen-reader user gets no other signal. |
...
guess which task owns each criterion — and a guess there reproduces the
mis-citation class this project has already hit seven times. ⚠ **The baseline
is a ratchet: it may only shrink.** `T-META-006e` fails if it grows, because
"add it to the list" is otherwise always the cheapest way to make the gate
pass, and that reinstates exactly the silence it exists to break.
`resolvedBaselineIds()` reports which entries have become owned and can be
struck.

⚠ **`T-META-006e` asserts the size EXACTLY, and it does so because the
inequality form failed here.** It was written as `<= 60`, the count at the time,
and the baseline was then worked down without the ceiling following, leaving
34 free slots that a later change used while CI stayed green. A high-water mark
is not a ratchet. It is now pinned, which also fails on an unrecorded shrink —
the same both-directions discipline `KNOWN_UNMAPPED` and
`KNOWN_PHANTOM_CITATIONS` use.

⚠ **There is exactly one legitimate reason the baseline may grow: a sharpened
detector.** It has happened once. `implementedTestIds()` credited an id that
appeared **anywhere** in a `.spec.*` file, and several gates record their known
gaps as arrays of id literals inside such files — so writing down that a test
was missing is what made the id look implemented. Nine ids were hidden that
way. The predicate now requires **title position** (`T-META-006i`), and the
nine carry their justification at the point they are listed. Growth for any
other reason is the failure this section describes.

⚠ **`BASELINE_ORPHANS` and `KNOWN_PHANTOM_CITATIONS` are coupled.**
`T-META-001e` subtracts the orphan baseline before comparing, because an id no
task cites and no test implements is already tracked by name in the other list.
Moving an id between them is therefore not a gap closing; it changes which gate
reports it. This is only safe because **both lists are pinned exactly**, so
neither can absorb an entry quietly.

~~The 63 are listed in `BASELINE_ORPHANS` so the gate can fail on anything
**new** without blocking on a 63-way assignment.~~ Superseded: the count is
maintained in code, and repeating it here guaranteed it would go stale.
...
guard - `specs/api.md` §5.0 named ~~`apps/api/src/images/pixelGuard.ts`~~ and
...
that the spec says is somewhere else.~~ **It was NOT settled before it was
built.** TASK-145 shipped as `apps/api/src/images/decodeGuard.ts` plus the pure
`packages/domain/src/pixelGuard.ts`, and `specs/api.md` kept naming a third
path that has never existed - exactly the outcome this paragraph warned of.
Both specs are now corrected in place, and `T-INFRA-014` fails the build on any
spec path whose file demonstrably lives somewhere else, so the warning no
longer depends on a human noticing.
...
**(a) Module naming - resolved, not guessed.** `specs/api.md` §5.0 names ~~`apps/api/src/images/pixelGuard.ts`~~ (corrected in
place: §5.0 now names `decodeGuard.ts`, matching this resolution) for the guard, §5.0.1 names
...
| `T-IMG-010k` `T-IMG-010l` | U | **Multer's own limit breaches are mapped into the closed error enum, never left to the envelope's catch-all.** An image past the `fileSize` backstop is **413 `IMAGE_TOO_LARGE`**, not 500 `INTERNAL_ERROR` — a ceiling the product deliberately enforces must not be reported as nextup having broken, because `INTERNAL_ERROR` is unactionable on the one screen where the owner is adding data. `l` re-runs the same assertion with `ingestSource=paste`: ceilings are never conditioned on the client-declared source (product invariant 17). ⚠ Found by the TASK-163 ingest-parity suite, not by this file. |
...
| `T-PASTE-007a`-`e` | U + I | Every ceiling applies identically to pasted images, including request-file-count refusal and empty-request validation. |
...
| `T-DATA-004` | U | A 401 redirects to `/.auth/login/aad` with the current path as `post_login_redirect_uri`, and renders no error. <br> ⚠ **AN EXPIRED SESSION DOES NOT ARRIVE AS A 401 IN THIS DEPLOYMENT, SO `a`-`c` GUARDED A BRANCH THAT NEVER FIRED FOR THE OWNER.** Container Apps Easy Auth runs with `login.tokenStore.enabled` **false** (`infra/aca.bicep` — a token store would persist C3 identity material and also requires a configured blob store, so enabling it without one breaks sign-in). Without the store an unauthenticated request is **not rejected**: it falls through to the app, which serves the SPA's `index.html` with a **200**. `response.ok` is true, the 401 branch is skipped, and `response.json()` throws a `SyntaxError` on `<!doctype` — surfacing as a generic *"something went wrong"* with a Retry that fails identically forever and **no mention of signing in**, the one remedy that works. `d` pins the fix: a 200 whose body is HTML is treated exactly as a 401, path preservation included. ⚠ **`e` pins the SIGNAL, because the obvious implementation is wrong and looks right** — Easy Auth does not redirect here, so `response.url` is still the API path and `response.redirected` is `false`; a check built on either reads as *authenticated* during exactly the failure it exists to catch, and would pass a test that hand-builds a redirected `Response`. `e` therefore asserts detection on a response carrying **no redirect information at all**. ⚠ **`f` is the bound and matters more than it looks**: claiming an expired session for *any* malformed 2xx would send a correctly signed-in owner to the IdP and return them to the same broken screen — an infinite loop through a login they never needed, with the real defect hidden behind it. Only HTML is proof the API did not answer; every other non-JSON body stays a shape failure for the `ErrorBoundary` (§1 *"Never a blank page"*). ⚠ **`g` guards the ORDERING, not the predicate, and its comment first claimed otherwise** — a 204 returns *before* the content-type check, so broadening that check to "not JSON" does not fail `g` (mutation failed only `f`). What `g` really pins is the 204 early return staying **above** the check: a 204 carries no content type, so moving the check up sends every successful delete to the sign-in page while every other case stays green — proven by exactly that mutation, which fails `f` and `g`. Mutation-proven: removing the branch fails `d` and `e`; broadening it to any non-JSON body fails `f`; detecting via `response.redirected` fails `d` and `e`; hoisting the check above the 204 return fails `f` and `g`. | REQ-098 |
...
### 36.2a The `T-DATA-002` sub-ids

`T-DATA-002` is asserted once per data screen, in that screen's container
spec. Each container gets a contiguous block:
...
**`T-DATA-002j` / `T-DATA-002k` (list, `apps/web/test/listRoute.spec.tsx`) — the removed count.** `j`: an unfiltered list that comes back **empty while removals exist** renders the `all-gone` empty state ("Nothing on your list right now"), not `never-uploaded` ("Nothing here yet"), and the removal-history link carries the **real number**. ⚠ This shipped wrong. `ListRoute` never passed `removedCount`, so it took its default of `0`, and an owner who had removed titles and suppressed none was told nothing had ever been on their list - the data-loss misreading US-019 AC-5 exists to prevent. The container carried a long comment calling the gap known and deliberate: the count was available (`GET /api/removed` shipped with TASK-099) and the stated blocker was **cost**, a fourth request on every list load on one 0.25 vCPU replica. That objection was real but self-answering - "it only matters when the list is empty" **is** the fetch predicate - so the read is now conditional on the unfiltered list being empty. `k` is what holds that in place: on a non-empty list `getRemoved` must **not** be called. ⚠ Note what no gate could see here: `ListEmptyState` was fully tested and correct, and `listEmptyKind` had a case for exactly this input; the defect was entirely in the **wiring**, which only a test rendering the CONTAINER can reach - the same shape as `T-DATA-002ac` below and as the `undoBatch` dead-call in `T-INFRA-013`.
...
| **`T-WAIT-002`** (`a`–`d`) | I/U | `TASK-185` | Reconciliation never runs for a discovery batch. `a` closing a second capture of the same page that omits a previously-seen title proposes **no** removal; `b` **the discriminating case** — the same omission in a Netflix full-update batch *does* propose one, without which `a` would pass against a build where reconciliation is simply broken (US-040 AC-4); `c` `ck_batch_discovery_append_only` refuses a full-update discovery batch at the STORE, beneath the API refusal `T-WAIT-001b` asserts; `d` an undecided candidate still blocks the close with `PENDING_ADDITIONS` and opens no transaction — no accept-by-inaction on this path either. |
| **`T-WAIT-003`** (`a`–`g`) | I/U | `TASK-185` | A closed discovery batch creates **no** `ServiceListing` and leaves the combined list byte-identical before and after (US-040 AC-3, ADR-0010 Trap 3). `b` no service badge count changes (REQ-025). `c` a work already in the combined list produces no intent and is REPORTED (US-040 AC-5) — its comment records why `discovery.alreadyListed` is 0 rather than 1 there: the held work is caught by the review classification before it ever reaches the plan, and the in-transaction counter is the narrower race net. `d` a waiting work is invisible to **both** the combined list and the removed log — its title is stored `removed` with no listing, which is what keeps it out of `listActiveTitles` while owning no removal to show. `e`–`g` are the UNIT half, driven through the route with a stubbed store so the branches score against the `apps/api/src/**` coverage floor CI job 4 measures without a database: `e` an intent and no listing, `f` no service state is written (REQ-039 must not claim a Netflix list was refreshed by a storefront screenshot), `g` the title is created `removed` with a null sort date. |
| **`T-WAIT-004`** (`a`–`i`) | U | `TASK-185` | A work already present in the combined list produces **no** `WatchIntent`; the review pass says so rather than silently discarding it (US-040 AC-5). `b` every candidate lands in exactly one bucket, so nothing is lost; `c`/`d` an existing open intent, and two candidates for one work in one batch, each yield ONE intent — `ux_intent_owner_title_waiting` would otherwise roll the close back; `e` the review pass classifies a held work `already-in-your-list`. `f` an existing title is REUSED as it stands — promoting a re-discovered removed title back to `active` would put it in the combined list with no listing, the one thing this path exists not to do; `g` a suppression is re-checked INSIDE the transaction, as the service path does. `h` end to end, a held work yields no intent while its batch-mate does — and `h`'s comment records WHY `discovery.alreadyListed` is 0 there rather than 1: the held work is caught earlier by the review classification, which is the reporting AC-5 actually asks for, and the in-transaction counter is the narrower race net only an engine can exercise. `i` a work already waiting yields no second intent. |
| **`T-WAIT-005`** (`a`–`e`) | U | `TASK-186` | Every extracted candidate is shown and every disposition defaults to `pending` — no accept-by-inaction (US-041 AC-1, REQ-014). `a` the response carries `service: null` and names the discovery source; `b` every extracted candidate routes to a section; `c` already-listed work remains visible (now also true for service captures); `d` every disposition defaults to `pending`; `e` removals are omitted and the label never names a service. |
| **`T-WAIT-006`** (`a`–`k`) | I + U | `TASK-186` | **Discard suppresses.** `a` discarding a discovery candidate creates a `Suppression` on canonical work identity; `b` **the load-bearing behavioural test** — capture the same page twice with a discard in between, and the second review pass does not contain it *at all*, the check being before record creation (US-041 AC-2/AC-3/AC-4, US-028 AC-2); `c` **the discriminating case** — discarding in a *Netflix* review pass does **not** suppress, so the behaviour is scoped to discovery sources (US-041 AC-5). `a`–`c` are integration (`apps/api/test/integration/discoveryCurate.spec.ts`); `d`–`k` are the pure selection rule in `discardedWorks` (`packages/domain/test/close.spec.ts`) — `d` the key is the work identity and never the row id, `e` confirmed/pending/corrected yield nothing, `f` a discard is collected from every section, `g` a candidate with no resolved identity yields nothing (the review gate could never act on it), `h` a collapsed SD-02 loser yields nothing, `i` two tiles for one work are one suppression, `j` the display snapshot is frozen, `k` the name falls back to what was read. |
| **`T-WAIT-007`** (`a`–`b`) | I | `TASK-186` | Suppression failure during close rolls back the whole batch; no partial curation is committed (US-041 AC-6). `a` the injected failure leaves no suppression, no intent, no title and the batch still `in-review`; `b` the same batch closes cleanly once the fault clears, so a rollback leaves a retryable batch rather than a wedged one. |
| **`T-WAIT-008`** (`a`–`c`) | I | `TASK-189` | Graduation. `a` a waiting work later captured on Netflix enters the combined list by the ordinary path and its intent leaves the waiting view — **and a batch-mate's unrelated waiting intent is left `waiting`**, without which `a` passes against a build that satisfies every intent on every close; `b` the satisfied intent is **retained, never hard-deleted**, with its discovery provenance intact (US-043 AC-3/AC-5, REQ-028); `c` a capture containing no discovered work graduates nothing, so the empty-set call on every service close is a no-op rather than a blanket update. |
| **`T-WAIT-009`** (`a`–`c`) | U | `TASK-189` | "Not interested" on a waiting work suppresses on canonical work identity like any other work (US-043 AC-4, REQ-070/071). `a` **the assertion is the CALL** — the row's `titleId` goes to the same `suppressTitle` endpoint every other surface uses, because a waiting-specific client path is a second thing that can disagree with the first; `b` the row leaves the view on success; `c` a failure keeps the row and offers a retry rather than dropping it optimistically. |
| **`T-WAIT-010`** (`a`–`e`) | E + U | `TASK-189` | The empty waiting view explains what it is for and how to fill it (US-043 AC-6). `a` (E) in a real browser the empty state names the view and its action is a **reachable link to `/upload`**, not prose; `b` (U) the same in jsdom; `c` a **load failure** renders an error with a retry rather than the empty state — the two look identical on screen and mean opposite things; `d` the loading state is announced rather than silently empty; `e` (E) **the discriminating half** — a populated view shows the row instead of the empty state, without which `a` passes against a build whose fetch never lands. |
...
| **`T-WAIT-012`** (`a`–`e`) | U | `TASK-126` | A discovery capture runs the **same** ingest, extraction, cross-check and TMDB matching path as a service capture — there is no parallel pipeline (US-040 AC-2, REQ-082, ADR-0010 D-1). ⚠ **PROVEN STRUCTURALLY, NOT BEHAVIOURALLY**: running one screenshot through both batch kinds and diffing would show only that two paths agree on one input, which two separately-maintained pipelines would also do. `a` no module on the extraction path reads any discovery/service discriminator, so it cannot branch on one; `b` `startExtraction` accepts `(ownerId, batchId, deps)` and no source, so no caller can select a variant; `c` both `beginExtraction` call sites in `routes/batches.ts` are unconditional and unguarded; `d` no rival pipeline module exists under `apps/api/src/jobs`; `e` **the positive control** — the discriminator is still read at batch creation and close, so `a` cannot pass by the discovery concept having been deleted. `tests/infra/discoveryPipelineParity.spec.ts` |
...
| **`T-AVAIL-003`** (`a`–`d`) | I + U | `TASK-188` | A work reported `flatrate` on a `SERVICES` member is **flagged with an invitation** and is **not** added to the combined list. `a` (I) the flag appears, the work is absent from `GET /api/titles`, and **no `ServiceListing` is invented**; `b` (I) **the load-bearing negative** — combined-list membership *and ordering* are identical before and after the refresh, compared against a list seeded with three real listings so the ordering claim is not vacuous as it is in `T-AVAIL-004b`; `c` (U) the rendered flag names the service and links to `/upload`, and the row stays in the waiting view; `d` (U) a rent-only answer is not flagged (US-042 AC-3, ADR-0010 §4). |
...
| **`T-AVAIL-006`** (`a`–`e`) | U | `TASK-188` | Absent provider data renders *"not seen on your services as of &lt;date&gt;"*, never *"not streaming anywhere"* — a claim the data cannot support (US-042 AC-6, ADR-0010 Trap 4). `a` NOT KNOWN (`null`) and `b` ASKED-AND-NOBODY (`[]`) render the **same bounded sentence**; `c` **the load-bearing negative**, asserted against the whole document across all three data shapes — the forbidden claim appears nowhere; `d` a never-checked row says so instead of inventing an as-of date; `e` the rule is asserted directly against the exported `availabilityLine`, because the claim under test is which sentence is CHOSEN and a DOM query passes just as happily against the wrong one. |
| **`T-AVAIL-007`** (`a`–`c`) | I + U | `TASK-188` | TMDB unreachable: the view renders from last-known availability with its as-of date plus an unobtrusive failure note. Never blank, never an error page (US-042 AC-7). `a` (I) a 200 carrying the last-known answer, its original as-of date, `availabilityRefreshFailed: true` — **and an untouched store**, because a failed lookup that wrote `null` would erase a known-good answer; `b` (I) **the discriminating half** — a wholly successful render does not raise the note, without which `a` passes against a hard-coded `true`; `c` (U) the note renders alongside the last-known rows rather than replacing them. |
...
| **`T-AVAIL-009`** (`a`–`b`) | E + U | `TASK-188` | Every surface rendering availability carries the **JustWatch** attribution (US-042 AC-9, REQ-087). `a` (E) in a real browser, on **both** the populated and the empty waiting view, with a non-trivial painted box — quiet is allowed, hidden is not, and rendering it beside the rows (the natural, wrong implementation) fails on the empty screen only; `b` (U) the same unconditional presence in jsdom. ⚠ Like the TMDB obligation, its absence is invisible from inside the product: nothing errors, nothing logs, the screen looks healthy. |
...
| **`T-AVAIL-012`** (`a`–`c`) | U | live incident, 2026-09-18 | **A serverless database that is WAKING UP must not be reported as a database that is BROKEN.** ⚠ **A REGRESSION TEST FOR A LIVE INCIDENT, NOT A HYPOTHETICAL.** Staging is Azure SQL serverless with `autoPauseDelay = 60` and is auto-paused ON PURPOSE (ADR-0003 Rev 3) so an idle deployment bills nothing. The first request after a pause **triggers a resume taking ~30–60 s**, and everything arriving in that window is refused. `mssql` defaults `connectionTimeout` to **15000 ms — shorter than the resume** — so the pool gave up before the database was ever ready, every time. Observed: `resumedDate` 12:54:34Z, with `titles`, `serviceState` and `suppressions` all failing `Failed to connect … in 15000ms` from 12:54:36 to 12:55:44, then recovering unaided. The owner saw **"Couldn't load your list. Nothing has changed."** above a Retry button that *could not* work, because every retry hit the same 15-second wall. ⚠ **"Nothing has changed" is indistinguishable from data loss to the person reading it**, which is why a cold-start false alarm is not cosmetic in this product. `a` the connect timeout outlasts a resume — asserted as *greater than the 15000 ms default* **and** ≥ 60 s, so it tests the property rather than restating the constant; `b` the pool's acquire timeout **exceeds** the connect timeout, without which `a` buys nothing (both default to 60000 ms, so a connection completing near the limit races the pool giving up and the caller gets a pool timeout naming nothing about a resuming database — the inner timeout must always report first); `c` both timeouts apply on the **managed-identity** path too, since `buildMssqlConfig` returns EARLY on the SQL-login branch and the database that actually pauses is the managed-identity one. ⚠ `c` **asserts the values are `number` BEFORE comparing them** — an equality check alone is satisfied by `undefined === undefined`, so it would otherwise report PASS while `a` and `b` failed, a parity test agreeing both paths are equally broken. All three mutation-proven: removing the two settings fails `a`, `b` and `c`. ⚠ **The trade-off is deliberate**: an unreachable database now takes ~60 s to say so. A paused database is the EXPECTED, billed-for state of staging, so the 15-second answer was wrong far more often than right — and wrong in the worse direction. |
...
| Id | Level | Owner | Claim |
|---|---|---|---|
| **`T-WAIT-013`** (`a`–`e`) | U | `TASK-251` | The rental storefronts are discovery sources and never services. `a` every storefront is forced append-only and refuses a full update; `b` **every label carries "(rent/buy)"** and differs from every service label; `c` no storefront slug is a `SERVICES` member (D-1); `d` `search` is an intent source, never a batch source; `e` `intentSourceLabel` names every stored source and never throws. |
| **`T-WAIT-014`** (`a`–`d`) | U | `TASK-251` | Migration 0017 is the approved widening and nothing more. `a` the migration gate authorizes only the complete approved file at its exact path (pinned by hash); `b` the widened CHECKs hold exactly the domain enums and are installed before any drop; `c` a batchless intent is allowed **only** for `search`, and the new columns are guarded; `d` weakening, reordering or appended SQL is rejected. |
| **`T-WAIT-015`** (`a`–`g`) | U | `TASK-251` | `POST /api/waiting` (search-to-add), handler arm with store and TMDB mocked. `a` a new work is recorded as a batchless `search` intent on a `removed`, dateless title; `b` a suppressed work is refused before TMDB is asked; `c` a work already in the library is refused (`already-listed`) and nothing is written; `d` a work already waiting is refused (`already-waiting`), never doubled; `e` two racing adds: the store's unique-index refusal is the same 409; `f` an existing removed title is reused as it stands; `g` bad bodies are 400 and an unknown TMDB work is 404. |
| **`T-WAIT-016`** (`a`–`d`) | I | `TASK-251` | Against the real store: `a` search-add leaves the combined list untouched and the row reads `rent-only`; `b` a duplicate is refused, not doubled; `c` **the store itself** refuses a search intent with a batch, a storefront intent without one, and rent offers without an as-of date; `d` the three new storefronts are accepted as batch sources and `prime-video` is not. |
| **`T-WAIT-017`** (`a`–`d`) | U | `TASK-251` | The storefronts are importable from `/upload`: `a` behind a toggle, apart from the services, each labelled "(rent/buy)"; `b` choosing one answers the mode append-only and disables full update; `c` a `?source=` preselect opens them with the answer given; `d` a storefront batch is labelled by its source, never "Discovery". |
| **`T-WAIT-018`** (`a`–`b`) | E | `TASK-251` | In a real browser: `a` the empty waiting view's action opens the storefront import with Fandango at Home (rent/buy) chosen, full update disabled, and no overflow at 280px; `b` every access state reads in words, the row on the owner's service leads with its import link, and nothing overflows at 280px. |
| **`T-WAIT-019`** (`a`–`d`) | U | `TASK-251` | Search-to-add on `/waiting`: `a` a result is waited for only by an explicit press; `b` every refusal names the fact that stopped it; `c` a refusal is an alert and the result stays; `d` without both handlers there is no search box. |
| **`T-WAIT-020`** (`a`–`g`) | U | `TASK-257` | #382, the waiting view in the Library look (`specs/ui.md` §7b). `a` one `h1` with the helper as its subtitle, no heading on the search, row names the only `h2`s; `b` a `year · type` meta line whose separators stay out of the text; `c` the forecast headline carries an *Estimate*/*Announced* tag and the when (no service mark since #389), is `aria-hidden`, and the sentence under it still leads with "Estimate:"; `d` *Not interested* is a ghost control in the card's panel column (`.waiting-row__aside`, the card footer until #389) whose accessible name includes the title; `e` the search is Library's field — visually hidden label, icon submit named *Search*, inside `.list-search__control`; `f` no raw ISO date anywhere on a row (discovery and rent-only note checked); `g` the stylesheet pins the catalogue surface, 2:3 posters at 4.5rem/6rem and, since #389, the three-zone row (poster, body, aside) from 1024 px ~~and two columns from 1024 px~~. `apps/web/test/waitingRestyle.spec.tsx` |
| **`T-WAIT-021`** (`a`–`c`) | E | `TASK-257` | In a real browser on both projects at 320, 640 and 1280 px: one page heading and a visible subtitle, poster width 72 px below 640 px and 96 px from it, the forecast headline visible with its tag, every *Not interested* at least 44 px tall and under 60% of its card's width, one card per line with the panel beside the details from 1024 px and under them below it (#389; ~~cards side by side only from 1024 px~~), and no horizontal overflow. `tests/e2e/waiting.spec.ts` |
| **`T-WAIT-022`** (`a`–`d`) | U | `TASK-258` | #389, each card says each thing once (`specs/ui.md` §7b). `a` the forecast panel has one tag and no service mark — the sentence names the service; `b` a streaming row says *Now streaming* once, in its panel with the flag, draws one mark, keeps its heading to the title and shows no availability line; `c` "(rent/buy)" is dropped from the discovery line only on a rent-only row (whose pill says it) and kept on every other row; `d` the stylesheet pins one card per line, the `aside aside` phone area, and a rounded, wrapping pill. `apps/web/test/waitingRestyle.spec.tsx` |
| **`T-WAIT-023`** (`a`–`d`) | U | `TASK-259` | #391, Grid and Compact on the waiting view (`specs/ui.md` §7b). `a` the Library's *List layout* switch is on the page, *Compact* pressed by default, and *Grid* sets `data-view=grid`; `b` a grid tile keeps the answer — the flag and its import link, the forecast sentence, the rent-only pill and the bounded availability line — and *Not interested*, and leaves the storefront chips, other services and discovery line to the details page; `c` the choice is kept in `nextup.waiting.layout.v1`, apart from the Library's, and restored on return; `d` the stylesheet pins wrapping `auto-fill` tiles with an `8rem` floor (two across at 320 px) and the stacked tile areas. `apps/web/test/waitingDetails.spec.tsx` |
| **`T-WAIT-024`** (`a`–`d`) | C | `TASK-259` | #391, the waiting details page `/waiting/:titleId`. `a` the title on a card links to it in both views; `b` it reads `GET /api/waiting` and `GET /api/titles/:titleId` and shows everything the Library page does (facts, synopsis, directors, cast, *Details*, trailer) plus the full waiting answer (rent pill, forecast, storefronts, other services, discovery) and both attributions, with no Library saved-title actions and a *Back to Waiting to stream* link; `c` *Not interested* suppresses the work and returns to `/waiting`, and a failure says so and retries; `d` a title no longer waiting says so, and a failed read offers *Try again*. `apps/web/test/waitingDetails.spec.tsx` |
| **`T-WAIT-025`** (`a`–`c`) | E | `TASK-260` | In a real browser on both projects: `a`/`b` Grid at 320 and 1280 px puts at least two tiles on a line, keeps the forecast and a 44 px *Not interested* on every tile, does not overflow, and survives a reload; `c` at 320 px a title opens its details page with runtime, director, cast, writers, the forecast and JustWatch credit, a 44 px trailer link to YouTube in a new tab with `noopener noreferrer` and no iframe, no overflow, and *Back* returns to the list. `tests/e2e/waiting.spec.ts` |
...
| **`T-AVAIL-015`** (`a`–`e`) | U | `TASK-251`, `TASK-257` | Each access state reads as what it is: `a` rent-only names the storefronts as chips in a list named *Rent or buy on*, under a *Rent or buy only* tag (the row's pill since #389), says "not streaming on your services yet" with a Library-style as-of date, and is never the flag — the redundant "(rent/buy)" no longer repeats the tag (#382); `b` rent-only with no named storefront, or no as-of date, falls back to Trap 4's sentence; `c` a service the owner does not use is said in words, apart from the flag — a known service as its mark, an unknown provider as a named chip; `d` a row on the owner's service is badged, dated (`3 Feb 2026`) and leads; `e` the source line says "(rent/buy)" for a storefront and names a search add, with a Library-style date. ~~Superseded at TASK-257: `a` asserted "(rent/buy)" and a comma-joined storefront sentence; `c` a comma-joined sentence; dates were raw ISO.~~ |
...
This register defines Epic P coverage and the owner-approved 2026-09-16
refinement. Existing test families retain their identities while obsolete
shape expectations are updated in place; `T-UX-138`–`143` and `T-API-030`
cover the refined controls and search. A definition here does not itself
claim a passing implementation; the runner must collect and execute it.
...
| `T-UX-111` | U | At 1280 px default Grid uses multiple columns of horizontal poster/details cards. `apps/web/test/listSurface.spec.tsx` pins the min-width grid rule and **REQ-111** poster bounds: uniform 2:3 ratio, minimum 72 × 108 and a missing poster retaining its box. No responsive override may shrink that floor | §4.1, §4.2 |
| `T-UX-112` | U | Grid and Compact expose the same row-menu items from **one ordered DOM tree**. No JS viewport branch in TitleList/TitleRow/RowMenu, no stylesheet hiding of actions, and the menu retains its positioned row ancestor. Explicit local view state selects density, while CSS handles widths; `T-UX-141` additionally checks data/order and restrictions | §4.1 |
| `T-UX-113` | U | The real `.list-controls` groups filters and sort; changing filters preserves direction. #328 replaces separate storage rules with a complete navigation bookmark, not mirrored live control state. `apps/web/test/sortControl.spec.tsx` | §5; #328 |
...
| `T-UX-115` | U | A saved complete destination restores its direction on bare entry; explicit query links use their own field/direction instead. Assert both URL and visible order. The direction-only session fallback is superseded by #328. | §5; #328 |
| `T-UX-116` | U | Oldest additions is one press on the selected Recently added button on the **real page**, without opening a disclosure. `T-UX-131` covers the isolated control; hiding the button on the page still deletes invariant 6 | §5 |
...
| `T-UX-118` | U | Opening Service updates exposes all service-preselected upload links (REQ-039, RSK-007), even when dates are unavailable. Upload remains a first-class phone destination; `T-UX-143` keeps unavailable-data notice outside the disclosure | §6 |
...
### 39.2a Approved refined-library contract (2026-09-16)

These supplement, rather than replace, the existing AC mappings above.
They introduce no new PRD AC numbers or weakened coverage baselines.
...
| `T-UX-142` | U | Approved dark indigo/violet tokens, Segoe UI/Aptos system stack, 10px/16px radii and reduced-motion behavior. Existing `T-CSS-004` computes contrast on actual surfaces, including dark surface foreground on accent fill; no network font or icon dependency. `apps/web/test/refinedLibrary.spec.tsx` | `ui-refresh.md` §7; US-052, US-059 |
| `T-UX-143` | U | Compact desktop List/Upload/Batches/More and phone List/Upload/More preserve one nav landmark and route reachability; Service updates exposes all factual links while unavailable-date notice stays visible outside. Closed 16-icon set includes Brand/Grid/Compact. `apps/web/test/refinedLibrary.spec.tsx` plus existing navigation/freshness/icon tests | `ui-refresh.md` §6–§7c; US-022, US-054, US-059 |

#### Pending-query component coverage

`apps/web/test/refinedLibrary.spec.tsx` additionally keeps the pending
transition observable rather than depending on an immediately fulfilled
fixture that can hide an unmount.

| Test | Level | Assertion |
|---|---|---|
| `T-UX-141h` | U | A service selection retains its open mounted picker and checked values through loading and response; pending result counts are absent and Done still restores trigger focus. |
| `T-UX-141i` | U | A selected genre remains available and checked in its still-open picker when the pending facet array is empty; pending result counts are absent. |

`countPending` also suppresses the server-derived runtime-hidden disclosure;
its normal nonpending semantics stay under `T-UX-124`. Failed online reads
still hide the filter/control group rather than rendering invented counts.

#### Real-browser refinement coverage

`tests/e2e/refinedLibrary.spec.ts` is collected by Playwright, not Vitest.
These browser cases supplement the component families above; a passing DOM
test does not establish computed contrast, dimensions or popup geometry.
...
| Test id | Layer | Asserts |
|---|---|---|
| `T-UX-144` | U/E | **REQ-035, US-019 AC-4 and US-055 AC-4.** `apps/web/test/filterBar.spec.tsx` cases `a`–`f` pin the Filter by group, external category labels, category-plus-value accessible names, decorative chevrons, default/single/deduplicated multiple summaries, stable open pickers, five runtime choices, legacy-link expansion and single-half chip removal, and clearing. `tests/e2e/refinedLibrary.spec.ts` case `g` verifies **two-column** fields at every width, bounded panels, 44px targets and keyboard focus at 320/640/1280px, both new options and canonical URL selection. ⚠ Case `g` asserted ~~two/three/six columns by viewport~~ until 2026-09-17; that codified the defect instead of catching it, because the fields live in a `26rem` panel whose width does not track the viewport. Existing `T-UX-141g` checks axe/contrast with the new fields. |
...
20-occurrence validation limit. `T-API-020j` exercises the two new halves
and legacy union against real SQL, retaining full-set unknown-runtime counts.

### Owner refinement: compact library controls (2026-09-17)

The search, filter and sort block consumed roughly half a 320 px viewport
before a single title was visible. The filter pickers and the six orders moved
into panels behind toolbar triggers (`specs/ui.md` §2.1 item 2, §10.1).

| Test id | Layer | Asserts |
|---|---|---|
| `T-UX-145` | U | **REQ-035, US-019 AC-4/AC-5.** `apps/web/test/filterBar.spec.tsx` cases `a`–`e`: the Filters trigger is a labelled disclosure for a dialog whose `aria-expanded` tracks the panel; the pickers are absent until it is clicked; the trigger carries a count of active filter **values** that excludes the `q` search term; choosing a service and pressing Done leaves the filter in the URL and returns focus to the trigger; Escape closes the panel without undoing a selection. ⚠ The chips, **Clear filters**, the live result count and the runtime-hidden disclosure stay **outside** the panel and keep their existing coverage — a count you can only see inside the control that changes it is not a count. |
| `T-UX-146` | U | **REQ-038, US-057 AC-3/AC-5/AC-7.** `apps/web/test/sortControl.spec.tsx` cases `a`–`i`: the toolbar states the complete current order with nothing open; the trigger is a dialog disclosure reporting its state; every chooser row names its field and spells its direction in words beside a decorative arrow (`T-A11Y-008`); choosing an order applies it, closes the chooser and restores focus; choosing the selected order reverses it; Escape and the close button dismiss without changing the order; **the reverse button never opens the chooser** (`h`); and the URL leads session storage (`i`, see US-057 AC-7). |
| `T-UX-147` | E2E | **REQ-035, REQ-038, NFR-008.** `tests/e2e/refinedLibrary.spec.ts` cases `a`–`g`, measured in a real browser because every claim is a layout fact jsdom cannot answer. *(Phone branches redefined in place by TASK-255: `b` and `g` measure the phone toolbar as two lines — the *My Library* heading with its tools, then sort, reverse and view — with the count in the heading; `c` puts the rating below the facts line beside the service marks; `e` runs at 640–1600 px, since below 640 px the filters are a sheet, not popovers; `f` taps a sheet chip at 390 px and keeps the popover half at 640 px.)* `a`: in Compact at 1280 px the rating, the badges and the priority control of **every** row share one left edge — each row is its own grid container, so a content-sized track lands at a different offset per row and the list stops reading as columns. `b`: at 360–430 px the Filters trigger, the result count, the order and the reverse button share one line, the count is never truncated, and the reverse button keeps its 44 px target despite the tighter padding that bought the row. `c`: a phone Compact row is shorter than the same row in Grid, the rating shares the priority control's line, and nothing leaves the viewport — `.title-row__body` is a **column** flex, so wrapping it wraps into new **columns**, which pushed 162 px of every row off-screen while the whole jsdom suite stayed green. `d`: **the single line is a `min-width` enhancement, not a floor** — at 280 px and 320 px the toolbar reflows with everything on screen and the 44 px target intact, because four controls do not fit one line there (the first attempt pushed the reverse button 11 px off a 280 px screen, `T-A11Y-015a`). `e`: **the filter trigger labels stay on one line at 390–1600 px, and each popover opens inside the panel.** The controls sit in `.dialog--panel`, capped at `26rem`, so viewport width says nothing about the room they have — a viewport media query widened the grid to SIX columns at 1024 px, putting six controls into ~400 px and wrapping every label to one character per line. It was correct at 320 px, where no override applied, and broke only on the big screen, so the wide widths are the whole point of the case. `f`: **a real TAP on a filter checkbox ticks it and leaves the popover open.** WebKit does not focus a checkbox, radio or button when it is tapped — focus drops onto `div.dialog--panel`, the dialog's `tabIndex={-1}` focus container — and `FilterDisclosure` read any focus outside itself as the owner tabbing away, so on a phone every filter shut on the very tap that ticked a box. The existing helpers use `dispatchEvent('click')`, which moves no focus at all and so could never see it. The case also pins that a touch popover does **not** open with a text field focused (that raises the on-screen keyboard over the checkboxes), that focus parking on a `tabIndex={-1}` container outside the picker does not close it, and that focusing a genuinely tabbable control outside still does. `g`: **with a filter active the toolbar rows are FULL, not half empty.** Reported by the owner from a phone — *"so much wasted space"* — with five rows ending 641 px down an 844 px screen. Two independent causes, both measured: a duplicate `.active-filters { flex-basis: 100% }` made the chip list claim the whole line (`w=340` for one ~110 px chip) so `Clear filters` wrapped beneath it, and the toolbar parts were placed by **column** with the row left to grid auto-placement — the cursor only moves forward, so the full-width chips row, which exists **only** when a filter is active, permanently pushed the count and the sort control off the Filters trigger's line. ⚠ The filtered case IS the case: cause 2 is invisible without a chip row, which is why `b` measures the default toolbar and never caught it. Rows are derived from measured overlap rather than from a height total, and the 300 px budget is a loose ceiling on a 357 px → 253 px improvement — a regression to five rows fails, a few pixels of padding does not. |
| `T-UX-148` | U | **US-003 AC-1/AC-2, REQ-002, REQ-003, issue #287.** `apps/web/test/uploadWizard.spec.tsx` cases `a`–`f` — the `/upload` progressive reveal (`specs/ui.md` §3.0). `a`: a locked step stays in the accessibility tree, `disabled` and `aria-describedby` its hint — hiding or unmounting it would pass by deleting the question, and a screen-reader owner would never learn the step exists. `b`: an answered step collapses to its answer beside a labelled `Change` that reopens it with the answer intact. `c`: unlocking step 2 leaves BOTH mode radios unchecked — a reveal must never pre-answer the step that proposes removals. `d`: changing the service CLEARS an agreed full update, because the consequence names the service and carrying it over would re-point a destructive choice at a list the owner never agreed to touch while the summary still read "Full update". `e`: re-choosing the same service keeps the mode, so backing out of a `Change` costs nothing. `f`: step 3 is never locked — the dropzone holds images pasted before the questions are answered (`ux-states.md` §4.3), so it carries a waiting hint instead. |
| `T-UX-149` | U | **REQ-016, REQ-018, NFR-008.** `apps/web/test/reviewRefetchHold.spec.tsx` cases `a`–`b` — the review list survives its own refetch. Reported by the owner mid-capture: every decision "would navigate me back to the top of the page rather than keeping me in line", which across ~19 rows meant re-finding their place after each one. The cause is structural, not a stray scroll call — each decision bumps the generation counter that keys `useResource`, that hook resets to `loading` on every key change, so `ReviewPage` swapped the entire list for the §6.1 skeleton and the collapsed document had no offset left to keep. `a`: with the post-decision refetch **held open**, the rows are still mounted and `review-loading` is absent — a refetch that resolves immediately repaints before anything can observe the gap, so holding it is the case. `b`: the limit on `a` — a **cold** first load still shows the skeleton, since suppressing it there would leave the owner staring at a blank screen. ⚠ The scroll offset itself is deliberately **not** asserted: jsdom does not lay out, `scrollY` is always 0, and such an assertion could never fail. |
| `T-UX-150` | U | **US-012 AC-3, REQ-014, `specs/ux-states.md` §6.7.** `apps/web/test/reviewPage.spec.tsx` cases `a`–`c` — the sticky bar's **"N to add"** counts what a close would WRITE, not the length of the additions section. Reported from a real pass: the owner decided four cards and the bar read *"11 to add"*, because it printed `sections.additions.count` — every row the extractor produced, discarded and still-pending rows included. That number sits directly above **Apply changes** and is the only check the owner has on a destructive action, so a count that disagrees with the close is worse than no count: it reads as authoritative while overstating what is about to happen. `a`: a **discarded** row is excluded — a discard writes nothing (REQ-012 keeps the candidate row, not a listing). `b`: a **pending** row is excluded too, since the close refuses the whole batch on it; the same case pins that a confirmed **unmatched** row IS counted, because a close writes it as an unresolved row. `c`: a local SD-11e decision moves the number before the refetch lands, for the same reason `T-UX-062d` exists. ⚠ The expectations are derived from `applicableCandidates` — the close's own function — not written as literals, so a change to the applied dispositions fails this file instead of leaving it agreeing with a rule that has moved. |
| `T-UX-151` | U | **REQ-122, US-008, `specs/ui.md` §5.3/§5.3a.** `apps/web/test/candidateEvidence.spec.tsx` cases `a`-`c` - a card the owner is asked to decide must SHOW what it is asking about. Reported from a phone, of a row in "Couldn't identify these": *"one with no title and no image - what am I confirming?"*. The screenshot carries the contradiction whole: the heading **"No title read from this tile"** printed directly above `GOOD LUCK VRACH FRANKENSHTEYN`, the text the reader had in fact read, beside an empty grey square. Two independent causes, both in `CandidateCard`. The heading was chosen on `displayName === null`, i.e. `match?.name ?? inferredTitle` - and **an unmatched row has neither by definition**, so the card claimed the reader had read nothing in precisely the rows whose purpose is to show the owner what it read; that line is a claim about the READER and is false whenever the tile produced text. And the tile thumbnail was rendered only for the two fabrication-adjacent verdicts, which §5.3a states as a floor, not a ceiling: a row with no TMDB match has no poster either, so every other no-match card fell through to the grey placeholder while its screenshot sat one fetch away. `a`: the read text IS the heading, and appears exactly ONCE - promoting it without suppressing the evidence line would read as two findings about one tile. `b`: the source tile renders instead of the placeholder. `c`: **the limit, and the reason the fix is not a deletion** - a genuine `unreadable-tile` (`rawText: ''`, `inferredTitle: null`) still says "No title read from this tile", because there it is the truth and an empty heading would be a worse version of the same defect. ⚠ `c` passes on the pre-fix code by construction; `a` and `b` were proven to fail on it. `T-REV-013f`/`j` pin the resulting ladder - poster, then tile, then placeholder only when there is no source image at all. |
| `T-UX-152` | E2E | **US-008 AC-2, REQ-014, NFR-008.** `tests/e2e/unmatchedResultLayout.spec.ts` cases `a`–`c` — the fix-match result row is a title and a **control**, not two blocks of prose. Reported by the owner mid-capture: *"I used the imdb search and the ui became difficult to interpret. the text was covering the button. I guessed at the confirm button."* ⚠ **Measured in a real browser because every claim is a layout fact jsdom cannot answer** — the component case for this row (`T-UNM-010f`) passed throughout, because the button did exist and did call its handler; what was false is that anyone could SEE it was a button. Measured at 390 px before the fix: the row was `nowrap` with both children at `flex: 0 1 auto`, so a 37-character title and a `Use {name}` label interpolated **from that same title** split the line into a 137 px column and a 158 px column and both reflowed to four lines — two adjacent walls of the same words, one of which happened to be clickable. `a`: the button is 83×44, wider than it is tall and still a 44 px target — a control taller than it is wide is prose with a border. `b`: the title gets more of the row than the control does, nothing overlaps, and nothing leaves the phone. `c`: **shortening the visible label did not cost the accessible name.** The eye reads `Use this`; the accessible name is still `Use {name}`. Without `c` the fix would trade a visual defect for a screen-reader one, presenting three results as three buttons called "Use this" — and `T-UNM-010f` finds this control by the name `use dune`. ⚠ All three were proven to fail against the original CSS block and the original interpolated label. TASK-262: below 640 px the review is the phone pager (`ui.md` §5.0a), whose result is one bordered row naming the title, so `a`–`c` run twice — the wide `Use this` row at 640 px, its narrowest drawn width, and the phone result row at 390 px, whose ceiling in `a` is 64 px because it is two lines by design (the title, then its year). |
| `T-UX-154` | I+U | **REQ-122, US-008, `specs/ui.md` §5.3a.** `apps/api/test/integration/batchReview.spec.ts` case `a` and `apps/web/test/candidateEvidence.spec.tsx` case `b` — the thumbnail on a decidable card must be the **tile**, not the whole pasted screenshot. Reported from a phone, of the fix-match prompt: *"it's asking me to confirm the match but the image shown is the entire screenshot that I pasted. that's not useful… if there are other titles that end up overlapping or are too similar, I won't know which tile I'm actually addressing."* ⚠ **This is `T-UX-151`'s floor-not-ceiling defect one layer down, and it was CREATED by fixing `T-UX-151`.** `tileCropFor` refused a crop for any verdict outside `inferred-unverified` / `unreadable-tile`, reading §5.3a's two-row table as an exhaustive list; §5.3a says those two MUST carry a thumbnail and nowhere says they are the only ones that MAY. While the client only rendered thumbnails for those same two verdicts the two gates agreed and the gap was invisible. `T-UX-151b` then widened the client to show the source tile for **every** card with no TMDB poster — i.e. every unmatched card — and those cards asked for a crop, got `null`, and fell through to the uncropped `<img>`. With ~20 tiles to a screenshot the result identifies the *batch*, not the *row*: the owner is answering "is this the right match?" about a tile they cannot locate. `a` is the failing evidence — an **unmatched `title-candidate`** with an LLM tile box is served a crop whose `w` and `h` are both `< 1`, a full-image rectangle being the defect wearing the fix's clothes. `b` pins that a crop which reaches the client renders through the clipping wrapper; ⚠ **`b` cannot fail on the defect and says so in the file** — a component fixture supplies `tileCrop` directly, so on pre-fix code it passes while the product is broken. What `b` buys is that `T-UX-151b`'s fixture (`tileCrop: null`) stops being the only unmatched-card render ever exercised, since that is a payload shape the server can no longer produce. ⚠ **`boxSource === 'llm'` was believed to be the real limit here and that belief was backwards** — see the `T-AI-041` row and `T-AI-055`. `'llm'` means *uncorroborated*, so the gate as written admitted only the reader's unverified geometry and excluded every measured box; the crop this row delivered was therefore wrong 8 times in 10, which is the owner's *second* report of the same symptom. The limit is now the opposite one, plus an agreement gate. ~~"`boxSource === 'llm'` is the real limit and survives untouched (`T-AI-041h`/`x`): an OCR box is a caption strip, and cropping to it fills the thumbnail with the `rawText` already printed beside it and shows none of the artwork."~~ (The reasoning in that sentence was sound; it was attached to the wrong branch. Its true half survives as `T-AI-041h`.) ⚠ `verdict` was **removed from `tileCropFor`'s signature**, not left unused — keeping it would leave the defect one line from returning and imply to callers that it is consulted; the two mandatory verdicts are enforced in `CandidateCard`'s `needsThumbnail`, which is what §5.3a actually constrains. Mutation-proven both ways: restoring the verdict gate fails `T-AI-041i` (domain) and `T-UX-154a` (integration, against a real SQL Server). `T-AI-041i` is itself **the inversion of its own former self** — it read *"a verdict with no mandatory thumbnail gets no crop"* and asserted the defect. |
| `T-UX-166` | U | **`specs/ui.md` §5.3a.** `apps/web/test/candidateThumbnail.spec.tsx` cases `a`–`b` — when no crop is trustworthy, the whole-screenshot thumbnail must **say** it is the whole screenshot. ⚠ **The no-crop path is not the neutral fallback it looks like.** Both thumbnails render in the same 96 px box and differ only in content, so a screenshot of ~20 tiles and a crop of one tile are the same kind of object beside *"is this the right match?"* — and the owner reads it as *this is the tile*, which is what they reported, twice, before `T-AI-055` fixed the crop rule. Refusing to crop is correct when the geometry is untrustworthy, and after the fix ~29 of ~48 candidates still take that path, so it is the common case rather than the exceptional one; labelling it is what turns a misleading picture into context. `b` keeps the caption meaningful by pinning that a **cropped** card carries neither the marker nor the caption — a label that appears on every card says nothing, and the failure is silent, since the cropped card still shows the right region and the only symptom is a label that has quietly stopped carrying information. ⚠ **NOT covered, deliberately and recorded in the file:** whether the uncropped image is `object-fit: contain`. Under `cover` a 16:9 screenshot in a square box is centre-cropped **by the browser**, so the "whole screenshot" becomes invisibly its own arbitrary crop — strictly worse than either honest option — but jsdom does not apply stylesheets, so no assertion in this file can see it. It lives in `apps/web/src/index.css` and would need a rendered browser. `a` is mutation-proven: changing the caption text fails it alone. |
| `T-UX-153` | U | **REQ-111, `specs/ui.md` §2.2.** `apps/web/test/titleRow.spec.tsx` cases `a`–`b` — the poster is requested at a resolution it is actually painted at. Reported by the owner: *"the images for the movies are not high quality."* ⚠ **TMDB's path segment IS the pixel width**, so `w154` is not a URL detail but a rendered-resolution setting: the browser is handed a 154 px-wide image whatever it intends to paint it at. In Grid the tile track is `repeat(auto-fill, minmax(28rem, 1fr))` and the poster takes `grid-column: 1` of a `0.8fr 1fr` split at `width: 100%`, so it paints at roughly 200 CSS px and upward — already a ~1.4× upscale in CSS pixels alone, and past 4× on the 2×/3× displays the owner actually uses. ⚠ **Why the whole suite missed it:** every existing poster assertion interpolates `TMDB_IMAGE_BASE`, so it agrees with whatever the constant happens to say — `w154`, or `w92`, keeps them all green. Nothing ever looked at the number. `a` therefore pins the literal widths and the density descriptors, without which a browser cannot choose between candidates and simply takes the first, leaving high-density displays on the 1× image. `b` is the property rather than the literal: every rendition offered must exceed REQ-111's 72 px floor, so shrinking the constant back toward the floor fails even if `a`'s literals are updated to match it. ⚠ **The floor is not the defect and must not be "fixed" by shrinking the box** — Compact sits exactly on 72 × 108 and `w154` is ample there, which is why this ships as a `srcset` rather than raising every view to `w500`: the browser spends the bytes only where the pixels are used. Mutation-proven three ways — reverting to `w154` fails `a`; `w45` fails `a` and `b`; deleting `srcSet` fails `a`, `b` and `T-UI-012d`. ⚠ `T-UI-012d` was *strengthened*, not relaxed, to accommodate this: `urlAttributes` returns `srcset` as one raw string holding several URLs, and comparing that string to an expected literal would have turned an NFR-010/009 assertion into a spelling test a second host could be appended to without failing. Each candidate is now split out and checked individually. |
...
| `T-BRAND-001` | U | **ADR-0014 Revision 3, TASK-242.** `apps/web/test/brands.spec.tsx` cases `a`–`g`: native-viewBox local data-URL artwork with matching geometry; closed eight-component/eight-asset sets; brand colours confined to assets; strict SVG element/attribute allow-list excluding executable or external content; no package/network fetching; exact CC0/Commons provenance and presentation records; decorative/named modes; SHA-256-pinned artwork rather than approximations. | ADR-0014 §2–§6 |
| `T-BRAND-002` | U | **ADR-0014 Revision 3.** `apps/web/test/brands.spec.tsx` cases `a`–`d`: canonical names for all eight services, unchanged by visual hiding; removal of the register restores visible service-name text even with `nameHidden`; each service renders exactly one local image beside its hidden name. | ADR-0014 §5 |
| `T-BRAND-003` | E | **TASK-242.** `tests/e2e/guidedCapture.spec.ts`: all eight chooser logos decode from bundled data URLs, preserve intrinsic proportions within equal bounded frames, retain visible canonical names and native selection at 280/390/1440px, make no external artwork requests and have no serious/critical axe violations. Existing library geometry cases cover badge/footer and Grid/Compact regressions. | ADR-0014 §4 |
...
| `T-API-030` | U + I | Submitted q is validated and applied as an owner-scoped, parameterized literal title search before filters, ordering, keyset paging, rating-refresh scope and runtime-hidden counts. Covers trim/blank/length/repeated/structured validation, case/accent-insensitive full-name and unmatched-text matching, literal SQL/LIKE punctuation, every sort/direction cursor, suppression/removed/other-owner exclusion and libraries beyond SQL parameter limits. Query/route units: `apps/api/test/unit/titlesQuery.spec.ts`, `titlesSortRoute.spec.ts`; real SQL integration: `apps/api/test/integration/titles.spec.ts` | `api.md` §6.2c; `ui-refresh.md` §5c |
| `T-API-031` | U | **The SPA shell is served `no-cache`; the content-hashed bundles under `/assets` are `public, max-age=31536000, immutable`** (`apps/api/test/unit/spaCache.spec.ts`). ⚠ **THIS IS A "THE DEPLOY DID NOT LAND" GUARD, NOT A PERFORMANCE ONE.** `express.static`'s default gave both the same `public, max-age=0`, which is exactly backwards: the immutable bundles were revalidated on every navigation, while `index.html` — the one file whose staleness hides an entire deploy — had no stronger instruction. A cached shell names the PREVIOUS build's hashed bundle, so the browser asks for last week's CSS by URL and the server correctly serves it; the screen is indistinguishable from a deploy that failed, on both desktop and phone, with no expiry. The owner reported exactly that on 2026-09-17 against a revision that provably contained the fix. ⚠ **Driven over real HTTP** (`a`–`e`): `express.static` writes `Cache-Control` itself, so a policy asserted on the helper alone passes with `setHeaders` never wired up. ⚠ `d` is the anti-overreach case — `immutable` is claimed for `/assets` ONLY, because that is the only directory Vite content-hashes; an unhashed `robots.txt` or `favicon.svg` given the same year is unreplaceable for a year. ⚠ `e` pins `no-cache` ≠ `no-store`: the shell keeps its validator and stays storable | `api.md` §1 |
...
### Owner-approved edition labels (TASK-244)

| Id | Level | What it asserts | Source |
|---|---|---|---|
| `T-EDITION-001` | U | Catalogue-typed edition recognition requires distinguishing source/search text; canonical film identity and original year remain unchanged. Validated labels merge without loss; malformed persisted data is not silently discarded. `packages/domain/test/editions.spec.ts`. | TASK-244 |
| `T-EDITION-002` | U | Live-shaped TMDB alternative-title lookup recognizes Vrach Frankenshteyn under movie 8836, carries full source evidence despite a shortened inference, caches bounded lookups and distinguishes absence from outages. Stage 3 keeps distinct cuts individually reviewable while collapsing repeated identical evidence. `apps/api/test/unit/clients/tmdbEditions.spec.ts`, `apps/api/test/unit/jobs/resolveCandidates.spec.ts`. | TASK-244 |
| `T-EDITION-003` | I | Real SQL close preserves one film and unchanged listings/dates, stores only confirmed labels with provenance, rolls back on failure, and does not create undo-blocking modifications for new titles. `apps/api/test/integration/batchClose.spec.ts`. | TASK-244 |
...
| `T-RANGE-002` | U | Visible Min/Max labels and unit-bearing values, per-handle accessible names and spoken values, clamp status without swapping, full-track reset removing `runtime`, a disclosed gapped saved selection that is not rewritten until a handle moves, Back/Forward positions from the URL, and g) a pointer drag writing the URL once on release (TASK-256). Existing `T-UX-123`/`T-UX-144` cases cover canonical tokens, chips, legacy expansion and preserved sort. `apps/web/test/filterBar.spec.tsx`. | TASK-246 |
...
### Owner-approved navigation and naming (TASK-250, #369; sidebar TASK-253)
...
| `T-UX-132` | U | *(Redefined in place again by TASK-255.)* Phone nav (outside the capture flow) = the tab bar's Library link plus Search, Filters and Menu buttons; ~~Phone header = Menu only;~~ wide header = Library, Import, Review + Menu; Menu is a dialog disclosure listing every destination with no nested *More*. ~~Phone bar = List, Upload, More.~~ | ui-refresh §6 REQ-117 |
...
| `T-UX-137` | U | *(Redefined in place.)* The retired `--nav-bar-height` token and bare `.nav` fixed positioning stay gone (TASK-255's phone tab bar is fixed only under `html[data-layout='phone'] .nav[data-tabs]`, with its own `--tab-bar-height` clearance — `T-PHONE-006c`); ~~No fixed nav at any width;~~ the bar clearance token and `scroll-padding-bottom` are gone; the drawer scrolls within the viewport and honours the safe-area inset; the Menu is present on every destination. ~~The phone bar is fixed to the bottom with matching clearance.~~ | ui-refresh §6 REQ-117 |
...
| `T-NAV-002` | E | *(Widths redefined in place: 1280 → 1023, the widest drawer width. Phone bar redefined in place by TASK-255: below 640 px the nav's one link is the tab bar's Library.)* At 320/390/640/1023 px in Chromium and Mobile Safari: hybrid bar, full drawer with hrefs and current page, no overflow, focus trap and return, link navigation closes the drawer, short-viewport scroll, and no serious or critical axe violation. | ui-refresh §6 REQ-117 |
...
| `T-UX-169` | U/E | *(TASK-256.)* a) the list route holds the genre facet while both the filtered and unfiltered reads are pending after a filter change, so the Genre pill stays mounted; b) *(E, 1280 px)* a mouse drag of the runtime minimum handle issues no `/api/titles` read and no URL write until release, then writes the range once; dragging back to the first stop leaves the open panel where it was and clears `runtime`; c) the stylesheet gives the root `scrollbar-gutter: stable`; d) *(E, 1280 px)* the engine computes the root's `scrollbar-gutter` as `stable`, and while a filter change shows the short loading skeleton the sidebar and list controls keep their horizontal position (headless browsers hide scrollbars, so the jolt itself is pinned by c and d together, not observed). `T-RANGE-002g` pins the release-only write in the unit suite. `apps/web/test/listRoute.spec.tsx`, `apps/web/test/filterBar.spec.tsx`, `apps/web/test/stylesheet.spec.ts`, `tests/e2e/refinedLibrary.spec.ts`. | ui.md §2.1 |
...
| `T-NAV-004` | U/E | *(TASK-261.)* The sign-out link is always reachable. a) at `--bp-lg` and up the header shows *Signed in as {name}* and a *Sign out* link to `/.auth/logout` beside the Primary nav, never inside it; b) at phone and `--bp-sm` widths it is absent from the header and present in the Menu drawer, below the destination list; c) with no display name the link still renders; d) it is a plain document link (no `aria-current`, no `target`, not a `nav__link`); e) *(E, 320/390/640/1023 px)* the drawer's *Sign out* is visible with that href; f) *(E, 1280 px)* the sidebar's is visible and outside the nav. `apps/web/test/signOut.spec.tsx`, `tests/e2e/navDrawer.spec.ts`. The landing after sign-out stays `T-AUTH-004`, a manual deployment check (§18). | ux-states §10.5, security.md §3 Sign-out |

### Owner-approved phone library mockup (TASK-255)

| Id | Level | What it asserts | Source |
|---|---|---|---|
| `T-PHONE-001` | U | Below `--bp-sm` the nav is a tab bar — a current Library link plus Search, Filters and Menu buttons — and the shell reserves room for it; the capture flow (`/upload`, `/batches/:id`) has Menu only; wide width has no tab bar; `html[data-layout]` mirrors `useWideViewport`. `apps/web/test/phoneLibrary.spec.tsx`. | TASK-255 |
| `T-PHONE-002` | U | The Search tab opens the library search (with no second trigger on the page) and the Filters tab opens the filters sheet, from the library or from another route (navigating to `/` first); a request is consumed once and never reopens on re-render. `apps/web/test/phoneLibrary.spec.tsx`. | TASK-255 |
| `T-PHONE-003` | U | The header avatar is the owner's initial with the name *Signed in as …*, is not a control, and is absent (not blank) without a display name. `apps/web/test/phoneLibrary.spec.tsx`. | TASK-255 |
| `T-PHONE-004` | U | The phone heading reads *My Library* with a decorative count (*at least* for a lower bound); Add title, Service updates and Filters keep accessible names as icon tools; the service row's short *All* is named *All services*. `apps/web/test/phoneLibrary.spec.tsx`. | TASK-255 |
| `T-PHONE-005` | U | The filters sheet's chips are native checkboxes that write the canonical URL (`service`, `category`, and the multi-select `status` from TASK-254); Reset clears every filter but keeps `q`; *Show N titles* counts and closes; genres past `PHONE_GENRE_LIMIT` sit behind a named More chip. `apps/web/test/phoneLibrary.spec.tsx`. | TASK-255 |
| `T-PHONE-006` | U | Every rule in the TASK-255 stylesheet block is scoped to `html[data-layout='phone']`; slim chips and tools are drawn on `::before` so the 44 px target stays; the tab-bar shell reserves `--tab-bar-height`. `apps/web/test/phoneLibrary.spec.tsx`. | TASK-255 |
| `T-PHONE-007` | E | At 390 and 320 px in Chromium and Mobile Safari: the tab bar is fixed to the bottom and inside the viewport, no horizontal overflow, the heading/tools/service chips/rows follow the mockup (title beside its status pill, facts line under both, rating and badges on one line), the Search and Filters tabs work and return focus to their tab when the search or sheet closes, every tab and chip is a 44 px target, the Added date shows on its own muted line under the rating and marks while genres stay hidden (owner decision 2026-09-25), the row menu never overlaps a row's content, and axe reports no serious or critical violations with the sheet open. The runtime slider in the sheet scrolls with the sheet body, and the *Show N titles* foot sits outside that scroller so it can cover nothing (`T-RANGE-003`). `tests/e2e/phoneLibrary.spec.ts`. | TASK-255 |
| `T-PHONE-008` | U | Below `--bp-sm` `/upload` is the owner's two-screen import (`ui.md` §3.0a). Screen 1 shows *Import your watchlist*, a Service/Mode/Screenshots stepper and the two questions with nothing pre-chosen. Continue is disabled and says why until both are answered; it opens screen 2 and focuses *Add screenshots*. Going back through the stepper keeps both answers and every held screenshot. The close control leaves for the library from screen 1 only. The wide layout has no stepper, no Continue and no paging. `apps/web/test/phoneImport.spec.tsx`. | TASK-260 |
| `T-PHONE-009` | U | The dropzone's Paste/Files/Drop tabs are pressed buttons, and *Choose files* and an enabled file input exist under every tab (invariant 16). The count reads *"n images added"* (singular at one). *Clear all* calls the container's discard, and is absent without one. Past six screenshots, a `+n` tile stands for the rest and reveals them; exactly six shows all six with no tile. `apps/web/test/phoneImport.spec.tsx`. | TASK-260 |
| `T-PHONE-010` | E | At 390 and 320 px in Chromium and Mobile Safari: `html[data-layout='phone']` is set and screen 2 is hidden until Continue. Continue waits with a visible reason, and every service tile, the close control and Continue are 44 px targets. On screen 2, every tab keeps *Choose files* and the drop target visible and the paste card holds the paste button. Eight images read *8 images added*, with five visible thumbnails and a `+3` tile that reveals the rest. The formats note sits under *Start extraction*. Back keeps the answers and the images, and *Clear all* empties the selection and disables submit with its reason. There is no horizontal overflow, and axe reports no serious or critical violation on either screen. `tests/e2e/phoneImport.spec.ts`. | TASK-260 |
| `T-PHONE-011` | U | Below `--bp-sm` `/review` is the owner's phone review overview (`ui.md` §5.0a). `a`: every visible candidate lands in exactly one group, taken from the server's sections. `b`: a full update keeps *Already in your library* on the overview (invariant 2). `c`: the chips filter the groups and are toggle buttons. `d`: ✓ on a new row confirms it at once and the row shows the outcome. `e`: the coverage card reports located of detected tiles, and *Learn more* lists every screenshot and keeps the caveat. `apps/web/test/phoneReview.spec.tsx`. | TASK-262 |
| `T-PHONE-012` | U | The phone pager, one candidate at a time. `a`: ✗ on a matched card opens the pager with the raw reading and the match score, while ✗ on an *Is this a title?* row discards at once. `b`: the pager confirms, shows its position in the group, moves focus to its heading, pages with Next, and *Back to review* returns focus to the opener. `c`: a refused close opens the first still-pending candidate. `d`: `ReviewPage` takes the phone path only when `phone` is set, and the wide page is unchanged without it. `apps/web/test/phoneReview.spec.tsx`. | TASK-262 |
| `T-PHONE-013` | E | At 390 and 320 px in Chromium and Mobile Safari (`tests/e2e/phoneReview.spec.ts`). `a`: `html[data-layout='phone']` is set, and the overview shows the heading, the service subtitle, the New and Uncertain groups, the saved and *probably not a title* rows and *Add all N clear matches*. Every filter chip is a 44 px target, there is no horizontal overflow, and axe reports no serious or critical violation. `b`: opening an uncertain candidate shows the pager with the screenshot hero, the raw reading and 44 px options, with no overflow, axe clean and Previous/Next present. *Back to review* returns to the overview and closes the pager. The review e2e specs also take the phone path below 640 px through `tests/e2e/phoneReviewSupport.ts`: `T-E2E-001` (`journey`, `captureJourney`), `groupedReview`, `tileReview`, `reviewPerformance`, `reviewContinuity`, `applyOutcome`, `finalReviewSummary` and `uploadPathRegression`, with their wide assertions unchanged. `T-UX-152` measures the wide `Use this` row at 640 px and the phone result row at 390 px. | TASK-262 |
