...
| — | C | `T-UX-069` | `specs/ux-states.md` §6.18: a 401 mid-review shows *"Your session ended. Sign in again — your review is still here."* with **Sign in** returning to this URL and local dispositions preserved. ⚠ **Catalogued but NOT separately implemented in v1.** A 401 is intercepted globally inside `apps/web/src/lib/apiClient.ts` (`request`), which redirects to sign-in before `ReviewRoute` ever sees the `ApiError` — so there is no review-local §6.18 branch and the review-local "your review is still here" preservation copy is unbuilt. Reported as a finding on the `T-UX-067` follow-up; not fixed here (scope discipline — the global redirect is the shared auth path). |
