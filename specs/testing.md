...
| `T-AI-067` | C+E | `apps/web/test/tileReview.spec.tsx` (`a`–`e`): every original beside its match, visible known/no-action states, other-service and uncertain next steps, same-tile fragments, alternatives correction, unique repeated-work DOM/search IDs and saved-decision counts. `tests/e2e/tileReview.spec.ts` (`f`): Chromium/WebKit at 280/390/1440px, source aspect ratio, no overflow, accessibility, persisted decisions after reload and final summary excluding known titles. |
...
| `T-MOCK-004` | C+E2E | Compact artwork Grid cards retain full titles, honest dates, missing data and actions; measure title-on-artwork placement, ordinary-card density, genres/rating row, badges/date footer, long-content growth, bright-artwork contrast and unchanged Compact geometry. `apps/web/test/listSurface.spec.tsx`, `tests/e2e/refinedLibrary.spec.ts`. |
| `T-MOCK-005` | C+E2E | Wide library heading, persistent search and controls share a compact header; service/filter choices remain canonical URL state. Phone disclosure, explicit search submit/clear, sorting, layout memory, keyboard and no-overflow behavior remain. `apps/web/test/listSearch.spec.tsx`, `tests/e2e/refinedLibrary.spec.ts`. |
...
| AC-2 | C | `T-REV-016` | "Already on your list" items have no addition/discard controls; deliberate identity correction is covered by `T-UX-162g`. |
...
| `T-BRAND-001` | U | **ADR-0014, issue #288.** `apps/web/test/brands.spec.tsx` cases `a`–`f` — the `components/brands/` register carries the icon register's contract, which is the entire price of excluding it from `T-A11Y-016d`. `a`: every mark renders `fill="currentColor"` on a `0 0 24 24` grid. `b`: the exported set is exactly the eight ADR-0014 marks, so the register cannot grow a file at a time. `c`: no colour literal anywhere in the directory — ⚠ the scan strips comments first, because `#288` is three hex digits and matches a `#rgb` pattern. `d`: no icon/brand **package** is imported and no `fetch`/URL appears — product invariant 10 forbids requesting anything from a streaming service. `e`: `ATTRIBUTION.md` names the CC0 source and a **pinned commit**, so a vendored mark can be re-vendored and audited. `f`: decorative by default, `role="img"` + `aria-label` when named. `g`: ⚠ **origin is asserted, not just shape** — the three owner-directed drawn marks declare themselves drawn and the five vendored ones declare their slug. Nothing at render time distinguishes them, so a contributor "improving" a drawn approximation by tracing the real logo would change the legal position (ADR-0014 Rev 2 §5) without changing any other test. | ADR-0014 §3 |
| `T-BRAND-002` | U | **ADR-0014, issue #288.** `apps/web/test/brands.spec.tsx` cases `a`–`d` — `ServiceMark` on both surfaces (`specs/ui.md` §2.2a). `a`: all **eight** services expose `SERVICE_LABELS[service]` as their accessible name. `b`: the accessible name is identical whether the name is visible or visually hidden — `nameHidden` controls visibility, never presence, and a logo-only badge is invisible to a screen reader *and* to find-in-page, neither of which a screenshot shows. `c`: ⚠ **asserts the word-mark fallback still works, with the register mocked empty** — all eight services now have a mark (ADR-0014 Rev 2), so that branch is unreachable in production and looks exactly like dead code, yet it IS the removal path the trademark position rests on: deleting a component and its `SERVICE_MARKS` entry must restore the word mark and change nothing else. `d`: every one of the eight renders exactly one `<svg>` beside the hidden name. | ADR-0014 §5/§6 |
