...
| AC | L | Test | Assertion |
|---|---|---|---|
| AC-1 | U | `T-WAIT-005` | A discovery review pass renders at all (`service: null`), routes **every** extracted candidate to a section — including the already-listed ones a SERVICE append-only pass omits — and leaves every disposition `pending`. |
...
| **`T-WAIT-005`** (`a`–`e`) | U | `TASK-186` | Every extracted candidate is shown and every disposition defaults to `pending` — no accept-by-inaction (US-041 AC-1, REQ-014). `a` the response carries `service: null` and names the discovery source; `b` every extracted candidate routes to a section; `c` **the discriminating case** — an already-listed work is SHOWN rather than omitted as it is in a SERVICE append-only pass; `d` every disposition defaults to `pending`; `e` the removal section is omitted and its label never names a service. |
