...
| **`T-WAIT-025`** (`a`–`c`) | E | `TASK-259` | In a real browser on both projects: `a`/`b` Grid at 320 and 1280 px puts at least two tiles on a line, keeps the forecast and a 44 px *Not interested* on every tile, does not overflow, and survives a reload; `c` at 320 px a title opens its details page with runtime, director, cast, writers, the forecast and JustWatch credit, a 44 px trailer link to YouTube in a new tab with `noopener noreferrer` and no iframe, no overflow, and *Back* returns to the list. `tests/e2e/waiting.spec.ts` |
