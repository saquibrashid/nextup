...
| `T-UX-169` | U/E | *(TASK-255.)* a) the list route holds the genre facet while both the filtered and unfiltered reads are pending after a filter change, so the Genre pill stays mounted; b) *(E, 1280 px)* a mouse drag of the runtime minimum handle issues no `/api/titles` read and no URL write until release, then writes the range once; dragging back to the first stop leaves the open panel where it was and clears `runtime`. `T-RANGE-002g` pins the release-only write in the unit suite. `apps/web/test/listRoute.spec.tsx`, `apps/web/test/filterBar.spec.tsx`, `tests/e2e/refinedLibrary.spec.ts`. | ui.md §2.1 |
