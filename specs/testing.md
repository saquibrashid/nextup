...
| — | U | **`T-MANUAL-016`**, `T-MANUAL-022`, `T-MANUAL-030` | *(reachability)* The row menu offers **Remove alongside Not interested**, not instead of it — the whole point of the feature, and the assertion that stops a later tidy-up collapsing the two decisions into one item. Disabled offline with the reason as text; absent entirely when unwired |
