...
| AC-4 | I/C | `T-LIST-023`, `T-UI-016`, `T-UX-139` | AND across dimensions, OR within a dimension; URL-driven pickers/chips preserve unrelated selections and ordering |
...
| **`T-UX-123`** | U | **REQ-035 · the runtime bucket filter** (`apps/web/test/filterBar.spec.tsx`, `packages/domain/test/titleRuntime.spec.ts`, and `titleRuntime.spec.ts` at integration for the SQL half). Selecting a bucket sets `runtime` in the query string; boundaries are **half-open**, so a 60-minute title is in `60-120` and **not** in `30-60`. ⚠ **The 60-minute case is the one that looks arbitrary and is not**: an inclusive upper bound puts one title in two buckets and any count over the buckets then contradicts the list it describes. ⚠ A zero or negative runtime lands in **no** bucket — asserted separately from the exactly-one-bucket partition case, which zero would otherwise break. ⚠ An unknown token is dropped client-side (a stale link must not be an error screen) while being a 400 at the API (`T-API-021`); the chip **names** the bucket rather than showing its wire token. |
