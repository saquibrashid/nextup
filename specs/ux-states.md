...
| Rule | Detail |
|---|---|
| **Never an indefinite spinner** | Any pending request that passes 1200 ms swaps the spinner for `SlowResponseNotice` — *"Still working…"* (renamed from `ColdStartNotice` / *"Waking things up…"* by `TASK-143`: `minReplicas = 1` means there is no cold start, so the old copy named a cause that cannot occur). Past **15 s** it becomes the `waking` notice — *"Waking the database — this takes up to a minute after a quiet spell. Nothing has been changed."* — still `role="status"`, still progress, **no Retry**. Past **70 s** it becomes the `slow` error state with a **Retry**. `T-UX-001`. <br> ⚠ **The `waking` phase does NOT restore the copy TASK-143 deleted, and must not be collapsed into it.** TASK-143's reasoning holds: the **app** never cold-starts. The **staging database** is Azure SQL serverless with `autoPauseDelay = 60` (ADR-0003 Rev 3) — paused on purpose, to bill nothing while idle — and a resume takes 30-60 s. `SQL_CONNECT_TIMEOUT_MS` is 60 s, so the server waits it out and the request *succeeds*; the client was declaring it stalled at 15 s and showing a failure with a Retry that could only hit the same wall. A healthy, expected, billed-for state was being reported as a fault. ⚠ **70 s must exceed that 60 s server connect timeout**; below it, the rule reports failure for requests about to succeed. Extending the threshold costs almost nothing because the phase machine only governs requests still *pending* — a request that genuinely fails rejects, and its caller renders the error at once without consulting any phase. <br> ~~Superseded: "Past 15 s it becomes the `slow` error state with a **Retry**."~~ |
...
| **2.11 Error — 403 (not allow-listed)** | Full-page *"This nextup instance isn't set up for this account."* + the signed-in email + **Sign out**. **No list data, no nav, no partial UI.** | Sign out | `T-UX-025` / `T-SEC-010` |
...
| **4.1 Initial** | Service and mode unselected. The attach area stays **enabled** — a paste or a drop arriving now is *held*, per §4.0a — and the visible reason sits on **submit**: `SUBMIT_NEEDS_SELECTION`, *"Choose a service and a mode first."* ~~*"Service and mode unselected; the attach area disabled with visible reason 'Choose a service and a mode first.'"*~~ — **superseded in place at A45: a disabled attach area cannot hold a paste**, so that wording directly contradicted §4.0a one row above it, and would have meant discarding the owner's screenshot for the ordinary case of pasting before picking a service. The sentence itself is unchanged; only what it disables moved | Choose; or paste/drop now and have it attach on choosing | `T-UX-040` |
...
| **6.16 Uncertain close response** | Saved-status check: applied/undone/discarded opens the durable batch outcome; verified in-review refreshes the summary before offering retry. Unreadable status never claims nothing changed and disables further writes. The summary and decisions remain visible. | Read-only **Check saved status**; verified explicit Apply retry or Back to review. Never reconnect replay. | `T-UX-067`, `T-UX-163a`–`c`, `T-UX-163g`, `T-UX-163i` |
