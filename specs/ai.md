...
Everything not caught by 2/3/3a/4/8 is `title-candidate`. Verdicts
...
| Screenshot bytes leave only for extraction (NFR-015) | **Two** destinations now: the Azure OpenAI endpoint and the Vision endpoint. `T-SEC-004` asserts no *other* outbound request carries an `image/*` content type or a data URI; `T-SEC-031` pins the outbound host allow-list to exactly **four** hosts (Azure OpenAI, Azure AI Vision, TMDB, **OMDb** — ADR-0011) **and pins the two image-bytes destinations to the two extractors**, so the list growing does not widen what a screenshot may reach. ~~Superseded (Epic M): "exactly three hosts (Azure OpenAI, Azure AI Vision, TMDB)."~~ |
