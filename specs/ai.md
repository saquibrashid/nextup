...
| Screenshot bytes leave only for extraction (NFR-015) | **Two** destinations now: the Azure OpenAI endpoint and the Vision endpoint. `T-SEC-004` asserts no *other* outbound request carries an `image/*` content type or a data URI; `T-SEC-031` pins the outbound host allow-list to exactly **five** hosts (Azure OpenAI, Azure AI Vision, TMDB, **OMDb** — ADR-0011, **Watchmode** — ADR-0010 Rev 3, sent a date window only) **and pins the two image-bytes destinations to the two extractors**, so the list growing does not widen what a screenshot may reach. ~~Superseded (Epic M): "exactly three hosts (Azure OpenAI, Azure AI Vision, TMDB)."~~ ~~Superseded (#380): "exactly four hosts (… OMDb)."~~ |
