...
| T18 | Information disclosure *(new — R2; amended Epic M and #380)* | Screenshot bytes reach a further host after a well-meaning change | Outbound host allow-list pinned to **exactly five** hosts (Azure OpenAI, Azure AI Vision, TMDB, OMDb, Watchmode — sent a date window only, ADR-0010 Rev 3), **and — the actual guarantee — every entry declares what it is sent, with exactly two (the extractors) permitted `image-bytes`.** ⚠ The count alone is *not* the mitigation: widening the list for OMDb, which receives an opaque `tt…` id and nothing else, is indistinguishable from widening it for a host the images are posted to. The payload classification is what T18 is about, and it survives the list growing again. **Not a destination:** the trailer link (#391) names `www.youtube.com` for the owner to click; it is exempt only in the one domain file that builds that link (`LINK_OUT_HOSTS`), and the same host anywhere else is still caught | `T-SEC-031` |
...
hosts (Azure OpenAI, Azure AI Vision, TMDB)."~~ The list is five since
ADR-0011 added OMDb and ADR-0010 Revision 3 added Watchmode (#380); the
wording is corrected in place rather than
...
| Vulnerability gate | **`npm audit --omit=dev --audit-level=high` in CI. High or critical in the RUNTIME tree blocks the merge.** A second, non-blocking `npm audit --audit-level=high` reports the full tree. Moderate and below are reported, not blocking. ~~`npm audit --audit-level=high` in CI. **High or critical blocks the merge.** Moderate and below are reported, not blocking~~ ⚠ Scoped 2026-08-13 for `GHSA-2v37-7h3g-55p8` (`nanoid <3.3.18`, high, reachable only via `postcss → vite → vitest`): the patched 3.3.18 **is not published on the Microsoft-directed registry this project installs from** (3.3.17 is the newest available), so the finding was **unfixable rather than unfixed** — and a gate that cannot be satisfied teaches the team to ignore a red build. The production tree reported **zero** vulnerabilities throughout. The dev-tree audit is retained and visible because a compromised build tool runs on every developer machine and in CI with repository credentials. **Restore the unscoped command when the registry mirrors 3.3.18.** |
...
| New dependency policy | Any new runtime dependency must be justified in the PR description against NFR-004 (mainstream, well-documented). The set is deliberately small: `express`, `zod`, **`@prisma/client` + `prisma`** *(R4 — provider `sqlserver`; still Prisma, see RSK-031 / ADR-0005 R3.4)*, `@azure/storage-blob`, `@azure/identity`, `@azure-rest/ai-vision-image-analysis`, **`openai`** *(TASK-056b — the `AzureOpenAI` client for the PRIMARY reader. `specs/ai.md` §2.1a has mandated this package, its API version and the single module allowed to import it since ADR-0001 Rev 2; its absence from this row was an omission, and this row is the one a CI gate reads. Managed identity only — the client is constructed with a bearer-token provider and **no `apiKey` exists anywhere**.)*, `ulid`, `jaro-winkler`, `compression`, `multer`, **`heic-convert`** *(A42 — HEIC/HEIF decode → PNG on ingest; pure JS/WASM, no native build; see the licence row below)* and **`sharp`** *(A42 — resize / dimension-clamp / metadata-strip of the decoded raster; prebuilt binaries, no HEIC decode needed from it. **⚠ TASK-147, corrected against the installed tree: `sharp` is Apache-2.0, NOT MIT, and its `@img/sharp-libvips-*` binaries are LGPL-3.0-or-later — so `sharp` carries a notice obligation of exactly the same kind as `libheif-js`, and `T-LICENSE-001` enforces it. Its 25 platform binaries are OPTIONAL dependencies, which is why the runtime container stage must copy `node_modules/@img` from the build stage — see `T-INFRA-006`.**)*, plus the SPA's own **`react`, `react-dom`, `react-router-dom`** (ADR-0004), **`@tanstack/react-virtual`** *(TASK-070 — `specs/ui.md` §5.4 SD-11c has named this package for the virtualised review list since Epic O; its absence from this row was an omission of the same kind as `openai`'s, and this row is the one a CI gate reads. MIT, headless, no transitive runtime dependencies. A 500-candidate review that renders every card is US-013 AC-5's failure mode on a 0.25 vCPU container.)* and **`@noble/hashes`** *(audited SHA-256 for canonical id derivation in `packages/domain`; deliberately not `node:crypto`, because the domain package is isomorphic and these ids are computed in the browser too)*. **This list is now a CI gate, not prose: `T-DEP-001` fails the build on any direct runtime dependency that is not on it** (`tools/check-deps.mjs`, `RUNTIME_DEPENDENCY_ALLOWLIST`). Adding a runtime dependency means editing this row first. **⚠ A48 / TASK-141 adds `@prisma/adapter-mssql`, and corrects a claim in this row that was disproven by execution.** The managed-identity path in §7 is **impossible** on Prisma's built-in `sqlserver` connector: that connector is the Rust **`tiberius`** driver, its URL grammar has no `authentication=` parameter, and Entra auth for it is unimplemented (prisma/prisma#12562, #13853, #7673 — all open). `@prisma/adapter-mssql` routes the same queries through Microsoft's Node **`mssql`/`tedious`** driver, which supports `azure-active-directory-default`; driver adapters are **GA** in Prisma 6.19 (no preview flag) and the adapter is published at our exact pinned version. Without it the container must hold a SQL password, so this dependency is what makes §7 achievable rather than aspirational. **Migrations deliberately stay on tiberius + the SQL admin login** — that credential is a deploy-time GitHub secret that never reaches the container, and migrations need DDL rights the app must not have. ~~**The Prisma `sqlserver` provider needs no extra npm package — it is built into `prisma`;** Microsoft's `mssql`/`tedious` driver is pulled in transitively by Prisma, not added directly~~ — **both halves of that sentence were wrong**: no extra package is needed only if managed identity is abandoned, and `mssql`/`tedious` is *not* a transitive dependency of Prisma at all (tiberius is Rust, compiled into the query engine), so it arrives only with the adapter |
